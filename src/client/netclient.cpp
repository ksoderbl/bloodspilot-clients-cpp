...
int Net_init(char *server, int port)
{
	...
	if (Sockbuf_init(&cbuf, NULL, CLIENT_RECV_SIZE, SOCKBUF_WRITE | SOCKBUF_READ | SOCKBUF_LOCK) == -1)
	{
...
int Receive_ship(void)
{
	...
	if ((n = Packet_scanf(&rbuf, "%c%hd%hd%hd"
								 "%c%c",
...
int Send_display_ng(int width, int height, int sparks, int spark_colors)
{
	...
	if (width_wanted == server_display.view_width &&
		...
		spark_colors == server_display.num_spark_colors && sparks == server_display.spark_rand && last_loops != 0)
	{
		return 0;
	}

	if (Packet_printf(&wbuf, "%c%hd%hd%c%c", PKT_DISPLAY, width_wanted, height_wanted, spark_colors,
					  sparks) == -1)
		return -1;
