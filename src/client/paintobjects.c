	...
	for (i = 0; i < max_; i++) {
		if (num_debris[i] > 0) {
			...
			for (j = 0; j < num_debris[i]; j++)
				Gui_paint_spark(color, x + debris_ptr[i][j].x,
						y - debris_ptr[i][j].y);
...
static void Paint_fastshots(int i, int x_areas, int y_areas, int areas)
{
	...
	if (num_fastshot[i] > 0) {
		...
		for (j = 0; j < num_fastshot[i]; j++)
			Gui_paint_fastshot(color, x + fastshot_ptr[i][j].x,
...
static void Paint_teamshots(int i, int t_, int x_areas, int y_areas, int areas)
{
	...
	if (num_fastshot[t_] > 0) {
		...
		for (j = 0; j < num_fastshot[t_]; j++)
			Gui_paint_teamshot(color, x + fastshot_ptr[t_][j].x,
...
static void Paint_all_ships(void)
{
	...
	if (num_ship > 0) {
		...
		for (i = 0; i < num_ship; i++) {
			...
			Gui_paint_ship(x, y,
				       ship_ptr[i].angle, ship_ptr[i].id,
				       ship_ptr[i].cloak, ship_ptr[i].phased,
				       ship_ptr[i].shield, ship_ptr[i].deflector,
				       ship_ptr[i].eshield);
...
static void Paint_connectors(void)
{
	...
	if (num_connector > 0) {
		...
		for (i = 0; i < num_connector; i++) {
			x_0 = connector_ptr[i].xa;
			y_0 = connector_ptr[i].ya;
			x_1 = connector_ptr[i].xb;
			y_1 = connector_ptr[i].yb;
			if (wrap(&x_0, &y_0) && wrap(&x_1, &y_1))
				Gui_paint_connector(x_0, y_0, x_1, y_1, connector_ptr[i].tractor);
		}
...
int Init_wreckage(void)
{
	...
	for (shp = 0; shp < NUM_WRECKAGE_SHAPES; shp++) {
		for (i = 0; i < NUM_WRECKAGE_POINTS; i++) {
			...
			Old_rotate_point(&wreckageShapes[shp][i][0]);
		}
	}
