...
static void Paint_meter(int xoff, int y, const char *title, int val, int max, int meter_color)
{
	...
	else
	{
		...
		xstr = (x) - (BORDER + ColorTextWidth(gameFont, title, (int)strlen(title)));
	}
...
static void Paint_lock(int hud_pos_x, int hud_pos_y)
{
	...
	if (hudColor != NO_COLOR)
	{
		...
		ColorDrawString(dpy, drawPixmap, gameGC, gameFont,
						...
						(hud_pos_y - hudSize + HUD_OFFSET - BORDER) - gameFont->descent,
		...
		if (!BIT(Setup->mode, LIMITED_LIVES) && lock_dist != 0)
		{
			...
			ColorDrawString(dpy, drawPixmap, gameGC, gameFont,
							...
							(hud_pos_y - hudSize + HUD_OFFSET - BORDER) - gameFont->descent, str, 3,
							color, shadowColor, BLACK,
							false, true);
		}
	}
...
#define hudVLineColor GREEN
	...
	if (hudColor != NO_COLOR && (fuelTime > 0.0 || fuelSum < fuelNotify))
	{
		...
		ColorDrawString(dpy, drawPixmap, gameGC, gameFont,
						(hud_pos_x + hudSize - HUD_OFFSET + BORDER),
						(hud_pos_y + hudSize - HUD_OFFSET + BORDER) + gameFont->ascent, str, (int)strlen(str),
						hudColor, shadowColor, BLACK,
						false, true);
		if (numItems[ITEM_TANK])
		{
			...
			ColorDrawString(dpy, drawPixmap, gameGC, gameFont,
							(hud_pos_x + hudSize - HUD_OFFSET + BORDER),
							(hud_pos_y + hudSize - HUD_OFFSET + BORDER) + gameFont->descent + 2 * gameFont->ascent, str,
							(int)strlen(str),
							hudColor, shadowColor, BLACK,
							false, true);
		}
	...
	if (have_hudmsg)
	{
		...
		ColorDrawString(dpy, drawPixmap, gameGC, gameFont,
						(hud_pos_x)-width / 2,
						(hud_pos_y - hudSize /*+ HUD_OFFSET */ - BORDER) - gameFont->descent, hudmsg, len,
	...
	if (hudColor != NO_COLOR)
	{
		...
		for (i = 0, j = 0; i < MAX_SCORE_OBJECTS; i++)
		{
			...
			if (sobj->hud_msg_len > 0)
			{
				...
				if (j == 0 && sobj->hud_msg_width > (2 * hudSize - HUD_OFFSET * 2) && (did_fuel /*|| hudVLineColor */))
					++j;
				ColorDrawString(dpy, drawPixmap, gameGC, gameFont,
								(hud_pos_x)-sobj->hud_msg_width / 2,
								(hud_pos_y + hudSize - HUD_OFFSET + BORDER) + gameFont->ascent + j * (gameFont->ascent + gameFont->descent),
		...
		if (time_left > 0)
		{
			...
			ColorDrawString(dpy, drawPixmap, gameGC, gameFont,
							(hud_pos_x - hudSize + HUD_OFFSET - BORDER) - size, (hud_pos_y - hudSize + HUD_OFFSET - BORDER) - gameFont->descent, str, (int)strlen(str),
							hudColor, shadowColor, BLACK,
							false, true);
		}
		...
		ColorDrawString(dpy, drawPixmap, gameGC, gameFont,
						(hud_pos_x - hudSize + HUD_OFFSET - BORDER) - ColorTextWidth(gameFont, modifiers, modlen),
						(hud_pos_y + hudSize - HUD_OFFSET + BORDER) + gameFont->ascent, modifiers, (int)strlen(modifiers),
						hudColor, shadowColor, BLACK,
						false, true);
		...
		if (autopilotLight)
		{
			...
			ColorDrawString(dpy, drawPixmap, gameGC, gameFont,
							...
							(hud_pos_y - hudSize + HUD_OFFSET - BORDER) - gameFont->descent * 2 - gameFont->ascent,
	...
	/* draw fuel gauge */
	if (fuelGaugeColor != NO_COLOR && ((fuelTime > 0.0) || (fuelSum < fuelNotify && ((fuelSum < fuelCritical && (twelveHz % 4) < 2) || (fuelSum < fuelWarning && fuelSum > fuelCritical && (twelveHz % 8) < 4) || (fuelSum > fuelWarning)))))
	{

		Set_fg_color(fuelGaugeColor);
		rd.drawRectangle(dpy, drawPixmap, gameGC,
						 (hud_pos_x + hudSize - HUD_OFFSET + FUEL_GAUGE_OFFSET) - 1,
						 (hud_pos_y - hudSize + HUD_OFFSET + FUEL_GAUGE_OFFSET) - 1,
		...
		rd.fillRectangle(dpy, drawPixmap, gameGC,
						 (hud_pos_x + hudSize - HUD_OFFSET + FUEL_GAUGE_OFFSET) + 1,
						 (hud_pos_y - hudSize + HUD_OFFSET + FUEL_GAUGE_OFFSET + HUD_FUEL_GAUGE_SIZE - size) + 1, (HUD_OFFSET - (2 * FUEL_GAUGE_OFFSET)), (size));
	}
}
