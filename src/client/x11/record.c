...
/* $Id: record.c,v 1.17 2007/10/09 22:37:33 kps Exp $ */
...
static void Dummy_paintItemSymbol(int type, Drawable drawable, GC mygc, int x, int y, int color)
{
	UNUSED_PARAM(type);
...
static void RWriteHeader(void)
{
	...
	Get_login_name(buf, sizeof(buf));
	RWriteString(buf);
	RWriteString(RECORD_HOST_NAME);
	 ...
	 */
	{
		...
		if (recordFPS > 0)
			...
			fps = FPS;
		Add_message("Started recording at %d FPS. [*Client notice*]", fps);
		RWriteByte(fps);
	}
	...
	putc(num_colors, recordFP);
	for (i = 0; i < num_colors; i++) {
		RWriteULong(Get_color_pixel(i));
...
static int RGetPixelIndex(unsigned long pixel)
{
	...
	for (i = 0; i < num_colors; i++) {
		if (pixel == Get_color_pixel(i))
			return i;
	}
	for (i = 1; i < num_colors; i++) {
		if (pixel == Get_color_pixel_xor(BLACK, i))
			return i + num_colors;
	}
...
static void RWriteTile(Pixmap tile)
{
	...
	for (y = 0; y < img->height; y++) {
		for (x = 0; x < img->width; x++) {
			...
			for (i = 0; i < num_colors - 1; i++) {	/* kps - why " - 1 " ? */
				if (pixel == Get_color_pixel(i))
					break;
			}
			RWriteByte(i);
...
static void REndFrame(void)
{
	if (damaged) {
		...
		if ((damaged & 1) != 0)
			Set_gc_fg_color(dpy, gameGC, BLUE);
		else
			Set_gc_fg_color(dpy, gameGC, BLACK);
...
static int RDrawLines(Display * display, Drawable drawable, GC gc, XPoint * points, int npoints,
		      int mode)
{
...
static int RDrawLine(Display * display, Drawable drawable, GC gc, int x_1, int y_1, int x_2,
		     int y_2)
{
...
static int RDrawRectangle(Display * display, Drawable drawable, GC gc, int x, int y, unsigned width,
			  unsigned height)
{
...
static int RDrawString(Display * display, Drawable drawable, GC gc, int x, int y,
...
static int RFillArc(Display * display, Drawable drawable, GC gc,
		    int x, int y, unsigned height, unsigned width, int angle1, int angle2)
{
...
static int RFillPolygon(Display * display, Drawable drawable, GC gc, XPoint * points, int npoints,
			int shape, int mode)
{
...
static void RPaintItemSymbol(int type, Drawable drawable, GC mygc, int x, int y, int color)
{
	UNUSED_PARAM(mygc);
	UNUSED_PARAM(color);
	if (drawable == drawPixmap) {
...
static int RFillRectangle(Display * display, Drawable drawable, GC gc, int x, int y, unsigned width,
			  unsigned height)
{
...
static int RFillRectangles(Display * display, Drawable drawable, GC gc, XRectangle * rectangles,
			   int nrectangles)
{
...
static int RDrawArcs(Display * display, Drawable drawable, GC gc, XArc * arcs, int narcs)
{
...
static int RDrawSegments(Display * display, Drawable drawable, GC gc, XSegment * segments,
			 int nsegments)
{
...
static int RSetDashes(Display * display, GC gc, int dash_offset, const char *dash_list, int n)
{
 ...
 */
typedef int (*draw_string_proto_t) (Display *, Drawable, GC, int, int, const char *, int);
...
void Record_cleanup(void)
{
	if (record_filename != NULL && record_frame_count > 0) {
		...
		printf("Recording size is %.2f MB (avg. %.2f kB/frame).\n",
		       (double) pos / 1e6, (1e-3 * pos) / (double) record_frame_count);
	}
}
