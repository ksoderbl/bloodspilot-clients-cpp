 ...
 *      Dick Balaska
 *
 * Copyright (C) 2003-2004 Kristian S�derblom
 *
...
int Gui_calculate_ship_color(other_t *other)
{
	...
	/* Team-mates are blue. */
	if (BIT(Setup->mode, TEAM_PLAY) && eyes != NULL && eyes->id != id && other != NULL && eyes->team == other->team)
		return BLUE;

	/* Allies are blue. */
	if (eyes != NULL && eyes->id != id && other != NULL && eyes->alliance != ' ' && eyes->alliance == other->alliance)
		return BLUE;
...
static int set_shipshape(int world_x, int world_y, float angle, shipshape_t *ship, XPoint *points)
{
	...
	for (cnt = 0; cnt < ship->shape.num_points; cnt++)
	{
		...
		xpts->y = (short)(((world.y + ext_view_height - world_y) / scaleFactor) - off_y);
		xpts++;
	}
	points[cnt++] = points[0];
					...
					int deflector, int eshield)
{
	...
	if ((!showShipShapes) && (self != NULL) && (self->id != id))
		cnt = set_shipshape(x, y, angle, Default_ship(), points);
	else if ((!showMyShipShape) && (self != NULL) && (self->id == id))
		cnt = set_shipshape(x, y, angle, Default_ship(), points);
	else
