...
{
	assert(pointerControl != on);

	if (on) {
		pointerControl = true;
		if (mouseAccelInClient)
			XChangePointerControl(dpy, True, True, new_acc_num, new_acc_denom,
					      new_threshold);
		XGrabPointer(dpy, drawWindow, True, 0, GrabModeAsync, GrabModeAsync, drawWindow,
			     pointerControlCursor, CurrentTime);
		XWarpPointer(dpy, None, drawWindow, 0, 0, 0, 0, (int) draw_width / 2,
			     (int) draw_height / 2);
...
void xevent_keyboard(int queued)
{
	...
	if (kdpy) {
		...
		for (i = 0; i < n; i++) {
			...
			default:
				warn("Unknown event type (%d) in xevent_keyboard", event.type);
				break;
			}
		}
	}
