...
{
	return "bloodspilot-client-ng-x11";
}
...
int main(int argc, char *argv[])
{
	 ...
	 */
	printf("  " COPYRIGHT ".\n"
	       "  " PACKAGE_STRING " comes with ABSOLUTELY NO WARRANTY; "
	...
	Store_messages_options();
	Store_X_options();
	Store_hud_options();
	Store_hudradar_options();
	Store_paintmessages_options();
	Store_paintscorelist_options();
	...
	if (xpArgs.text || xpArgs.auto_connect || argv[1]) {
		...
		result = Contact_servers(argc - 1, &argv[1],
					 ...
					 auto_shutdown, xpArgs.shutdown_reason, 0, NULL, NULL, NULL,
					 NULL, conpar);
	}
	else
