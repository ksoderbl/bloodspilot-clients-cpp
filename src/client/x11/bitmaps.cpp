...
int Bitmap_create(Drawable d, int img)
{
	...
	for (j = 0; j < ABS(pix->count); j++)
	{
		...
		if (pix->height == pix->picture.height && pix->width == pix->picture.width)
		{
...
static void Bitmap_blend_with_color(int img, int bmp, int rgb)
{
	...
	scaled = pix->height != pix->picture.height || pix->width != pix->picture.width;
...
static int Bitmap_init(int img)
{
	...
	for (j = 0; j < count; j++)
	{
		pixmaps[img].bitmaps[j].bitmap = pixmaps[img].bitmaps[j].mask = None;
...
static void Bitmap_picture_scale(xp_pixmap_t *xp_pixmap, int image)
{
	...
	for (y = 0; y < height; y++)
	{
		...
		{
			color =
				Picture_get_pixel_area(&(xp_pixmap->picture), image, x_scaled, y_scaled, dx_scaled,
									   dy_scaled);
 ...
 */
static void Bitmap_set_pixel(xp_pixmap_t *xp_pixmap, int bmp, int x, int y, RGB_COLOR color)
{
	unsigned long pixel;
	int r, g, b;
...
void Bitmap_paint_area(Drawable d, xp_bitmap_t *bit, int x, int y, irec_t *r)
{
	...
	XCopyArea(dpy, bit->bitmap, d, gameGC, r->x, r->y, (unsigned)r->w, (unsigned)r->h, x, y);
