...
void Config_key_event(char *ksname)
{
	...
	if (!strcasecmp(Option_value_to_string(opt), "none")
	    || !strcasecmp(Option_value_to_string(opt), "")) {
		/*
...
static void Create_config(void)
{
	...
	config_key_width = XTextWidth(buttonFont, "iiiiiiiiiiiiiiiiiii", 20)
	    + 2 * config_button_space;
			     ...
			     void *data)
{
	...
	keyw = Widget_create_key(widget_desc,
				 offset, *height
				 + (config_entry_height - config_key_height) / 2,
