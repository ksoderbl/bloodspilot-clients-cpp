...
/* $Id: dbuff.c,v 1.10 2007/09/27 23:31:05 kps Exp $ */
...
dbuff_state_t *start_dbuff(Display *display, Colormap xcolormap,
						   dbuff_t type, unsigned num_planes, XColor *colorarray)
{
	dbuff_state_t *state;
	...
	if (state->colormaps[1] == NULL || state->colormaps[0] == NULL || state->planes == NULL)
	{
	...
	case COLOR_SWITCH:
		if (XAllocColorCells(state->display,
							 state->xcolormap, False, state->planes, 2 * num_planes,
							 &state->pixel, 1) == 0)
		{
			dbuff_release(state);
			return NULL;
	...
	case MULTIBUFFER:
		...
		if (!XdbeQueryExtension(display, &state->dbe.dbe_major, &state->dbe.dbe_minor))
		{
			dbuff_release(state);
		...
		if (!XmbufQueryExtension(display, &state->mbx.mbx_ev_base, &state->mbx.mbx_err_base))
		{
			dbuff_release(state);
	...
	else if (num_planes > 1)
	{
		for (i = 0; i < (1 << num_planes); i++)
		{
			...
			{
				while (--i >= 0)
					XFreeColors(display, xcolormap, &colorarray[i].pixel, 1, 0);
				dbuff_release(state);
				return NULL;
			}
		}
	...
	if (state->type == COLOR_SWITCH)
		XStoreColors(state->display,
					 state->xcolormap, state->colormaps[state->colormap_index],
					 state->colormap_size);
...
void dbuff_init_buffer(dbuff_state_t *state)
{
	...
	if (state->type == MULTIBUFFER)
	{
		if (state->colormap_index == 2)
		{
			state->colormap_index = 0;
			state->dbe.dbe_draw =
				XdbeAllocateBackBufferName(state->display, drawWindow, XdbeBackground);
...
void dbuff_switch(dbuff_state_t *state)
{
	...
	else if (state->type == MULTIBUFFER)
	{
		...
		XmbufDisplayBuffers(state->display, 1, &state->mbx.mbx_draw[state->colormap_index],
							0, 200);
#endif
...
void end_dbuff(dbuff_state_t *state)
{
	if (state->type == COLOR_SWITCH)
		XFreeColors(state->display, state->xcolormap,
					&state->pixel, 1,
					~(state->drawing_plane_masks[0] & state->drawing_plane_masks[1]));
	dbuff_release(state);
}
...
static void dbuff_list_dbe(Display *display)
{
	...
	for (i = 0; i < n; i++)
	{
		...
		for (j = 0; j < info[i].count; j++)
		{
			...
			printf("    0x%02x  %6d  %8d\n",
				   (unsigned)visinfo->visual, visinfo->depth, visinfo->perflevel);
		}
	}
