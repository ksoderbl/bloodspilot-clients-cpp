...
typedef struct widget_key {
	bool pressed;		/* If button press active */
	bool inside;		/* If pointer inside window */
	const char *val;	/* Key(s) as string */
...
typedef struct widget_viewer {
	...
	int len,
	    vert_slider_desc,
	    hori_slider_desc,
	    save_button_desc, close_button_desc, visible_x, visible_y, real_width, real_height,
	    max_width, num_lines;
	viewer_line_t *line;
	XFontStruct *font;
} widget_viewer_t;
...
static int buttonColor;		/* Color index for button drawing */
static int windowColor;		/* Color index for window drawing */
static int borderColor;		/* Color index for border drawing */

static xp_option_t widget_options[] = {
	COLOR_INDEX_OPTION("windowColor",
			   DARK_BLUE,
			   &windowColor,
			   "Which color number to use for drawing windows.\n"),

	COLOR_INDEX_OPTION("buttonColor",
			   GRAY,
			   &buttonColor,
			   "Which color number to use for drawing buttons.\n"),

	COLOR_INDEX_OPTION("borderColor",
			   LIGHT_GRAY,
			   &borderColor,
			   "Which color number to use for drawing borders.\n"),
};

void Store_widget_options(void)
{
	STORE_OPTIONS(widget_options);
}
...
static int Widget_create(widget_type_t type, const char *name, Window window, int width, int height,
			 void *sub)
{
	int desc = NO_WIDGET;
	widget_t *widget;
...
static int Widget_add_child(int parent_desc, int child_desc)
{
	...
	if (form->num_children == 0) {
		...
		form->children = XREALLOC(int, form->children, (form->num_children + incr));
	}
	if (form->children == NULL) {
		form->num_children = 0;
...
static void Widget_draw_button(widget_t * widget, bool inverse, const char *label)
{
	...
	Set_gc_fg_color(dpy, buttonGC, bg_color);
	XFillRectangle(dpy, widget->window, buttonGC, 0, 0, widget->width, widget->height);
	Set_gc_fg_color(dpy, buttonGC, buttonColor);
	...
	x = (widget->width - ColorTextWidth(buttonFont, label, (int) strlen(label))) / 2;
	ColorDrawString(dpy, widget->window, buttonGC, buttonFont,
			x, buttonFont->ascent + y, label, strlen(label),
			fg_color, bg_color, BLACK,
			true, false);
}
...
static void Widget_draw_input(widget_t * widget, const char *str)
{
	...
	XDrawString(dpy, widget->window, textGC,
		    ...
		    textFont->ascent + (widget->height -
					(textFont->ascent + textFont->descent)) / 2, str,
		    strlen(str));
}
...
static void Widget_draw_color(widget_t * widget, const char *str, int color)
{
	...
	XSetWindowBackground(dpy, widget->window, Get_color_pixel(color));

	if (Get_color_pixel(color) == Get_color_pixel(WHITE))
		/* change the text color to black */
		Set_gc_fg_color(dpy, textGC, BLACK);
	else
		/* change the text color to white */
		Set_gc_fg_color(dpy, textGC, WHITE);
...
{
	int bg, fg, left, right, top, bottom;
	XPoint pts[4];
	...
	Set_gc_fg_color(dpy, buttonGC, bg);
	XFillRectangle(dpy, widget->window, buttonGC, 0, 0, widget->width, widget->height);
	Set_gc_fg_color(dpy, buttonGC, fg);
	pts[0].x = left;
	...
	pts[3].x = pts[0].x;
	pts[3].y = pts[0].y;
	XFillPolygon(dpy, widget->window, buttonGC, pts, 4, Convex, CoordModeOrigin);
	Set_gc_fg_color(dpy, buttonGC, WHITE);
}

static void Widget_draw_slider(widget_t * widget)
{
	int i, unit, block_size, block_max_size, block_offset, block_start;
	XPoint pts[4];
	...
	if (widget->type == WIDGET_SLIDER_HORI) {
		...
		XFillRectangle(dpy, widget->window, buttonGC,
			       block_offset + block_start, unit / 4, block_size, unit / 2);
		Set_gc_fg_color(dpy, buttonGC, BLACK);
		XFillRectangle(dpy, widget->window, buttonGC, block_offset, unit / 4, block_start,
			       unit / 2);
		XFillRectangle(dpy, widget->window, buttonGC,
		...
		XFillRectangle(dpy, widget->window, buttonGC,
			       unit / 4, block_offset + block_start, unit / 2, block_size);
		Set_gc_fg_color(dpy, buttonGC, BLACK);
		XFillRectangle(dpy, widget->window, buttonGC, unit / 4, block_offset, unit / 2,
			       block_start);
		XFillRectangle(dpy, widget->window, buttonGC, unit / 4,
			       block_offset + block_start + block_size, unit / 2,
			       block_max_size - (block_start + block_size));
	}
	Set_gc_fg_color(dpy, buttonGC, WHITE);
}
...
static void Widget_viewer_draw_lines(widget_t * widget, int start, int end)
{
	...
	int text_height = viewerw->font->ascent
	    + viewerw->font->descent, i, x = 20, y =
	    20 + viewerw->font->ascent + start * text_height;

	for (i = start; i < end; i++) {
		Set_gc_fg_color(dpy, motdGC, BLACK);
		XDrawString(dpy, widget->window, motdGC, x + 1, y + 1, viewerw->line[i].txt,
			    viewerw->line[i].len);
		Set_gc_fg_color(dpy, motdGC, WHITE);
		XDrawString(dpy, widget->window, motdGC, x - 1, y - 1, viewerw->line[i].txt,
			    viewerw->line[i].len);
		y += text_height;
	}
...
{
	widget_viewer_t *viewerw = (widget_viewer_t *) widget->sub;
	int start, end, y_0, y_1, text_height = viewerw->font->ascent + viewerw->font->descent;
...
static void Widget_draw_expose(int widget_desc, XExposeEvent * expose)
{
	...
	case WIDGET_LABEL:
		...
		ColorDrawString(dpy, widget->window, textGC, textFont,
				labelw->x_offset, textFont->ascent + labelw->y_offset,
				labelw->str, strlen(labelw->str),
				WHITE, BLACK, BLACK,
				true, false);
		break;
	...
	case WIDGET_BUTTON_BOOL:
		...
		Widget_draw_button(widget,
				   (boolw->pressed
				    && boolw->inside) ? true : false,
				   (boolw->state ? "Yes" : "No"));
		break;

	case WIDGET_BUTTON_KEY:
		if (expose && expose->count > 0)
			break;
		keyw = (widget_key_t *) widget->sub;
		Widget_draw_button(widget, (keyw->pressed && keyw->inside) ? true : false,
				   keyw->val);
		break;
	...
	case WIDGET_BUTTON_ACTIVATE:
		...
		Widget_draw_button(widget, (activw->pressed && activw->inside) ? true : false,
				   activw->str);
		break;
	...
	case WIDGET_INPUT_COLOR:
		...
		Color_index_to_string(*colorw->val, buf, sizeof(buf));
...
{
	int unit, new_x, new_y, block_size, block_max_size, block_start, block_new_start,
	    block_offset;
	widget_slider_t *sliderw = (widget_slider_t *) widget->sub;
	...
	if (widget->type == WIDGET_SLIDER_VERT) {
		...
		block_size = block_max_size * viewer_widget->height / viewerw->real_height;
		block_start =
		    block_offset + block_max_size * viewerw->visible_y / viewerw->real_height;
		...
		if (block_new_start != block_start) {
			...
			XMoveWindow(dpy, viewer_widget->window, -viewerw->visible_x,
				    -viewerw->visible_y);
			Widget_draw_slider(widget);
		}
	}
		...
		block_size = block_max_size * viewer_widget->width / viewerw->real_width;
		block_start =
		    block_offset + block_max_size * viewerw->visible_x / viewerw->real_width;
		block_new_start = block_start;
...
{
	widget_t *widget, *entry_widget, *pulldown_widget, *sub_widget;
	widget_bool_t *boolw;
	widget_key_t *keyw;
	widget_menu_t *menuw;
	widget_form_t *pullw;
	...
	widget_arrow_t *arroww;
	widget_entry_t *entryw;
	int i, ival, cval, sub_widget_desc;
	double dval, delta, dmin, offset, newoffset;
	...
	case WIDGET_BUTTON_BOOL:
		...
		if (pressed == false) {
			if (boolw->callback) {
				if ((*boolw->callback) (widget_desc, boolw->user_data,
							&boolw->state) == 1)
					Widget_draw(widget_desc);
			}
		}
	...
	case WIDGET_BUTTON_KEY:
		keyw = (widget_key_t *) widget->sub;
		if (keyw->pressed == false && pressed == false) {
			warn("Key widget not pressed");
			break;
		}
		keyw->pressed = pressed;
		if (keyw->inside == false)
			break;

#if 0
		if (pressed == false)
			keyw->state = (keyw->state) ? false : true;
#endif

		Widget_draw(widget_desc);
		if (pressed == false) {
			if (keyw->callback) {
				if ((*keyw->callback) (widget_desc, keyw->user_data) == 1)
					Widget_draw(widget_desc);
			}
		}
		break;
	case WIDGET_BUTTON_MENU:
		...
		if (pressed) {
			...
			for (i = 0; i < pullw->num_children; i++) {
				...
				entryw->inside = false;
				if (entryw->callback)
					(*entryw->callback) (pullw->children[i], entryw->user_data,
							     &entryw->str);
			}
		}
		break;
	case WIDGET_BUTTON_ACTIVATE:
		...
		if (pressed == false && activw->callback) {
			if ((*activw->callback) (widget_desc, activw->user_data, &activw->str) == 1)
				Widget_draw(widget_desc);
		}
		break;
	case WIDGET_BUTTON_ARROW_LEFT:
...
static int Widget_form_window(Window window, int parent_desc, int width, int height)
{
	int widget_desc;
	...
	widget_desc = Widget_create(WIDGET_FORM, "form", window, width, height, formw);
...
int Widget_create_form(int parent_desc, Window parent_window, int x, int y, int width, int height,
		       int border)
{
	Window window;
	widget_t *parent_widget;
	...
	window =
	    XCreateSimpleWindow(dpy, parent_window,
				x, y, width, height,
				border, Get_color_pixel(borderColor), Get_color_pixel(windowColor));
...
int Widget_create_activate(int parent_desc,
			   ...
			   int border, const char *str, int (*callback) (int, void *,
									 const char **),
			   void *user_data)
{
	int widget_desc;
	...
	window =
	    XCreateSimpleWindow(dpy, parent_widget->window,
				x, y, width, height,
				border, Get_color_pixel(borderColor), Get_color_pixel(buttonColor));
	XSelectInput(dpy, window,
		     ExposureMask | ButtonPressMask | ButtonReleaseMask
		     | OwnerGrabButtonMask | EnterWindowMask | LeaveWindowMask);
	widget_desc =
	    Widget_create(WIDGET_BUTTON_ACTIVATE, "activate", window, width, height, activw);
	if (widget_desc == NO_WIDGET)
		return NO_WIDGET;
...
int Widget_create_bool(int parent_desc,
		       int x, int y, int width, int height,
		       int border, bool val, int (*callback) (int, void *, bool *), void *user_data)
{
	int widget_desc;
	Window window;
	widget_t *parent_widget;
	...
	XSelectInput(dpy, window,
		     ExposureMask | ButtonPressMask | ButtonReleaseMask
		     | OwnerGrabButtonMask | EnterWindowMask | LeaveWindowMask);
	widget_desc = Widget_create(WIDGET_BUTTON_BOOL, "bool", window, width, height, boolw);
	if (widget_desc == NO_WIDGET)
		return NO_WIDGET;
...
int Widget_create_key(int parent_desc,
		      int x, int y, int width, int height,
		      int border, const char *val, int (*callback) (int, void *), void *user_data)
{
	int widget_desc;
	Window window;
	widget_t *parent_widget;
	...
	if ((parent_widget = Widget_pointer(parent_desc)) == NULL
	    || parent_widget->type != WIDGET_FORM) {
		warn("Widget_create_key: Invalid parent widget");
		return NO_WIDGET;
	}
	if ((keyw = XMALLOC(widget_key_t, 1)) == NULL) {
		error("No memory for key widget");
		return NO_WIDGET;
	}
	keyw->pressed = false;
	keyw->inside = false;
	keyw->val = val;
	keyw->callback = callback;
	keyw->user_data = user_data;
	window =
	    XCreateSimpleWindow(dpy, parent_widget->window,
				x, y, width, height,
				border, Get_color_pixel(borderColor), Get_color_pixel(buttonColor));
	XSelectInput(dpy, window,
		     ExposureMask | ButtonPressMask | ButtonReleaseMask
		     | OwnerGrabButtonMask | EnterWindowMask | LeaveWindowMask);
	widget_desc = Widget_create(WIDGET_BUTTON_KEY, "key", window, width, height, keyw);
	if (widget_desc == NO_WIDGET)
		return NO_WIDGET;
...
int Widget_add_pulldown_entry(int menu_desc, const char *str,
			      int (*callback) (int, void *, const char **), void *user_data)
{
	int entry_desc, pulldown_desc, width, height, pull_width, pull_height;
	widget_t *menu_widget, *pulldown_widget;
	widget_menu_t *menuw;
	widget_form_t *pullw;
	widget_entry_t *entryw;
	Window window;
	...
	if (pulldown_desc == NO_WIDGET) {
		mask = 0;
		sattr.background_pixel = Get_color_pixel(windowColor);
		mask |= CWBackPixel;
		sattr.border_pixel = Get_color_pixel(borderColor);
		mask |= CWBorderPixel;
		...
		window = XCreateWindow(dpy,
				       ...
				       0, 0, pull_width, pull_height, 0, dispDepth, InputOutput,
				       visual, mask, &sattr);
		pulldown_desc = Widget_form_window(window, NO_WIDGET, pull_width, pull_height);
	...
	window =
	    XCreateSimpleWindow(dpy, pulldown_widget->window,
				...
				width, height, 1, Get_color_pixel(borderColor),
				Get_color_pixel(buttonColor));
	XSelectInput(dpy, window, ExposureMask | EnterWindowMask | LeaveWindowMask);
	entry_desc = Widget_create(WIDGET_BUTTON_ENTRY, "entry", window, width, height, entryw);
...
int Widget_create_menu(int parent_desc, int x, int y, int width, int height, int border,
		       const char *str)
{
	int widget_desc;
	Window window;
	...
	XSelectInput(dpy, window,
		     ExposureMask | ButtonPressMask | ButtonReleaseMask
		     | OwnerGrabButtonMask | EnterWindowMask | LeaveWindowMask);
	widget_desc = Widget_create(WIDGET_BUTTON_MENU, "menu", window, width, height, menuw);
	if (widget_desc == NO_WIDGET)
		return NO_WIDGET;
	if (Widget_add_child(parent_desc, widget_desc) == NO_WIDGET) {
		Widget_destroy(widget_desc);
...
int Widget_create_int(int parent_desc,
		      int x, int y, int width, int height,
		      int border, int *val, int min, int max, int (*callback) (int, void *, int *),
		      void *user_data)
{
	int widget_desc;
	Window window;
	...
	window =
	    XCreateSimpleWindow(dpy, parent_widget->window,
				x, y, width, height, border, Get_color_pixel(borderColor),
				Get_color_pixel(BLACK));
	XSelectInput(dpy, window, ExposureMask);
	widget_desc = Widget_create(WIDGET_INPUT_INT, "input_int", window, width, height, intw);
	if (widget_desc == NO_WIDGET) {
		return NO_WIDGET;
	}
	if (Widget_add_child(parent_desc, widget_desc) == NO_WIDGET) {
...
int Widget_create_color(int parent_desc, int color,
			int x, int y, int width, int height,
			int border, int *val, int min, int max, int (*callback) (int, void *,
										 int *),
			void *user_data)
{
	int widget_desc;
	...
	window =
	    XCreateSimpleWindow(dpy, parent_widget->window,
				x, y, width, height, border, Get_color_pixel(color),
				Get_color_pixel(color));

	XSelectInput(dpy, window, ExposureMask);

	widget_desc =
	    Widget_create(WIDGET_INPUT_COLOR, "input_color", window, width, height, colorw);
	if (widget_desc == NO_WIDGET)
		return NO_WIDGET;
	if (Widget_add_child(parent_desc, widget_desc) == NO_WIDGET) {
		Widget_destroy(widget_desc);
...
int Widget_create_double(int parent_desc,
			 ...
			 int (*callback) (int, void *, double *), void *user_data)
{
	int widget_desc;
	Window window;
	widget_t *parent_widget;
	...
	XSelectInput(dpy, window, ExposureMask);
	widget_desc =
	    Widget_create(WIDGET_INPUT_DOUBLE, "input_double", window, width, height, doublew);
	if (widget_desc == NO_WIDGET)
		return NO_WIDGET;
	if (Widget_add_child(parent_desc, widget_desc) == NO_WIDGET) {
		Widget_destroy(widget_desc);
...
int Widget_create_label(int parent_desc,
			int x, int y, int width, int height, bool centered, int border,
			const char *str)
{
	int widget_desc;
	Window window;
	...
	window =
	    XCreateSimpleWindow(dpy, parent_widget->window,
				...
				border, Get_color_pixel(borderColor), Get_color_pixel(windowColor));
	XSelectInput(dpy, window, ExposureMask);
	widget_desc = Widget_create(WIDGET_LABEL, "label", window, width, height, labelw);
	if (widget_desc == NO_WIDGET)
		return NO_WIDGET;
	if (Widget_add_child(parent_desc, widget_desc) == NO_WIDGET) {
		Widget_destroy(widget_desc);
...
int Widget_create_colored_label(int parent_desc,
				int x, int y,
				int width, int height, bool centered, int border, int bg, int bord,
				const char *str)
{
	int widget_desc;
	Window window;
	...
	window =
	    XCreateSimpleWindow(dpy, parent_widget->window,
				x, y, width, height, border, Get_color_pixel(bord),
				Get_color_pixel(bg));
	XSelectInput(dpy, window, ExposureMask);
	widget_desc = Widget_create(WIDGET_LABEL, "label", window, width, height, labelw);
	if (widget_desc == NO_WIDGET)
...
static int Widget_create_arrow(widget_type_t type, int parent_desc,
			       int x, int y, int width, int height, int border, int related_desc)
{
	int widget_desc;
	Window window;
	widget_t *parent_widget;
	...
	window =
	    XCreateSimpleWindow(dpy, parent_widget->window,
				...
				Get_color_pixel(BLACK));
	XSelectInput(dpy, window,
		     ExposureMask | ButtonPressMask | ButtonReleaseMask | OwnerGrabButtonMask |
		     EnterWindowMask | LeaveWindowMask);
	widget_desc = Widget_create(type, "arrow", window, width, height, arroww);
	if (widget_desc == NO_WIDGET)
		return NO_WIDGET;
	if (Widget_add_child(parent_desc, widget_desc) == NO_WIDGET) {
		Widget_destroy(widget_desc);
...
int Widget_create_arrow_right(int parent_desc, int x, int y, int width, int height, int border,
			      int related_desc)
{
	return Widget_create_arrow(WIDGET_BUTTON_ARROW_RIGHT, parent_desc, x, y, width, height,
				   border, related_desc);
}

int Widget_create_arrow_left(int parent_desc, int x, int y, int width, int height, int border,
			     int related_desc)
{
	return Widget_create_arrow(WIDGET_BUTTON_ARROW_LEFT, parent_desc, x, y, width, height,
				   border, related_desc);
}

int Widget_create_popup(int width, int height, int border, const char *window_name,
			const char *icon_name)
{
	int disp_width = DisplayWidth(dpy, 0), disp_height =
	    DisplayHeight(dpy, 0), x, y, popup_desc;
	widget_t *widget;
	Window window;
	XSetWindowAttributes sattr;
	unsigned long mask;
	...
	window = XCreateWindow(dpy,
			       DefaultRootWindow(dpy),
			       x, y, width, height, border, dispDepth, InputOutput, visual, mask,
			       &sattr);
	popup_desc = Widget_form_window(window, NO_WIDGET, width, height);
...
int Widget_create_confirm(const char *confirm_str, int (*callback) (int, void *, const char **))
{
	...
	const int label_height = textFont->ascent + textFont->descent,
	    ...
	    popup_width = MAX(label_width + 2 * label_space, button_width + 2 * button_space);
	int popup_desc, label_desc, button_desc;

	popup_desc = Widget_create_popup(popup_width, popup_height, 1, "Confirm", "Confirm");
	if (popup_desc == NO_WIDGET)
		return NO_WIDGET;
	label_desc = Widget_create_label(popup_desc,
					 (popup_width - label_width) / 2,
					 label_space, label_width, label_height, true, 0,
					 confirm_str);
	...
	button_desc = Widget_create_activate(popup_desc,
					     ...
					     0, button_str, callback, (void *) (long) popup_desc);
...
int Widget_set_background(int widget_desc, int bgcolor)
{
	...
	if ((widget = Widget_pointer(widget_desc)) == NULL)
		return NO_WIDGET;
	sattr.background_pixel = Get_color_pixel(bgcolor);
...
static int Widget_create_slider(int parent_desc, widget_type_t slider_type,
				int x, int y, int width, int height, int border, int viewer_desc)
{
	int widget_desc;
	Window window;
	widget_t *parent_widget;
	...
	XSelectInput(dpy, window,
		     ...
		     OwnerGrabButtonMask);
	widget_desc = Widget_create(slider_type, "slider", window, width, height, sliderw);
	if (widget_desc == NO_WIDGET)
		return NO_WIDGET;
	if (Widget_add_child(parent_desc, widget_desc) == NO_WIDGET) {
		Widget_destroy(widget_desc);
...
static int Widget_viewer_save_callback(int widget_desc, void *data, const char **strptr)
{
...
static int Widget_viewer_close_callback(int widget_desc, void *data, const char **strptr)
{
	UNUSED_PARAM(widget_desc);
	UNUSED_PARAM(strptr);
...
static int Widget_viewer_calculate_text(int viewer_desc)
{
	...
	for (count = i = 0; count < viewerw->num_lines; count++, i++) {
		...
		line[count].txt_width = XTextWidth(viewerw->font, line[count].txt, line[count].len);
...
static void Widget_resize_viewer(XEvent * event, int ind)
{
	...
	const int
	    ...
	    - close_spaced_height, vert_slider_x = width - vert_slider_width, vert_slider_y = 0;
	...
	XMoveWindow(dpy, Widget_pointer(formw->children[1])->window, hori_slider_x, hori_slider_y);
	Widget_resize(formw->children[2], vert_slider_width, vert_slider_height);
	XMoveWindow(dpy, Widget_pointer(formw->children[2])->window, vert_slider_x, vert_slider_y);
	Widget_resize(formw->children[3], save_width, save_height);
	XMoveWindow(dpy, Widget_pointer(formw->children[3])->window, save_x_offset,
		    viewer_height + save_y_offset);
	Widget_resize(formw->children[4], close_width, close_height);
	XMoveWindow(dpy, Widget_pointer(formw->children[4])->window, close_x_offset,
...
int Widget_create_viewer(const char *buf, int len,
			 ...
			 const char *window_name, const char *icon_name, XFontStruct * font)
{
	const int save_width = 2 * 8 + XTextWidth(font, "SAVE", 5),
	    ...
	    hori_slider_y = height - hori_slider_height
	    - close_spaced_height, vert_slider_x = width - vert_slider_width, vert_slider_y = 0;
	int popup_desc, viewer_desc;
	widget_t *popup_widget;
	widget_viewer_t *viewerw;
	Window window;


	popup_desc = Widget_create_popup(width, height, border, window_name, icon_name);
	if (popup_desc == NO_WIDGET
	...
	viewerw->overlay =
	    XCreateSimpleWindow(dpy, popup_widget->window,
				...
				0, Get_color_pixel(borderColor), Get_color_pixel(windowColor));
	...
	viewer_desc =
	    Widget_create(WIDGET_VIEWER, "viewer", window, viewer_width, viewer_height, viewerw);
	...
	viewerw->hori_slider_desc = Widget_create_slider(popup_desc,
							 ...
							 hori_slider_x,
							 hori_slider_y,
							 hori_slider_width, hori_slider_height, 0,
							 viewer_desc);
	if (viewerw->hori_slider_desc == NO_WIDGET) {
		Widget_destroy(popup_desc);
		return NO_WIDGET;
	}
	Widget_window_gravity(Widget_pointer(viewerw->hori_slider_desc)->window, SouthWestGravity);
	viewerw->vert_slider_desc = Widget_create_slider(popup_desc,
							 WIDGET_SLIDER_VERT,
							 vert_slider_x,
							 vert_slider_y,
							 vert_slider_width, vert_slider_height, 0,
							 viewer_desc);
	if (viewerw->vert_slider_desc == NO_WIDGET) {
		Widget_destroy(popup_desc);
		return NO_WIDGET;
	}
	Widget_window_gravity(Widget_pointer(viewerw->vert_slider_desc)->window, NorthEastGravity);
	viewerw->save_button_desc
	    = Widget_create_activate(popup_desc,
				     save_x_offset,
				     viewer_height + save_y_offset,
				     save_width,
				     save_height, 0, "SAVE", Widget_viewer_save_callback,
				     (void *) (long) popup_desc);
	if (viewerw->save_button_desc == NO_WIDGET) {
		Widget_destroy(popup_desc);
		return NO_WIDGET;
	}
	Widget_window_gravity(Widget_pointer(viewerw->save_button_desc)->window, SouthGravity);
	viewerw->close_button_desc
	    = Widget_create_activate(popup_desc,
				     ...
				     close_width,
				     close_height,
				     0, "CLOSE", Widget_viewer_close_callback,
				     (void *) (long) popup_desc);
	if (viewerw->close_button_desc == NO_WIDGET) {
		Widget_destroy(popup_desc);
		return NO_WIDGET;
	}
	Widget_window_gravity(Widget_pointer(viewerw->close_button_desc)->window, SouthGravity);
...
int Widget_update_viewer(int popup_desc, const char *buf, int len)
{
	...
	widget_t *viewer_widget;
	widget_viewer_t *viewer_sub;
	int text_height, first_visible, last_visible, start, end;
