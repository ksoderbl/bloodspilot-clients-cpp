...
/* $Id: welcome.c,v 1.11 2007/09/27 23:31:05 kps Exp $ */
 ...
 * Other prototypes.
 */
static int Welcome_process_one_event(XEvent * event, Connect_param_t * conpar);
...
static int Welcome_create_label(int pos, const char *label_text)
{
	...
	(void) Widget_get_dimensions(subform_widget, &subform_width, &subform_height);
	...
	subform_label_widget =
	    Widget_create_label(subform_widget, label_x, label_y, label_width, label_height, true,
				0, label_text);
		...
		for (i = 0; i < n; i++) {
			...
			button =
			    Widget_create_colored_label(subform_widget, button_x, button_y,
							...
							2, 0, localnet_conpars[i].server_name);
...
static int Internet_server_join_cb(int widget, void *user_data, const char **text)
{
	...
	*conpar = *global_conpar;
	...
	result =
	    Contact_servers(1, &server_addr_ptr, 1, 0, 0, NULL, 0, NULL, NULL, NULL, NULL, conpar);
	if (result) {
		/* structure copy */
		*global_conpar = *conpar;
		joining = true;
...
static int Internet_server_show_cb(int widget, void *user_data, const char **text)
{
	...
	static char *p = NULL;
	char *eqptr = NULL;
	char *playslist = xp_safe_strdup(sip->playlist);
	...
	data_label_width = XTextWidth(buttonFont, longest_text, (int) strlen(longest_text));

	max_label_width = 0;


	for (i = 0; i < NELEM(labels); i++) {
		label_width = XTextWidth(textFont, labels[i].label, (int) strlen(labels[i].label));
	...
	(void) Widget_create_activate(subform_widget,
				      ...
				      label_border, "Join This Server", Internet_server_join_cb,
				      (void *) sip);
	i++;
	...
	(void) Widget_create_activate(subform_widget,
				      ...
				      labels[i].yoff,
				      data_label_width, labels[i].height,
				      label_border, "Back to List", Internet_cb,
				      (void *) global_conpar);
...
static int Welcome_show_server_list(Connect_param_t * conpar)
{
	...
	int ping_offset = server_offset + server_width + server_border_width + space_width;
	...
	w = Widget_create_label(subform_widget,
				player_offset, yoff, player_width, label_height, true, border,
				player_header);
	if (!w)
		return -1;
	...
	w = Widget_create_label(subform_widget,
				queue_offset, yoff, queue_width, label_height, true, border,
				queue_header);
	...
	if (all_offset < subform_width) {
		...
		Widget_create_label(subform_widget,
				    bases_offset, yoff, bases_width, label_height, true, border,
				    bases_header);

		Widget_create_label(subform_widget,
				    team_offset, yoff, team_width, label_height, true, border,
				    team_header);

		Widget_create_label(subform_widget,
				    fps_offset, yoff, fps_width, label_height, true, border,
				    fps_header);

		Widget_create_label(subform_widget,
				    status_offset, yoff, status_width, label_height, true, border,
				    status_header);
	}
	else {
		...
		ping_offset = server_offset + server_width + server_border_width + space_width;
		stat_offset = ping_offset + ping_width + space_width;
	}

	Widget_create_label(subform_widget,
			    version_offset, yoff, version_width, label_height, true, border,
			    version_header);


	Widget_create_label(subform_widget, map_offset, yoff, map_width, label_height, true, border,
			    map_header);

	Widget_create_label(subform_widget,
			    server_offset, yoff,
			    server_width + server_border_width, label_height, true, border,
			    server_header);

	Widget_create_label(subform_widget, ping_offset, yoff, ping_width, label_height, true,
			    border, ping_header);

	Widget_create_label(subform_widget, stat_offset, yoff, stat_width, label_height, true,
			    border, stat_header);
	...
	for (; server_it != List_end(server_list); LI_FORWARD(server_it)) {
		...
		Widget_create_label(subform_widget,
				    player_offset, yoff,
				    player_width, label_height, true, border,
				    sip->users ? sip->users_str : "");
		Widget_create_label(subform_widget, queue_offset, yoff, queue_width, label_height,
				    true, border, sip->queue ? sip->queue_str : "");
		if (all_offset < subform_width) {

			Widget_create_label(subform_widget,
					    bases_offset, yoff,
					    bases_width, label_height, true, border,
					    sip->bases_str);
			Widget_create_label(subform_widget, team_offset, yoff, team_width,
					    label_height, true, border,
					    (sip->teambases > 0) ? sip->teambases_str : "");
			Widget_create_label(subform_widget, fps_offset, yoff, fps_width,
					    label_height, true, border, sip->fps_str);
			...
			Widget_create_label(subform_widget,
					    ...
					    border, strcmp(sip->status, "ok") ? sip->status : "");
		...
		Widget_create_label(subform_widget,
				    version_offset, yoff, version_width, label_height, true, border,
				    sip->version);
		Widget_create_label(subform_widget, map_offset, yoff, map_width, label_height, true,
				    border, sip->mapname);
		...
		Widget_create_label(subform_widget,
				    ping_offset, yoff,
				    ping_width, label_height, true, border,
		...
		Widget_create_activate(subform_widget,
				       ...
				       border ? border : 1, stat_header, Internet_server_show_cb,
				       (void *) sip);
	}
	...
	if (!ping_servers_widget)
		ping_servers_widget =
		    Widget_create_activate(form_widget,
					   ...
					   first_border, ping_text, Internet_ping_cb,
					   (void *) conpar);
...
/* XXX TODO add help page . */
	...
	form_widget =
	    Widget_create_form(0, topWindow, form_x, form_y, form_width, form_height, form_border);
	...
	for (i = 0; i < NELEM(my_buttons); i++) {
		text_width =
		    XTextWidth(buttonFont, my_buttons[i].text, (int) strlen(my_buttons[i].text));
		if (text_width > max_width)
			max_width = text_width;
	}
	max_width += 20;
...
static int Welcome_input_loop(Connect_param_t * conpar)
{
	...
	/* Start main loop */
	Welcome_create_label(1, "Welcome to BloodsPilot!");
