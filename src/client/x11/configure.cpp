...
/* $Id: configure.c,v 1.22 2007/09/27 23:31:05 kps Exp $ */
...
static int num_key_options = 0;
static int max_key_options = 0;
static int *key_option_indices = NULL;

static bool config_created = false;
/*bool			config_mapped = false;*/
static int config_page,
	...
	config_bool_width,
	config_bool_height,
	config_int_width, config_double_width, config_key_width, config_key_height, config_arrow_width,
	config_arrow_height;
...
static int Nelem_config_creator(void)
{
	...
	if (config_what == CONFIG_KEYS)
		return num_key_options + 1;
...
static xp_option_t *Config_creator_option(int i)
{
	...
	if (config_what == CONFIG_KEYS && i >= 0 && i < num_key_options)
		ind = key_option_indices[i];
...
static int Update_color_index_option(int widget_desc, void *data, int *val)
{
	xp_option_t *opt = (xp_option_t *)data;

	UNUSED_PARAM(widget_desc);
	Set_color_index_option(opt, *val, xp_option_origin_config);
...
static xp_option_t *key_config_option = NULL;

/*
 * Function called when we are in keyConfigMode and
 * user has pressed a key. Argument 'ksname' is the
 * name of the key the user pressed.
 */
void Config_key_event(char *ksname)
{
	char value[256]; /* enough chars? */
	xp_option_t *opt = key_config_option;

	/*warn("Config_key_event: key is %s", ksname); */

	if (!strcasecmp(Option_value_to_string(opt), "none") || !strcasecmp(Option_value_to_string(opt), ""))
	{
		/*
		 * No key defined for function, set key to ksname.
		 * NOTE: this can also be BackSpace, see below.
		 */
		Set_key_option(opt, ksname, xp_option_origin_config);
	}
	else if (!strcasecmp("backspace", ksname))
	{
		/*
		 * BackSpace can be used to remove the current setting.
		 */
		Set_key_option(opt, "None", xp_option_origin_config);
	}
	else
	{
		/*
		 * Append the new key to the existing list of keys.
		 */
		snprintf(value, sizeof(value), "%s %s", Option_value_to_string(opt), ksname);
		Set_key_option(opt, value, xp_option_origin_config);
	}
...
/*
 * This callback is called by widget code when the user wants
 * to configure a certain key option using the GUI interface.
 */
static int Update_key_option(int widget_desc, void *data)
{
	xp_option_t *opt = (xp_option_t *)data;

	UNUSED_PARAM(widget_desc);
	keyConfigMode = true;
	key_config_option = opt;
...
{
	int i, num, height = 0, offset, width, widget_desc;
	bool full;
	...
	config_key_width = XTextWidth(buttonFont, "iiiiiiiiiiiiiiiiiii", 20) + 2 * config_button_space;
	config_key_height = config_button_height;
	...
	for (i = 0; i < Nelem_config_creator(); i++)
	{
		...
		if (full)
		{
			...
			config_widget_desc[num] = Widget_create_form(NO_WIDGET, topWindow,
														 config_x, config_y, config_width, config_height,
														 0);
							  ...
							  void *data)
{
	...
	Widget_create_label(widget_desc, config_space, *height + (config_entry_height - config_text_height) / 2,
						label_width, config_text_height, true, 0, str);
	if (config_space + label_width > offset)
...
static int Config_create_color(int widget_desc, int *height, int color,
							   const char *str, int *val, int (*callback)(int, void *, int *),
							   void *data)
{
	...
	colw = Widget_create_color(widget_desc, color, offset, *height + (config_entry_height - config_text_height) / 2,
							   ...
							   0, val, NO_COLOR, MAX_DEFAULT_COLORS - 1, callback, data);
	offset += config_int_width;
	Widget_create_arrow_left(widget_desc, offset, *height + (config_entry_height - config_arrow_height) / 2,
							 config_arrow_width, config_arrow_height, 0, colw);
	offset += config_arrow_width;
	Widget_create_arrow_right(widget_desc, offset, *height + (config_entry_height - config_arrow_height) / 2,
							  config_arrow_width, config_arrow_height, 0, colw);
	*height += config_entry_height + config_space;
...
static int Config_create_double(int widget_desc, int *height,
								...
								double min, double max, int (*callback)(int, void *, double *),
								void *data)
{
	int offset, label_width, doublew;
	...
	doublew = Widget_create_double(widget_desc, offset, *height + (config_entry_height - config_text_height) / 2,
								   config_double_width, config_text_height, 0, val, min, max,
								   callback, data);
	offset += config_double_width;
	Widget_create_arrow_left(widget_desc, offset, *height + (config_entry_height - config_arrow_height) / 2,
							 config_arrow_width, config_arrow_height, 0, doublew);
	offset += config_arrow_width;
	Widget_create_arrow_right(widget_desc, offset, *height + (config_entry_height - config_arrow_height) / 2,
							  config_arrow_width, config_arrow_height, 0, doublew);
	*height += config_entry_height + config_space;
...
static int Config_create_key(int widget_desc, int *height,
							 const char *str, const char *val, int (*callback)(int, void *),
							 void *data)
{
	int offset, label_width, keyw;

	if (*height + 2 * config_entry_height + 2 * config_space >= config_height)
		return 0;
	label_width = XTextWidth(textFont, str, (int)strlen(str)) + 2 * config_text_space;
	offset = config_width - (config_space + config_key_width);
	if (config_space + label_width > offset)
	{
		if (*height + 3 * config_entry_height + 2 * config_space >= config_height)
			return 0;
	...
	if (config_space + label_width > offset)
		*height += config_entry_height;
	keyw = Widget_create_key(widget_desc,
							 offset, *height + (config_entry_height - config_key_height) / 2,
							 config_key_width, config_key_height, 0, val, callback, data);
	*height += config_entry_height + config_space;
...
static int Config_create_save(int widget_desc, int *height)
{
	...
	int space, button_desc, width = 2 * config_button_space + XTextWidth(buttonFont, save_str, (int)strlen(save_str));
...
static int Config_creator(xp_option_t *opt, int widget_desc, int *height)
{
	...
	case xp_bool_option:
		return Config_create_bool(widget_desc, height,
								  Option_get_name(opt), *opt->bool_ptr, Update_bool_option,
								  opt);
	case xp_int_option:
		return Config_create_int(widget_desc, height,
								 Option_get_name(opt), opt->int_ptr,
								 opt->int_minval, opt->int_maxval, Update_int_option, opt);

	case xp_color_index_option:
		return Config_create_color(widget_desc, height, *opt->color_index_ptr,
								   Option_get_name(opt), opt->color_index_ptr,
								   Update_color_index_option, opt);

	case xp_double_option:
		return Config_create_double(widget_desc, height,
									Option_get_name(opt), opt->dbl_ptr,
									opt->dbl_minval, opt->dbl_maxval, Update_double_option,
									opt);

	case xp_key_option:
		return Config_create_key(widget_desc, height, Option_get_name(opt) + 3, /* skip "key" prefix */
								 Option_value_to_string(opt), Update_key_option, opt);
	default:
		return 0;
	}
...
void Config_init(void)
{
	...
	for (i = 0; i < num_options; i++)
	{
		...
		if (Option_get_type(opt) == xp_color_index_option)
		{
			STORE(int, color_option_indices, num_color_options, max_color_options, i);
		}
		else if (Option_get_type(opt) == xp_key_option)
		{
			STORE(int, key_option_indices, num_key_options, max_key_options, i);
		}
		else if (Option_get_flags(opt) & XP_OPTFLAG_CONFIG_DEFAULT)
		{
			STORE(int, default_option_indices, num_default_options, max_default_options,
				  i);
		}
	}

	max_ids = MAX(num_color_options, num_default_options);
	max_ids = MAX(max_ids, num_key_options);
