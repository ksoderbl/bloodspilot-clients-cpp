...
/* $Id: guimap.c,v 1.48 2007/11/30 17:28:06 kps Exp $ */
...
int fuelColor;							/* fuel drawing */
static int decorColor;					/* Color index for decoration drawing */
static int baseNameColor;				/* Color index for base name drawing */
static int backgroundPointColor = BLUE; /* background point drawing */
...
void Gui_paint_walls(int x, int y, int type, int xi, int yi)
{
	...
	{
		if (type & BLUE_LEFT)
			Segment_add(wallColor, X(x), Y(y), X(x), Y(y + BLOCK_SZ));
		if (type & BLUE_DOWN)
			Segment_add(wallColor, X(x), Y(y), X(x + BLOCK_SZ), Y(y));
		if (type & BLUE_RIGHT)
			Segment_add(wallColor, X(x + BLOCK_SZ), Y(y), X(x + BLOCK_SZ),
						Y(y + BLOCK_SZ));
		if (type & BLUE_UP)
			Segment_add(wallColor, X(x), Y(y + BLOCK_SZ), X(x + BLOCK_SZ),
						Y(y + BLOCK_SZ));
		if ((type & BLUE_FUEL) == BLUE_FUEL)
			;
		else if (type & BLUE_OPEN)
			Segment_add(wallColor, X(x), Y(y), X(x + BLOCK_SZ), Y(y + BLOCK_SZ));
		else if (type & BLUE_CLOSED)
			Segment_add(wallColor, X(x), Y(y + BLOCK_SZ), X(x + BLOCK_SZ), Y(y));
	}
	else
	{
...
#define FUEL_BORDER 2
		...
		if (fuelColor != BLACK)
			Set_fg_color_xor(BLACK, fuelColor);
		else
			Set_fg_color(RED);
...
#define BITMAP_FUEL_BORDER 3
		...
		image = ABS((twelveHz + x + x * y) % (fuel_images * 2));
		...
		if (bit != NULL)
		{
			...
			Bitmap_paint_area(drawPixmap, bit,
							  ...
							  SCALEY(y + size + BITMAP_FUEL_BORDER), &area);
		}
	}
}
...
{
	int color = baseNameColor;
	const int BORDER = 4; /* in pixels */
	int size = 0, size2 = 0;
	other_t *other = NULL;
	int other_name_width = 0;
	char s[3];
	char info[6];
	...
	if (!texturedObjects)
	{
		...
		{
		case SETUP_BASE_UP:
			Segment_add(WHITE, X(x), Y(y - 1), X(x + BLOCK_SZ), Y(y - 1));
			break;
		case SETUP_BASE_DOWN:
			Segment_add(WHITE, X(x), Y(y + BLOCK_SZ + 1), X(x + BLOCK_SZ),
						Y(y + BLOCK_SZ + 1));
			break;
		case SETUP_BASE_LEFT:
			Segment_add(WHITE, X(x + BLOCK_SZ + 1), Y(y + BLOCK_SZ),
						X(x + BLOCK_SZ + 1), Y(y));
			break;
		case SETUP_BASE_RIGHT:
			Segment_add(WHITE, X(x - 1), Y(y + BLOCK_SZ), X(x - 1), Y(y));
			break;
		default:
			warn("Bad base dir.");
			return;
	...
	if (baseNameColor == NO_COLOR)
		return;

	base = Homebase_by_id(id);
	other = Other_by_id(id);

	if (other != NULL && other->mychar != 'P')
	{
		color = Player_state_color(other);
		if (color == BLACK || color == WHITE)
			color = baseNameColor;
		if (color == NO_COLOR)
			color = WHITE; /* don't accept NO_COLOR for bases for now */
		other_name_width = Other_name_width(other, maxCharsInNames);
	}
	else if (other != NULL && other->mychar == 'P')
		/* HACK - this will cause the base to be painted as if it is
		 * nobody's base. It doesn't interest us which pauser is at
		 * which base.
		 */
		other = NULL;

	/*
	 * kps - bases should be painted in life colors I suppose.
	 */
	...
	if (base != NULL)
	{
		 ...
		 */
		if (twelveHz < base->appeartime)
			do_basewarning = true;

		if (do_basewarning)
		{
			/* We assume the ship will appear after 3 seconds. */
			int count = 10 * (base->appeartime - twelveHz);

			LIMIT(count, 0, 360);
			/* red box and/or ship basewarning */
			if (count > 0)
			{
				Gui_paint_appearing(x + BLOCK_SZ / 2,
									y + BLOCK_SZ / 2,
									id, count);
			}
		}
	...
	case SETUP_BASE_LEFT:
		...
		break;
	case SETUP_BASE_RIGHT:
		x -= BORDER + other_name_width + size;
		y += -WINSCALE(BLOCK_SZ / 2) + gameFont->ascent / 2;
		break;
	default:
	...
	if (other)
	{
		...
		size2 = ColorTextWidth(gameFont, info, (int)strlen(info));
	}
...
void Gui_paint_decor(int x, int y, int xi, int yi, int type, bool last, bool more_y)
{
	...
	int mask;
	int fill_top_left = -1, fill_top_right = -1, fill_bottom_left = -1, fill_bottom_right = -1;
	static uint8_t decor[256];
	static int decorReady = 0;

	if (decorColor == NO_COLOR)
		return;

	Set_fg_color(decorColor);
	...
	if (!(decorDrawMode == DRAW_FILLED || decorDrawMode == DRAW_TEXTURED))
	{
		if (mask & DECOR_LEFT)
		{
			if ((xi == 0)
					? (!BIT(Setup->mode, WRAP_PLAY) || !(decor[Setup->map_data[(Setup->x - 1) * Setup->y + yi]] & DECOR_RIGHT))
					: !(decor[Setup->map_data[(xi - 1) * Setup->y + yi]] & DECOR_RIGHT))
			{
				Segment_add(decorColor, X(x), Y(y), X(x), Y(y + BLOCK_SZ));
			}
		}
		if (mask & DECOR_DOWN)
		{
			if ((yi == 0)
					? (!BIT(Setup->mode, WRAP_PLAY) || !(decor[Setup->map_data[xi * Setup->y + Setup->y - 1]] & DECOR_UP))
					: !(decor[Setup->map_data[xi * Setup->y + (yi - 1)]] & DECOR_UP))
			{
				Segment_add(decorColor, X(x), Y(y), X(x + BLOCK_SZ), Y(y));
			}
		}
		if (mask & DECOR_RIGHT)
		{
			if (decorDrawMode != DRAW_OUTLINE || ((xi == Setup->x - 1)
													  ? (!BIT(Setup->mode, WRAP_PLAY) || !(decor[Setup->map_data[yi]] & DECOR_LEFT))
													  : !(decor[Setup->map_data[(xi + 1) * Setup->y + yi]] & DECOR_LEFT)))
			{
				Segment_add(decorColor, X(x + BLOCK_SZ), Y(y), X(x + BLOCK_SZ),
							Y(y + BLOCK_SZ));
			}
		}
		if (mask & DECOR_UP)
		{
			if (decorDrawMode != DRAW_OUTLINE || ((yi == Setup->y - 1)
													  ? (!BIT(Setup->mode, WRAP_PLAY) || !(decor[Setup->map_data[xi * Setup->y]] & DECOR_DOWN))
													  : !(decor[Setup->map_data[xi * Setup->y + (yi + 1)]] & DECOR_DOWN)))
			{
				Segment_add(decorColor, X(x), Y(y + BLOCK_SZ), X(x + BLOCK_SZ),
							Y(y + BLOCK_SZ));
			}
		}
		if (mask & DECOR_OPEN)
		{
			Segment_add(decorColor, X(x), Y(y), X(x + BLOCK_SZ), Y(y + BLOCK_SZ));
		}
		else if (mask & DECOR_CLOSED)
		{
			Segment_add(decorColor, X(x), Y(y + BLOCK_SZ), X(x + BLOCK_SZ), Y(y));
		}
	}
	else
	{
		...
		if (fill_top_right != -1)
		{
			...
			points[4] = points[0];
			rd.fillPolygon(dpy, drawPixmap, gameGC, points, 5, Convex, CoordModeOrigin);
			fill_top_left = fill_top_right = fill_bottom_left = fill_bottom_right = -1;
		}
	}
}
...
{
	Segment_add(wallColor, X(x), Y(y), X(xi), Y(yi));
}

static void Gui_paint_rectangle(int x, int y, int xi, int yi, int color)
{
	Segment_add(color, X(x), Y(y), X(x), Y(yi));

	Segment_add(color, X(xi), Y(y), X(xi), Y(yi));

	Segment_add(color, X(x), Y(y), X(xi), Y(y));

	Segment_add(color, X(x), Y(yi), X(xi), Y(yi));
}
...
{
	Arc_add(RED, X(x + 5), Y(y + BLOCK_SZ - 5), BLOCK_SZ - 10, BLOCK_SZ - 10, 64 * 150,
			64 * 300);
	Segment_add(RED, X(x + BLOCK_SZ / 2), Y(y + BLOCK_SZ - 5), X(x + BLOCK_SZ / 2 + 4),
				Y(y + BLOCK_SZ - 1));
	Segment_add(RED, X(x + BLOCK_SZ / 2), Y(y + BLOCK_SZ - 5), X(x + BLOCK_SZ / 2 + 4),
				Y(y + BLOCK_SZ - 9));
}
...
{
	Arc_add(RED, X(x + 5), Y(y + BLOCK_SZ - 5), BLOCK_SZ - 10, BLOCK_SZ - 10, 64 * 90,
			64 * 300);
	Segment_add(RED, X(x + BLOCK_SZ / 2), Y(y + BLOCK_SZ - 5), X(x + BLOCK_SZ / 2 - 4),
				Y(y + BLOCK_SZ - 1));
	Segment_add(RED, X(x + BLOCK_SZ / 2), Y(y + BLOCK_SZ - 5), X(x + BLOCK_SZ / 2 - 4),
...
void Gui_paint_setup_pos_grav(int x, int y)
{
	...
	if (!texturedObjects)
	{
		...
		Arc_add(RED, X(x + 1), Y(y + BLOCK_SZ - 1), INSIDE_BL, INSIDE_BL, 0, 64 * 360);
		Segment_add(RED, X(x + BLOCK_SZ / 2), Y(y + 5), X(x + BLOCK_SZ / 2),
					Y(y + BLOCK_SZ - 5));
		Segment_add(RED, X(x + 5), Y(y + BLOCK_SZ / 2), X(x + BLOCK_SZ - 5),
					Y(y + BLOCK_SZ / 2));
	}
	else
...
{
	Segment_add(RED, X(x + BLOCK_SZ / 2), Y(y), X(x + BLOCK_SZ / 2), Y(y + BLOCK_SZ));
	Segment_add(RED, X(x + BLOCK_SZ / 2), Y(y + BLOCK_SZ), X(x + BLOCK_SZ / 2 - 10),
				Y(y + BLOCK_SZ - 10));
	Segment_add(RED, X(x + BLOCK_SZ / 2), Y(y + BLOCK_SZ), X(x + BLOCK_SZ / 2 + 10),
				Y(y + BLOCK_SZ - 10));
}
...
{
	Segment_add(RED, X(x + BLOCK_SZ / 2), Y(y), X(x + BLOCK_SZ / 2), Y(y + BLOCK_SZ));
	Segment_add(RED, X(x + BLOCK_SZ / 2), Y(y), X(x + BLOCK_SZ / 2 - 10), Y(y + 10));
	Segment_add(RED, X(x + BLOCK_SZ / 2), Y(y), X(x + BLOCK_SZ / 2 + 10), Y(y + 10));
}

void Gui_paint_setup_right_grav(int x, int y)
{
	Segment_add(RED, X(x), Y(y + BLOCK_SZ / 2), X(x + BLOCK_SZ), Y(y + BLOCK_SZ / 2));
	Segment_add(RED, X(x + BLOCK_SZ), Y(y + BLOCK_SZ / 2), X(x + BLOCK_SZ - 10),
				Y(y + BLOCK_SZ / 2 + 10));
	Segment_add(RED, X(x + BLOCK_SZ), Y(y + BLOCK_SZ / 2), X(x + BLOCK_SZ - 10),
				Y(y + BLOCK_SZ / 2 - 10));
}
...
{
	Segment_add(RED, X(x), Y(y + BLOCK_SZ / 2), X(x + BLOCK_SZ), Y(y + BLOCK_SZ / 2));
	Segment_add(RED, X(x), Y(y + BLOCK_SZ / 2), X(x + 10), Y(y + BLOCK_SZ / 2 + 10));
	Segment_add(RED, X(x), Y(y + BLOCK_SZ / 2), X(x + 10), Y(y + BLOCK_SZ / 2 - 10));
}

void Gui_paint_setup_worm(int x, int y)
{
	int wormDrawCount = twelveHz & 7;
	if (!texturedObjects)
	{
		static const int INSIDE_BL = BLOCK_SZ - 2;
...
#define _O wormOffset[wormDrawCount]
		...
		Set_fg_color(RED);
		Arc_add(RED, X(x) + 0, Y(y + BLOCK_SZ) + 0, INSIDE_BL - 0, INSIDE_BL - 0, 0,
				64 * 360);

		Arc_add(RED, X(x) + _O[0], Y(y + BLOCK_SZ) + _O[1], INSIDE_BL - _O[2],
				INSIDE_BL - _O[2], 0, 64 * 360);
...
void Gui_paint_setup_item_concentrator(int x, int y)
{
	...
	if (!texturedObjects)
	{
		...
		for (i = 0; i < NELEM(tris); i++)
		{
			...
			pts[1].x =
				WINSCALE(cx + (int)(tris[i].radius * tcos(MOD2(tdir + RES / 3, RES))));
			pts[1].y =
				WINSCALE(cy + (int)(tris[i].radius * tsin(MOD2(tdir + RES / 3, RES))));
			pts[2].x =
				WINSCALE(cx +
						 (int)(tris[i].radius * tcos(MOD2(tdir + 2 * RES / 3, RES))));
			pts[2].y =
				WINSCALE(cy +
						 (int)(tris[i].radius * tsin(MOD2(tdir + 2 * RES / 3, RES))));
			...
			pts[3] = pts[0];
			rd.drawLines(dpy, drawPixmap, gameGC, pts, NELEM(pts), CoordModeOrigin);
		}
	}
	else
		Bitmap_paint(drawPixmap, BM_CONCENTRATOR, WINSCALE(X(x)),
					 WINSCALE(Y(y + BLOCK_SZ)), (twelveHz + (x + x * y)) % 32);
}
...
void Gui_paint_setup_asteroid_concentrator(int x, int y)
{
	...
	if (!texturedObjects)
	{
		...
		for (i = 0; i < NELEM(sqrs); i++)
		{
			...
			pts[1].x =
				WINSCALE(cx + (int)(sqrs[i].size * tcos(MOD2(tdir + RES / 4, RES))));
			pts[1].y =
				WINSCALE(cy + (int)(sqrs[i].size * tsin(MOD2(tdir + RES / 4, RES))));
			pts[2].x =
				WINSCALE(cx +
						 (int)(sqrs[i].size * tcos(MOD2(tdir + 2 * RES / 4, RES))));
			pts[2].y =
				WINSCALE(cy +
						 (int)(sqrs[i].size * tsin(MOD2(tdir + 2 * RES / 4, RES))));
			pts[3].x =
				WINSCALE(cx +
						 (int)(sqrs[i].size * tcos(MOD2(tdir + 3 * RES / 4, RES))));
			pts[3].y =
				WINSCALE(cy +
						 (int)(sqrs[i].size * tsin(MOD2(tdir + 3 * RES / 4, RES))));
			/* Trace("DC: %d cx=%d/%d %d/%d %d/%d %d/%d %d/%d\n",
			   i, cx, cy, pts[0].x, pts[0].y,
			   pts[1].x, pts[1].y, pts[2].x, pts[2].y,
			   pts[3].x, pts[3].y); */
...
void Gui_paint_decor_dot(int x, int y, int size)
{
	...
	Rectangle_add(backgroundPointColor,
				  ...
				  Y(y + BLOCK_SZ / 2) - (backgroundPointSize >> 1), size, size);
}
...
void Gui_paint_setup_target(int x, int y, int team, int damage, bool own)
{
	...
	rd.drawRectangle(dpy, drawPixmap, gameGC,
					 ...
					 WINSCALE(Y(y + 3 * BLOCK_SZ / 4)), UWINSCALE(BLOCK_SZ / 2),
	...
	if (BIT(Setup->mode, TEAM_PLAY))
	{
		s[0] = '0' + team;
		s[1] = '\0';
		size = ColorTextWidth(gameFont, s, 1);
		rd.drawString(dpy, drawPixmap, gameGC,
					  WINSCALE(X(x + BLOCK_SZ / 2)) - size / 2,
					  WINSCALE(Y(y + BLOCK_SZ / 2)) + gameFont->ascent / 2, s, 1);
	}
	...
	if (damage != TARGET_DAMAGE)
	{
		...
		Segment_add(RED, X(a1), Y(a2), X(b1), Y(b2));

		Segment_add(RED, X(a1), Y(b2), X(b1), Y(a2));
	}
}
...
void Gui_paint_setup_treasure(int x, int y, int team, bool own)
{
	...
	if (!texturedObjects)
	{
		...
		Set_fg_color(color);
		Segment_add(color, X(x), Y(y), X(x), Y(y + BLOCK_SZ / 2));
		Segment_add(color, X(x + BLOCK_SZ), Y(y), X(x + BLOCK_SZ), Y(y + BLOCK_SZ / 2));
		Segment_add(color, X(x), Y(y), X(x + BLOCK_SZ), Y(y));
		Arc_add(color, X(x), Y(y + BLOCK_SZ), BLOCK_SZ, BLOCK_SZ, 0, 64 * 180);
		if (BIT(Setup->mode, TEAM_PLAY))
		{
			s[1] = '\0';
			s[0] = '0' + team;
	...
	else
	{
		...
		Bitmap_paint(drawPixmap, type, WINSCALE(X(x)), WINSCALE(Y(y + BLOCK_SZ)), 0);
...
void Gui_paint_polygon(int i, int xoff, int yoff)
{
	...
	textured = (wallDrawMode == DRAW_TEXTURED) && fullColor;
	filled = (wallDrawMode == DRAW_TEXTURED || wallDrawMode == DRAW_FILLED);
	...
	did_fill = 0;
	if ((filled || textured) && (BIT(style.flags, STYLE_TEXTURED | STYLE_FILLED)))
	{
		if (textured && BIT(style.flags, STYLE_TEXTURED))
		{
			 ...
			 */
			XSetTSOrigin(dpy, gameGC, WINSCALE(polygon.bounds.x + xoff * Setup->width) - ship.x,
		...
		notexture:
			...
			if (fullColor)
				Fullcolor_set_fg(style.pixel);
			else
				Set_fg_color(wallColor);
		}
		did_fill = 1;
		rd.fillPolygon(dpy, drawPixmap, gameGC, points, polygon.num_points, Nonconvex,
					   CoordModeOrigin);
	}
	XSetFillStyle(dpy, gameGC, FillSolid);
	...
	{ /* No special edges */
		...
		{ /* did_fill to avoid invisibility */
			...
			if (fullColor)
				Fullcolor_set_fg(edge_styles[sindex].pixel);
			else
				Set_fg_color(wallColor);

			rd.drawLines(dpy, drawPixmap, gameGC, points, polygon.num_points + 1,
						 CoordModeOrigin);
		}
	}
	else
	{
		...
		for (j = 0; j < polygon.num_points;)
		{
			...
			if (sindex != 0 && (edge_styles[sindex].width != -1 || !did_fill))
			{
				...
				rd.drawLines(dpy, drawPixmap, gameGC, points + begin, j + 1 - begin,
							 CoordModeOrigin);
			}
		}
	}
