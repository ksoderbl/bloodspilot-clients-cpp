...
/* $Id: xdefault.c,v 1.12 2007/10/02 22:32:33 kps Exp $ */
 ...
 */
#if 1
#define GAME_FONT	"fixed"
/*"-*-fixed-bold-*-*--13-*-*-*-c-*-iso8859-1"*/
#define MESSAGE_FONT	"fixed"
/*"-*-fixed-bold-*-*--13-*-*-*-c-*-iso8859-1"*/
#define SCORE_LIST_FONT	"fixed"
/*"-*-fixed-bold-*-*--13-*-*-*-c-*-iso8859-1"*/
...
static bool Set_mouseAccelNum(xp_option_t * opt, int value)
{
	...
	if (value < 0) {
		...
		if (dpy && mouseAccelInClient)
			XChangePointerControl(dpy, True, True, value, new_acc_denom, new_threshold);
		return true;
	}
...
static bool Set_mouseAccelDenom(xp_option_t * opt, int value)
{
	...
	if (value < 0) {
		...
		new_acc_denom = value;
		if (dpy && mouseAccelInClient)
			XChangePointerControl(dpy, True, True, new_acc_num, value, new_threshold);
		return true;
	}
}
...
static bool Set_mouseAccelThresh(xp_option_t * opt, int value)
{
	...
	if (value < 0) {
		...
		new_threshold = value;
		if (dpy && mouseAccelInClient)
			XChangePointerControl(dpy, True, True, new_acc_num, new_acc_denom, value);
		return true;
	}
}
...
xp_option_t xdefault_options[] = {
	...
	XP_BOOL_OPTION("texturedObjects",
		       false,
		       &texturedObjects,
		       Set_texturedObjects,
		       XP_OPTFLAG_CONFIG_DEFAULT,
	...
	XP_BOOL_OPTION("ignoreWindowManager",
		       false,
		       &ignoreWindowManager,
		       NULL,
		       XP_OPTFLAG_DEFAULT,
	...
	XP_STRING_OPTION("scoreListFont",
			 SCORE_LIST_FONT,
			 scoreListFontName,
	...
	XP_BOOL_OPTION("mouseAccelInClient",
		       true,
		       &mouseAccelInClient,
		       NULL,
		       XP_OPTFLAG_CONFIG_DEFAULT,
	...
	XP_INT_OPTION("mouseAccelThresh",
		      0,
		      0,
		      10,
		      &new_threshold,
	...
	/* X debug stuff */
#ifdef DEVELOPMENT
	XP_NOARG_OPTION("testxsync",
			&testxsync,
			XP_OPTFLAG_NEVER_SAVE,
			"Test XSynchronize() ?\n"),

	XP_NOARG_OPTION("testxdebug",
			&testxdebug,
			XP_OPTFLAG_NEVER_SAVE,
			"Test X_error_handler() ?\n"),

	XP_NOARG_OPTION("testxafter",
			&testxafter,
			XP_OPTFLAG_NEVER_SAVE,
			"Test XAfterFunction ?\n"),

	XP_NOARG_OPTION("testxcolors",
			&testxcolors,
			XP_OPTFLAG_NEVER_SAVE,
			"Do Colors_debug() ?\n"),
#endif
