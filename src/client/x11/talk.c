...
static void Talk_create_window(void)
{
	...
	talkWindow
	    = XCreateSimpleWindow(dpy, drawWindow,
				  ...
				  TALK_OUTSIDE_BORDER,
				  (teamTalk?
				   Get_color_pixel(BLUE)
				   : Get_color_pixel(WHITE)),
...
{
	if (talking == false || visible == talk_cursor.visible)
		return;

	if (visible == false) {
		Set_gc_fg_color(dpy, talkGC, BLACK);
		XDrawString(dpy, talkWindow, talkGC,
			    talk_cursor.offset + TALK_INSIDE_BORDER,
			    talkFont->ascent + TALK_INSIDE_BORDER, "_", 1);
		Set_gc_fg_color(dpy, talkGC, WHITE);
		if (talk_cursor.point < strlen(talk_str)) {
			...
			if (selection.talk.state == SEL_EMPHASIZED
				...
				Set_gc_fg_color(dpy, talkGC, DRAW_EMPHASIZED);
			XDrawString(dpy, talkWindow, talkGC,
				    talk_cursor.offset + TALK_INSIDE_BORDER,
		...
		/* visible */
		talk_cursor.offset = XTextWidth(talkFont, talk_str, (int) talk_cursor.point);
		/*
...
/*
 * This function is called, eventually, when a player has pressed
 * the talk button.
 */
void Talk_map_window(bool map)
{
	...
	if (map) {
		...
		if (talk_created == false) {
			...
			/* Destroy it so we can recreate it. */
			XUnmapWindow(dpy, talkWindow);
			XDestroyWindow(dpy, talkWindow);
			Talk_create_window();
		}
...
static void Talk_refresh(void)
{
	...
	XDrawString(dpy, talkWindow, talkGC, XTextWidth(talkFont, talk_str, (int) selection.talk.x1)
		    + TALK_INSIDE_BORDER,
		    talkFont->ascent + TALK_INSIDE_BORDER,
		    &talk_str[selection.talk.x1], (int) (selection.talk.x2 - selection.talk.x1));
	Set_gc_fg_color(dpy, talkGC, WHITE);
}
...
static char *Get_msg_from_history(int *pos, char *message, keys_t direction)
{
	...
	if (direction != KEY_TALK_CURSOR_UP && direction != KEY_TALK_CURSOR_DOWN
	    && direction != KEY_DUMMY) {
		return NULL;
	}
...
static void Talk_delete_emphasized_text(void)
{
	...
	if (newlen < oldlen) {
		...
		XDrawString(dpy, talkWindow, talkGC,
			    ...
			    talkFont->ascent + TALK_INSIDE_BORDER,
			    &talk_str[talk_cursor.point], (int) (oldlen - talk_cursor.point));
		Set_gc_fg_color(dpy, talkGC, WHITE);
	}
	if (talk_cursor.point < newlen)
		XDrawString(dpy, talkWindow, talkGC,
			    width + TALK_INSIDE_BORDER,
			    talkFont->ascent + TALK_INSIDE_BORDER,
			    &new_str[talk_cursor.point], (int) (newlen - talk_cursor.point));
	Talk_cursor(true);
	...
	case KeyPress:
		...
		if (XLookupString(&event->xkey, &ch, 1, &keysym, &compose)
			...
			for (key = Lookup_key(event, keysym, true);
			     key != KEY_DUMMY; key = Lookup_key(event, keysym, false)) {
				switch (key) {
		...
		case '\033':
			...
			/*
			 * Clear talk window in case we didn't send the message, 
			 */
			talk_str[0] = '\0';
			talk_cursor.point = 0;
		...
		case CTRL('K'):
			...
			if (newlen < oldlen) {
				...
				assert(oldlen >= talk_cursor.point);
				XDrawString(dpy, talkWindow, talkGC,
					    XTextWidth(talkFont, talk_str, (int) talk_cursor.point)
					    + TALK_INSIDE_BORDER,
					    talkFont->ascent + TALK_INSIDE_BORDER,
					    &talk_str[talk_cursor.point],
		...
		default:
			...
			strlcpy(&new_str[talk_cursor.point + 1],
				&talk_str[talk_cursor.point], MAX_CHARS - (talk_cursor.point + 1));
...
int Talk_paste(char *data, size_t data_len, bool overwrite)
{
	...
	if (overwrite) {
		...
		strlcpy(&tmp_str[talk_cursor.point + accept_len], &talk_backup[talk_cursor.point],
	...
	if (overwrite) {
		...
		Set_gc_fg_color(dpy, talkGC, WHITE);
		XDrawString(dpy, talkWindow, talkGC,
			    TALK_INSIDE_BORDER, talkFont->ascent + TALK_INSIDE_BORDER, tmp_str,
			    (int) accept_len);
	}
	else {
		if (selection.talk.state == SEL_EMPHASIZED) {
		...
		if (talk_cursor.point < str_len) {
			...
			Set_gc_fg_color(dpy, talkGC, BLACK);
			XDrawString(dpy, talkWindow, talkGC,
				    XTextWidth(talkFont, talk_backup, (int) talk_cursor.point)
				    + TALK_INSIDE_BORDER, talkFont->ascent + TALK_INSIDE_BORDER,
		...
		XDrawString(dpy, talkWindow, talkGC,
			    XTextWidth(talkFont, talk_backup, (int) talk_cursor.point)
			    + TALK_INSIDE_BORDER, talkFont->ascent + TALK_INSIDE_BORDER,
			    &tmp_str[talk_cursor.point], (int) (new_len - talk_cursor.point));
	}
...
{
	if (talk_created)
		XMoveResizeWindow(dpy, talkWindow, TALK_WINDOW_X, TALK_WINDOW_Y, TALK_WINDOW_WIDTH,
				  TALK_WINDOW_HEIGHT);
}

/* Return length of first prefix with length at most len wider that 'width',
 * or len + 1 if such doesn't exist. */
static int Text_width_to_pos(XFontStruct * font, char *text, size_t len, int width)
{
...
{
	if (talking && selection.talk.state == SEL_EMPHASIZED) {
...
void Talk_window_cut(XButtonEvent * xbutton)
{
	...
	if (selection.talk.state == SEL_PENDING && ButtonState == ButtonRelease
	    && Button == Button1)
		was_pending = true;
...
{
	int X_BORDER = 10 /*+ (256 - drawWindowXOffset) */ , Y_BORDER = 10;
	int SPACING = messageFont->ascent + messageFont->descent + 1;
	...
	msg0 = Get_talk_message(0);
	if (Message_get_length(msg0) == 0) {
		Selection_set_state();
		return;
	}

	x = xbutton->x - X_BORDER;
	y = xbutton->y - Y_BORDER;
	...
	if (xbutton->type == ButtonPress) {
		...
		while (last_msg_index < maxMessages
		       && Message_get_length(Get_talk_message(last_msg_index)) != 0) {
			last_msg_index++;
		}
		...
		ptr = Get_talk_message(c1.y);
		ptr_txt = Message_get_text(ptr);
		ptr_len = Message_get_length(ptr);
		if ((c1.x > XTextWidth(messageFont, (char *)ptr_txt, ptr_len)
		...
		c1.str_index = 0;
		if (c1.x_off == 1)
			c1.str_index = ptr_len - 1;
		else
			c1.str_index = Text_width_to_pos(messageFont, ptr_txt, ptr_len, c1.x);

		ptr = Get_talk_message(c2.y);
		c2.str_index = 0;
		if (c2.x_off == 1)
			c2.str_index = ptr_len - 1;
		else
			c2.str_index = Text_width_to_pos(messageFont, ptr_txt, ptr_len, c2.x);
		 ...
		 */
		if (c1.y == c2.y
		    && (c1.str_index > c2.str_index
			|| (c1.str_index == c2.str_index && c1.x_off == c2.x_off))) {
		...
		if (c2.str_index == 0) {
			if (c1.y == c2.y) {
				...
				c2.y--;
				c2.x_off = 1;
				c2.str_index = Message_get_length(Get_talk_message(c2.y)) - 1;
		...
		else if (c2.str_index == Message_get_length(Get_talk_message(c2.y))) {
			c2.str_index = Message_get_length(Get_talk_message(c2.y)) - 1;
			c2.x_off = 0;
		}
		...
		if (c1.str_index == Message_get_length(Get_talk_message(c1.y)))
			c1.str_index = Message_get_length(Get_talk_message(c1.y)) - 1;
		...
		msg = Get_talk_message(current_line);
		strlcpy(cut_str, Message_get_text(msg), sizeof(cut_str));
		cut_str_len = Message_get_length(msg);
		current_line++;
		...
		if (selection.draw.y1 == selection.draw.y2) {
			...
			for (i = selection.draw.y1 + 1; i < selection.draw.y2; i++) {
				m = Get_talk_message(current_line);
				strlcpy(cut_str, Message_get_text(m), sizeof(cut_str));
				cut_str_len = Message_get_length(m);
				current_line++;
