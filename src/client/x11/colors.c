...
XColor *Get_color(int color)
{
	...
	if (maxColors < 16 && color >= 8) {
		...
		case 13:	color = DARK_GRAY;	break;
		case 14:	color = WHITE;	break;
		case 15:	color = DARK_RED;	break;
		default:			break;
		}
	}
	...
	if (maxColors < 8 && color >= 4) {
		...
		case DARK_RED:	color = RED;	break;
		case DARK_GRAY: color = BLUE;	break;
...
static void Fill_colormap(void)
{
	...
	cells_needed = (maxColors == 16) ? 256 : (maxColors == 8) ? 64 : 16;
...
int Colors_init(void)
{
	...
	if (colorSwitch) {
		maxColors = (maxColors >= 16 && visual->map_entries >= 256) ? 16
		    : (maxColors >= 8 && visual->map_entries >= 64) ? 8 : 4;
	}
	else {
		maxColors = (maxColors >= 16 && visual->map_entries >= 16) ? 16
		    : (maxColors >= 8 && visual->map_entries >= 8) ? 8 : 4;
	}
	num_planes = (maxColors == 16) ? 4 : (maxColors == 8) ? 3 : 2;
	...
	if (multibuffer)
		dbuf_state = start_dbuff(dpy, (colormap != 0)
					 ? colormap
					 : DefaultColormap(dpy, DefaultScreen(dpy)), MULTIBUFFER,
					 num_planes, colors);
	if (dbuf_state == NULL)
		dbuf_state = start_dbuff(dpy, (colormap != 0)
					 ? colormap
	...
	if (dbuf_state == NULL && colormap == 0) {
		...
		if (dbuf_state == NULL)
			dbuf_state = start_dbuff(dpy, colormap, ((colorSwitch)
...
void Colors_init_style_colors(void)
{
	...
	for (i = 0; i < num_polygon_styles; i++)
		polygon_styles[i].pixel = (fullColor && RGB) ?
		    RGB2COLOR(polygon_styles[i].rgb) : (unsigned long) wallColor;
	for (i = 0; i < num_edge_styles; i++)
		edge_styles[i].pixel = (fullColor
					&& RGB) ? RGB2COLOR(edge_styles[i].
							    rgb) : (unsigned long) wallColor;
...
static int Colors_init_color_cube(void)
{
	...
	for (i = 0; i < NELEM(rgb_cube_sizes); i++) {
		...
		if (XAllocColorCells(dpy, (colormap != 0)
				     ? colormap
				     : DefaultColormap(dpy,
						       DefaultScreen(dpy)),
		...
		XStoreColors(dpy, (colormap != 0)
			     ? colormap : DefaultColormap(dpy, DefaultScreen(dpy)), colorarray, n);
...
void Colors_debug(void)
{
	...
	for (i = 0; i < NELEM(rgb_cube_sizes); i++) {
		...
		fprintf(fp,
			...
			"\ngreen %3lu\nblue  %3lu\n",
			RGB_PC(0, 0, 0),
			RGB_PC(255, 255, 255), RGB_PC(255, 0, 0), RGB_PC(0, 255, 0), RGB_PC(0, 0,
											    255));
	}
