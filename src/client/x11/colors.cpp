...
#define XP_COLOR5 "#992200" /* dark red */
#define XP_COLOR6 "#202020" /* dark gray */
#define XP_COLOR7 "#FFBB11" /* "xpyellow" */

#define XP_COLOR8 "#002299" /*"#770000", in ng */
#define XP_COLOR9 "#CC4400"
#define XP_COLOR10 "#DD8800"
#define XP_COLOR11 "#EE9900"

#define XP_COLOR12 "#9F9F9F"
#define XP_COLOR13 "#5F5F5F"
#define XP_COLOR14 "#DFDFDF"
#define XP_COLOR15 "#BB7700"
...
static struct Visual_class_name
{
	...
	{StaticColor, "StaticColor"}, {PseudoColor, "PseudoColor"}, {TrueColor, "TrueColor"}, {DirectColor, "DirectColor"}};
...
/*
 * This function maps xpilot color indices into XColors.
 * There are 2 issues at work here:
 * i) if fullColor is false the color index is mapped into the range
 * 0 to MAX_COLORS - 1 (15).
 * ii) if maxColors < 16 the color has to be mapped into the range
 * 0 to maxColors - 1.
 */
XColor *Get_color(int color)
{
	/* If called with color == NO_COLOR e.g. let's return BLACK. */
	if (color < 0)
		return &colors[BLACK];
	...
	/* No fullColor => map color code colors to lower colors. */
	if (color >= MAX_COLORS)
	{
		color = XPilot_color(color - MAX_COLORS);
	}

	assert(color < MAX_COLORS);

	/* Possibly map colors 8 to 15 into the range 0 to 7. */
	if (maxColors < 16 && color >= 8)
	{
		switch (color)
		{
		case 8:
	...
	/* Possibly map colors 7 to 4 into the range 0 to 3. */
	if (maxColors < 8 && color >= 4)
	{
		switch (color)
		{
		case GREEN:
...
{
	int i, num, best_size, cmap_size, using_default, visual_id, visual_class;
	...
	if (visual_class >= 0 || visual_id >= 0)
	{
		...
		else
		{
			...
			printf("Using visual %s with depth %d and %d colors\n",
				   Visual_class_name(visual->class), dispDepth, visual->map_entries);
			Get_colormap();
		}
	}
	if (visual_class < 0)
...
static int Parse_colors(Colormap cmap)
{
	...
	for (i = 0; i < maxColors; i++)
	{
		...
		STORE(XColor, colors, num_colors, max_colors, xcolor);
...
static void Fill_colormap(void)
{
	...
	cells_needed = (maxColors == 16) ? 256 : (maxColors == 8) ? 64
															  : 16;
	max_fill = MAX(256, visual->map_entries) - cells_needed;
	if (max_fill <= 0)
		return;

	if (XAllocColorCells(dpy, colormap, False, NULL, 0, pixels, (unsigned)max_fill) == False)
	{
	...
	for (i = 0; i < max_fill; i++)
	{
		...
		{
#ifdef DEVELOPMENT
			warn("Can't pre-fill color map, got %d'th pixel %lu", i, pixels[i]);
#endif
	...
	XQueryColors(dpy, DefaultColormap(dpy, DefaultScreen(dpy)), mycolors, max_fill);
...
int Colors_init(void)
{
	...
	/* Code in this file assumes these defines. */
	assert(BLACK == 0);
	assert(WHITE == 1);
	assert(BLUE == 2);
	assert(RED == 3);
	 ...
	 */
	if (visual->class == StaticColor || visual->class == TrueColor)
		colorSwitch = false;
	...
	num_planes = (maxColors == 16) ? 4 : (maxColors == 8) ? 3
														  : 2;
	...
	// xpinfo("%d colors in palette.", num_colors);
	...
	if (multibuffer)
		dbuf_state = start_dbuff(dpy, (colormap != 0) ? colormap : DefaultColormap(dpy, DefaultScreen(dpy)), MULTIBUFFER,
								 num_planes, colors);
	if (dbuf_state == NULL)
		dbuf_state = start_dbuff(dpy, (colormap != 0) ? colormap : DefaultColormap(dpy, DefaultScreen(dpy)),
								 ((colorSwitch) ? COLOR_SWITCH : PIXMAP_COPY), num_planes,
								 colors);
	if (dbuf_state == NULL && colormap == 0)
	{
		...
		if (multibuffer)
			dbuf_state = start_dbuff(dpy, colormap, MULTIBUFFER, num_planes, colors);

		if (dbuf_state == NULL)
			dbuf_state = start_dbuff(dpy, colormap, ((colorSwitch) ? COLOR_SWITCH : PIXMAP_COPY),
									 num_planes, colors);
	}
...
void Colors_init_style_colors(void)
{
	...
	for (i = 0; i < num_polygon_styles; i++)
		polygon_styles[i].pixel = (fullColor && RGB) ? RGB2COLOR(polygon_styles[i].rgb) : (unsigned long)wallColor;
	for (i = 0; i < num_edge_styles; i++)
		edge_styles[i].pixel = (fullColor && RGB) ? RGB2COLOR(edge_styles[i].rgb) : (unsigned long)wallColor;

	if (num_colors == 48 && fullColor)
	{
		int i;
		...
		for (i = 0; i < num_polygon_styles; i++)
		{
			XColor c, *cp;
			char tmp[16];
			int rgb = polygon_styles[i].rgb;

			memset(&c, 0, sizeof(XColor));
			STORE(XColor, colors, num_colors, max_colors, c);

			sprintf(tmp, "#%06x", rgb);
			cp = &colors[num_colors - 1];
			XParseColor(dpy, cmap, tmp, cp);
			cp->pixel = RGB2COLOR(rgb);
		}
		...
		xpinfo("Number of polygon styles: %d", num_polygon_styles);
		xpinfo("Number of edge styles: %d", num_edge_styles);
		xpinfo("Number of colors including style colors: %d", num_colors);
	}
}
...
int Colors_init_bitmaps(void)
{
	...
	if (dbuf_state->type == COLOR_SWITCH)
	{
		if (fullColor)
		{
			warn("Can't do fullColor if colorSwitch.");
			fullColor = false;
			texturedObjects = false;
		}
	}

	if (maxColors < MAX_COLORS)
	{
		warn("Won't do fullColor with maxColors < %d.\n", MAX_COLORS);
		fullColor = false;
	}
	...
	/* ok now we know if fullcolor is true */
	if (num_colors == 16 && fullColor)
	{
		int i;
		Colormap cmap = DefaultColormap(dpy, DefaultScreen(dpy));
 ...
 * less color distance.
 */
static void Fill_color_cube(int reds, int greens, int blues, XColor colorarray[256])
{
	int i, r, g, b;
...
static int Colors_init_color_cube(void)
{
	...
	for (i = 0; i < NELEM(rgb_cube_sizes); i++)
	{
		...
		if (XAllocColorCells(dpy, (colormap != 0) ? colormap : DefaultColormap(dpy, DefaultScreen(dpy)),
							 False, NULL, 0, &color_cube->pixels[0],
							 (unsigned)n) == False)
		{
		...
		XStoreColors(dpy, (colormap != 0) ? colormap : DefaultColormap(dpy, DefaultScreen(dpy)), colorarray, n);
...
static void Colors_free_color_cube(void)
{
	if (color_cube)
	{
		...
		{
			XFreeColors(dpy, (colormap != 0) ? colormap : DefaultColormap(dpy, DefaultScreen(dpy)),
						&color_cube->pixels[0],
						color_cube->reds * color_cube->greens * color_cube->blues, 0);
...
static int Colors_init_true_color(void)
{
	...
	if ((visual->red_mask == 0) ||
		(visual->green_mask == 0) ||
		(visual->blue_mask == 0) || ((visual->red_mask & visual->green_mask & visual->blue_mask) != 0))
	{
...
void Colors_debug(void)
{
	...
	for (i = 0; i < NELEM(rgb_cube_sizes); i++)
	{
		...
		fprintf(fp,
				...
				"\ngreen %3lu\nblue  %3lu\n",
				RGB_PC(0, 0, 0),
				RGB_PC(255, 255, 255), RGB_PC(255, 0, 0), RGB_PC(0, 255, 0), RGB_PC(0, 0, 255));
	}
...
void Init_spark_colors(void)
{
	...
	for (src = sparkColors; *src && (num_spark_colors < 9); src++)
	{
		if (isascii(*src) && isdigit(*src))
		{
			...
			{
				*dst++ = *src++;
			} while (*src && isascii(*src) && isdigit(*src) && ((size_t)(dst - buf) < (sizeof(buf) - 1)));
			*dst = '\0';
			src--;
