...
static void Widget_viewer_draw_lines(widget_t *widget, int start, int end)
{
	...
	int text_height = viewerw->font->ascent + viewerw->font->descent, i, x = 20, y = 20 + viewerw->font->ascent + start * text_height;
...
int Widget_create_activate(int parent_desc,
						   ...
						   int border, const char *str, int (*callback)(int, void *, const char **),
						   void *user_data)
{
	int widget_desc;
	Window window;
...
{
	int disp_width = DisplayWidth(dpy, 0), disp_height = DisplayHeight(dpy, 0), x, y, popup_desc;
	widget_t *widget;
	Window window;
	XSetWindowAttributes sattr;
	unsigned long mask;
...
static void Widget_resize_viewer(XEvent *event, int ind)
{
	...
	const int
		...
		hori_slider_x = 0,
		hori_slider_y = height - hori_slider_height - close_spaced_height, vert_slider_x = width - vert_slider_width, vert_slider_y = 0;
