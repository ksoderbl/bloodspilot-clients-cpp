...
/* $Id: xeventhandlers.c,v 1.13 2007/09/27 23:31:06 kps Exp $ */
...
static void Selection_paste(Window win, unsigned prop, int Delete)
{
	...
	for (nread = 0, bytes_after = 1; bytes_after > 0; nread += nitems)
	{
		if ((XGetWindowProperty(dpy, win, prop, (nread / 4), MAX_CHARS / 4,
								...
								&actual_fmt, &nitems, &bytes_after, &data) != Success))
		{
			XFree(data);
			return;
		}
...
static void Selection_request(void)
{
	...
	else
	{
		...
		XConvertSelection(dpy, XA_PRIMARY, XA_STRING, prop, talkWindow, CurrentTime);
...
static void Selection_send(const XSelectionRequestEvent *rq)
{
	...
	else if (rq->target == XA_STRING)
	{
		XChangeProperty(dpy, rq->requestor, rq->property,
						rq->target, 8, PropModeReplace, (unsigned char *)selection.txt,
						(int)selection.len);
		ev.xselection.property = rq->property;
	}
...
{
	Selection_paste(event->xselection.requestor, event->xselection.property, True);
}
...
void FocusIn_event(XEvent *event)
{
	...
	if (restorePointerControl && !talking) {
...
void ConfigureNotify_event(XEvent *event)
{
	...
	if (((unsigned)conf->width != conf_width) || ((unsigned)conf->height != conf_height))
	{
...
void KeyChanged_event(XEvent *event)
{
	...
	else if (event->xkey.window == talkWindow)
	{
		...
		else if (talk_key_repeating && event->xkey.keycode == talk_key_repeat_event.xkey.keycode)
...
int ButtonRelease_event(XEvent *event)
{
	...
	if (pointerControl)
	{
		/*assert(!talking); KHS: wrong assert */
...
{
	if (event->xexpose.window == talkWindow)
	{
