...
static void Talk_refresh(void)
{
	...
	XDrawString(dpy, talkWindow, talkGC, XTextWidth(talkFont, talk_str, (int)selection.talk.x1) + TALK_INSIDE_BORDER,
				talkFont->ascent + TALK_INSIDE_BORDER,
	...
	case KeyPress:
		...
		case CTRL('K'):
			...
			if (newlen < oldlen)
			{
				...
				assert(oldlen >= talk_cursor.point);
				XDrawString(dpy, talkWindow, talkGC,
							XTextWidth(talkFont, talk_str, (int)talk_cursor.point) + TALK_INSIDE_BORDER,
							talkFont->ascent + TALK_INSIDE_BORDER,
							&talk_str[talk_cursor.point],
...
int Talk_paste(char *data, size_t data_len, bool overwrite)
{
	...
	else
	{
		...
		if (talk_cursor.point < str_len)
		{
			...
			Set_gc_fg_color(dpy, talkGC, BLACK);
			XDrawString(dpy, talkWindow, talkGC,
						XTextWidth(talkFont, talk_backup, (int)talk_cursor.point) + TALK_INSIDE_BORDER, talkFont->ascent + TALK_INSIDE_BORDER,
...
void Talk_cut_from_messages(XButtonEvent *xbutton)
{
	...
	if (xbutton->type == ButtonPress)
	{
		/* how many messages? */
		last_msg_index = 0;
		while (last_msg_index < maxMessages && Message_get_length(Get_talk_message(last_msg_index)) != 0)
		{
			last_msg_index++;
		}
	...
	else if (xbutton->type == ButtonRelease)
	{
		...
		if ((c1.x > XTextWidth(messageFont, (char *)ptr_txt, ptr_len) || c1.x_off == 1) && c1.y < c2.y)
		{
			c1.x = 0;
			c1.y += 1;
		}
