	...
	if (arg == NULL) {
		...
		case 7:	/* h */
			Add_message("'\\help <command>' shows help about <command>. "
				    "Just '\\help' show avaiable commands " "[*Client reply*]");
			break;
		case 8:	/* set */
		case 9:	/* s */
		...
		case 10:	/* get */
		case 11:	/* g */
			Add_message("'\\get <option>' gets an option value. " "[*Client reply*]");
			break;
		case 12:	/* quit */
		case 13:	/* q */
...
static void ignorePlayer(const char *name, int level)
{
	...
	if (other != NULL) {
		if (level == 1)
			Add_message("Ignoring %s (textmask). [*Client reply*]",
				    other->nick_name);
		else
			Add_message("Ignoring %s (completely). [*Client reply*]",
				    other->nick_name);
...
static void unignorePlayer(const char *name)
{
	...
	Add_message("Stopped ignoring %s. [*Client reply*]",
		    other->nick_name);
	other->ignorelevel = 0;
}
