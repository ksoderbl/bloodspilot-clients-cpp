...
msg_type_t Message_type(const char *message)
{
	...
	if (strncmp(message, " <", 2) == 0 && message[len - 1] == '>')
...
void Add_message(const char *fmt, ...)
{
	...
	if (msgtype == TalkMessage || msgtype == ImportantGameMessage)
	{
	...
	else if (msgtype == TalkMessage)
	{
		...
		if (strlen(receiver_name) == 1 && strspn(receiver_name, "0123456789") == 1)
			receiver_team = atoi(receiver_name);
		else
			receiver = Other_by_name(receiver_name, false);

		if (self && sender && BIT(Setup->mode, TEAM_PLAY) && self->team == sender->team)
		...
		else if (strlen(receiver_name) > 0)
		{ /* team? */
	 ...
	 */
	if (messagesToStdout == 3 || (messagesToStdout >= 1 && msgtype == TalkMessage))
		printf("%s\n", message);
}
