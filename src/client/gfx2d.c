...
int Picture_load(xp_picture_t * picture, const char *filename)
{
	...
	for (p = 0; p < count; p++) {
		if (!(picture->data[p] = XMALLOC(RGB_COLOR, picture->width * picture->height))) {
			error("Not enough memory.");
			return -1;
		}
	}
...
int Picture_rotate(xp_picture_t * picture)
{
	...
	for (image = 1; image < picture->count; image++) {
		if (!(picture->data[image] = XMALLOC(RGB_COLOR, picture->width * picture->height))) {
			error("Not enough memory.");
			return -1;
		}
		for (y = 0; y < size; y++) {
...
static void Picture_scale_x_slice(const xp_picture_t * picture, int image,
				  int *r, int *g, int *b, int x, int y, double xscale, double xfrac,
				  double yfrac)
{
	double weight;
	RGB_COLOR col;
 ...
 */
RGB_COLOR Picture_get_pixel_area(const xp_picture_t * picture, int image, double x_1, double y_1,
				 double dx, double dy)
{
	int r, g, b;
	double area;
	...
	if (dy > yfrac) {
		...
		y++;
		while (dy >= 1.0) {
			Picture_scale_x_slice(picture, image, &r, &g, &b, x, y, dx, xfrac, 1.0);
			y++;
			dy -= 1.0;
		}
	}
