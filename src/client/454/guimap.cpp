/* $Id: guimap.c,v 1.6 2007/12/08 16:02:47 kps Exp $
 *
...
#define FUEL_BORDER 2
		...
		if (!text_is_bigger)
			Erase_rectangle(WINSCALE(X(x)) - 1,
							WINSCALE(Y(y + BLOCK_SZ)) - 1, WINSCALE(BLOCK_SZ) + 2,
							WINSCALE(BLOCK_SZ) + 2);
...
#define BITMAP_FUEL_BORDER 3
		...
		PaintFuelSlice(p_draw, BM_FUEL,
					   ...
					   WINSCALE(BLOCK_SZ - 2 * BITMAP_FUEL_BORDER), image, WINSCALE(size));
...
void Gui_paint_base(int x, int y, int xi, int yi, int type)
{
	if (!blockBitmaps)
	{
		...
		if (!BIT(instruments, SHOW_SHIP_NAME) || Base_info_by_pos(xi, yi, &id, &team) == -1)
			return;
		...
		if ((other = Other_by_id(id)) != NULL)
		{
			if (other->name_width == 0)
			{
				...
				other->name_width =
					2 + XTextWidth(gameFont, other->name, other->name_len);
			}
		}
...
void Gui_paint_setup_check(int x, int y, int xi, int yi)
{
	...
	else
	{
		if (Check_index_by_pos(xi, yi) == nextCheckPoint)
		{
			PaintBitmap(p_draw, BM_CHECKPOINT, WINSCALE(X(x)),
						WINSCALE(Y(y + BLOCK_SZ)), WINSCALE(BLOCK_SZ),
						WINSCALE(BLOCK_SZ), 1);
		}
		else
		...
		Erase_rectangle(WINSCALE(X(x)), WINSCALE(Y(y + BLOCK_SZ)), WINSCALE(BLOCK_SZ),
						WINSCALE(BLOCK_SZ));
	}
}
...
{
	Segment_add(hudColor, X(x), Y(y), X(x), Y(yi));

	Segment_add(hudColor, X(xi), Y(y), X(xi), Y(yi));

	Segment_add(hudColor, X(x), Y(y), X(xi), Y(y));

	Segment_add(hudColor, X(x), Y(yi), X(xi), Y(yi));
}
...
void Gui_paint_setup_item_concentrator(int x, int y)
{
	...
	if (!blockBitmaps)
	{
		...
		for (i = 0; i < NELEM(tris); i++)
		{
			...
			pts[3] = pts[0];
			rd.drawLines(dpy, p_draw, gc, pts, NELEM(pts), CoordModeOrigin);
...
void Gui_paint_decor_dot(int x, int y, int size)
{
	Rectangle_add(BLUE, X(x + BLOCK_SZ / 2) - (map_point_size >> 1),
				  Y(y + BLOCK_SZ / 2) - (map_point_size >> 1), size, size);
}
...
void Gui_paint_setup_target(int x, int y, int target, int damage, bool own)
{
	...
	rd.drawRectangle(dpy, p_draw, gc,
					 ...
					 WINSCALE(Y(y + 3 * BLOCK_SZ / 4)), WINSCALE(BLOCK_SZ / 2),
					 WINSCALE(BLOCK_SZ / 2));
	Erase_4point(WINSCALE(X(x + (BLOCK_SZ + 2) / 4)), WINSCALE(Y(y + 3 * BLOCK_SZ / 4)),
	...
	if (BIT(Setup->mode, TEAM_PLAY))
	{
		...
		rd.drawString(dpy, p_draw, gc, WINSCALE(X(x + BLOCK_SZ / 2)) - size / 2,
					  WINSCALE(Y(y + BLOCK_SZ / 2)) + gameFont->ascent / 2, s, 1);
		Erase_rectangle(WINSCALE(X(x + BLOCK_SZ / 2)) - size / 2 - 1,
						WINSCALE(Y(y + BLOCK_SZ / 2)) + gameFont->ascent / 2 - gameFont->ascent - 1, size + 2,
						gameFont->ascent + gameFont->descent + 2);
	}
...
void Gui_paint_setup_treasure(int x, int y, int treasure, bool own)
{
	if (!blockBitmaps)
	{
		...
		if (BIT(Setup->mode, TEAM_PLAY))
		{
			...
			Erase_rectangle(WINSCALE(X(x + BLOCK_SZ / 2)) - size / 2 - 1,
							WINSCALE(Y(y + 2 * BALL_RADIUS)) - gameFont->ascent - 1, size + 2,
							gameFont->ascent + gameFont->descent + 2);
		}
	}
	else
	{
		...
		if (BIT(Setup->mode, TEAM_PLAY))
		{
			...
			Erase_rectangle(WINSCALE(X(x + BLOCK_SZ / 2)) - size / 2 - 1,
							WINSCALE(Y(y + BALL_RADIUS + 5)) - gameFont->ascent - 1, size + 2,
							gameFont->ascent + gameFont->descent + 2);
		}
	}
}
