/* $Id: xpilot.c,v 1.5 2007/09/27 23:30:59 kps Exp $
 *
...
{
	int result;
	int auto_connect = false, text = false, list_servers = false, auto_shutdown = false, noLocalMotd = false;
	char *cp;
	...
	Parse_options(&argc, argv, conpar->real_name,
				  ...
				  &text, &list_servers,
				  &auto_connect, &noLocalMotd, conpar->nick_name, conpar->disp_name,
	...
	if (text || auto_connect || argv[1] || is_this_windows())
	{
		...
		result = Contact_servers(argc - 1, &argv[1],
								 ...
								 auto_shutdown, shutdown_reason, 0, 0, 0, 0, conpar);
	}
	else
	{
