...
static void Paint_lock(int hud_pos_x, int hud_pos_y)
{
	...
	rd.drawString(dpy, p_draw, gc,
		      ...
		      - gameFont->descent, target->name, target->name_len);
	...
	if (BIT(Setup->mode, LIMITED_LIVES) || lock_dist != 0) {
		...
		Erase_rectangle(WINSCALE(hud_pos_x + HUD_SIZE - HUD_OFFSET
					 + BORDER) - 1,
				WINSCALE(hud_pos_y - HUD_SIZE + HUD_OFFSET - BORDER)
				- gameFont->descent - gameFont->ascent, XTextWidth(gameFont, str,
										   3) + 2,
				gameFont->ascent + gameFont->descent);
	}
	SET_FG(colors[hudColor].pixel);
	...
	if (lock_dist != 0) {
		...
		if (lock_dist > WARNING_DISTANCE || warningCount++ % 2 == 0) {
			...
			if (self != NULL
				...
				Arc_add(hudColor, (int) (hud_pos_x + HUD_SIZE * 0.6 * tcos(lock_dir)
							 - size * 0.5),
					(int) (hud_pos_y - HUD_SIZE * 0.6 * tsin(lock_dir)
					       - size * 0.5), size, size, 0, 64 * 360);
			}
			else {
				...
				x = (int) (hud_pos_x + HUD_SIZE * 0.6 * tcos(lock_dir)
					   - size * 0.5), y =
				    (int) (hud_pos_y - HUD_SIZE * 0.6 * tsin(lock_dir)
					   - size * 0.5), rd.fillArc(dpy, p_draw, gc, WINSCALE(x),
								     WINSCALE(y), WINSCALE(size),
								     WINSCALE(size), 0, 64 * 360);
...
void Paint_HUD(void)
{
	...
	if (fuelCount || fuelSum < fuelLevel3) {
		...
		rd.drawString(dpy, p_draw, gc,
			      WINSCALE(hud_pos_x + HUD_SIZE - HUD_OFFSET + BORDER),
			      WINSCALE(hud_pos_y + HUD_SIZE - HUD_OFFSET + BORDER)
			      + gameFont->ascent, str, strlen(str));
		...
		if (numItems[ITEM_TANK]) {
			...
			rd.drawString(dpy, p_draw, gc,
				      WINSCALE(hud_pos_x + HUD_SIZE - HUD_OFFSET + BORDER),
				      WINSCALE(hud_pos_y + HUD_SIZE - HUD_OFFSET + BORDER)
				      + gameFont->descent + 2 * gameFont->ascent, str, strlen(str));
			Erase_rectangle(WINSCALE(hud_pos_x + HUD_SIZE - HUD_OFFSET + BORDER)
					- 1, WINSCALE(hud_pos_y + HUD_SIZE - HUD_OFFSET + BORDER)
					+ gameFont->descent + gameFont->ascent,
					XTextWidth(gameFont, str, strlen(str)) + 2,
					gameFont->ascent + gameFont->descent);
		}
	...
	for (i = 0, j = 0; i < MAX_SCORE_OBJECTS; i++) {
		...
		if (sobj->hud_msg_len > 0) {
			...
			rd.drawString(dpy, p_draw, gc,
				      ...
				      WINSCALE(hud_pos_y + HUD_SIZE - HUD_OFFSET + BORDER)
				      + gameFont->ascent
				      + j * (gameFont->ascent + gameFont->descent), sobj->hud_msg,
				      sobj->hud_msg_len);
	...
	if (time_left > 0) {
		...
		rd.drawString(dpy, p_draw, gc,
			      ...
			      WINSCALE(hud_pos_y - HUD_SIZE + HUD_OFFSET - BORDER)
			      - gameFont->descent, str, strlen(str));
		Erase_rectangle(WINSCALE(hud_pos_x - HUD_SIZE + HUD_OFFSET - BORDER)
				- size - 1, WINSCALE(hud_pos_y - HUD_SIZE + HUD_OFFSET - BORDER)
				- gameFont->ascent - gameFont->descent, size + 2,
				gameFont->ascent + gameFont->descent);
	}

	/* Update the modifiers */
	modlen = strlen(mods);
	rd.drawString(dpy, p_draw, gc, WINSCALE(hud_pos_x - HUD_SIZE + HUD_OFFSET - BORDER)
		      - XTextWidth(gameFont, mods, modlen),
		      WINSCALE(hud_pos_y + HUD_SIZE - HUD_OFFSET + BORDER)
		      + gameFont->ascent, mods, strlen(mods));
	...
	if (autopilotLight) {
		...
		rd.drawString(dpy, p_draw, gc,
			      ...
			      WINSCALE(hud_pos_y - HUD_SIZE + HUD_OFFSET - BORDER)
			      - gameFont->descent * 2 - gameFont->ascent, autopilot,
			      sizeof(autopilot) - 1);
	...
	if (BIT(instruments, SHOW_FUEL_GAUGE) == 0 || !((fuelCount)
							|| (fuelSum < fuelLevel3
							    &&
							    ((fuelSum < fuelLevel1
							      && (loops % 4) < 2)
...
void Add_message(char *message)
{
	...
	if (messagesToStdout == 2
	    || (messagesToStdout == 1 && message[0] && message[strlen(message) - 1] == ']')) {
