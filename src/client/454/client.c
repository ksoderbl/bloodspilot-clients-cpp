...
void Map_dots(void)
{
	...
	if (map_point_size > 0) {
		...
		for (i = 0; i < num_cannons; i++) {
			...
			else if (map_point_distance > 0 && x % map_point_distance == 0
...
int Handle_war(int robot_id, int killer_id)
{
	...
	if ((robot = Other_by_id(robot_id)) == NULL) {
		errno = 0;
		IFNWINDOWS(error
			   ("Can't update war for non-existing player (%d,%d)", robot_id,
			    killer_id);
		    )
		    return 0;
	}
	...
	if ((killer = Other_by_id(killer_id)) == NULL) {
		errno = 0;
		IFNWINDOWS(error
			   ("Can't update war against non-existing player (%d,%d)", robot_id,
			    killer_id);
		    )
		    return 0;
	}
...
int Client_power(void)
{
	...
	if (Send_power(power) == -1
	    ...
	    || Send_turnresistance_s(turnresistance_s) == -1 || Send_display() == -1
