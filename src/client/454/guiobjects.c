/* $Id: guiobjects.c,v 1.7 2007/12/08 16:02:47 kps Exp $
 *
	...
	if (!blockBitmaps) {
		...
		if (ballTile != None) {
			...
			rd.fillArc(dpy, p_draw, gc,
				   ...
				   WINSCALE(2 * BALL_RADIUS), WINSCALE(2 * BALL_RADIUS), 0,
				   64 * 360);
			Erase_rectangle(WINSCALE(x - BALL_RADIUS), WINSCALE(y - BALL_RADIUS),
					WINSCALE(2 * BALL_RADIUS), WINSCALE(2 * BALL_RADIUS));
		}
		else {
			Arc_add(WHITE, x - BALL_RADIUS, y - BALL_RADIUS, 2 * BALL_RADIUS,
				2 * BALL_RADIUS, 0, 64 * 360);
		}
		if (ballTile != None) {
...
static void Gui_paint_mine_name(int x, int y, char *name)
{
	...
	if (name != NULL) {
		rd.drawString(dpy, p_draw, gc,
			      WINSCALE(x) - name_width / 2, WINSCALE(y + 4) + gameFont->ascent + 1,
			      name, name_len);
		Erase_rectangle(WINSCALE(x) - name_width / 2 - 1, WINSCALE(y + 4) - 3 + 1,
				name_width + 2, gameFont->ascent + gameFont->descent + 6);
	}
}
...
void Gui_paint_mine(int x, int y, int teammine, char *name)
{
	if (!blockBitmaps) {
		...
		rd.drawLines(dpy, p_draw, gc, mine_points, 21, CoordModePrevious);
		Erase_rectangle(WINSCALE(x - 8) - 1, WINSCALE(y - 4) - 1, WINSCALE(17) + 2,
				WINSCALE(9) + 2);
...
void Gui_paint_spark(int color, int x, int y)
{
	...
	Rectangle_add(color, x - spark_size / 2, y - spark_size / 2, spark_size, spark_size);
...
void Gui_paint_fastshot(int color, int x, int y)
{
	if (!blockBitmaps) {
		...
		if (showNastyShots) {
			...
			Rectangle_add(color, x - z, y - z, shot_size, shot_size);
		}
	}
	else {
...
void Gui_paint_teamshot(int color, int x, int y)
{
	if (!blockBitmaps) {
		...
		int z = s_size / 2;
		PaintBitmap(p_draw, BM_BULLET_OWN, WINSCALE(x) - z, WINSCALE(y) - z, 8, 8,
			    s_size - 1);
	}
}
...
void Gui_paint_missile(int x, int y, int len, int dir)
{
	...
	rd.drawLine(dpy, p_draw, gc, WINSCALE(x1), WINSCALE(y1), WINSCALE(x2), WINSCALE(y2));
	Erase_segment(4, WINSCALE(x1), WINSCALE(y1), WINSCALE(x2), WINSCALE(y2));
}
...
#endif
	rd.drawLine(dpy, p_draw, gc, WINSCALE(X(x1)), WINSCALE(Y(y1)), WINSCALE(X(x2)),
		    WINSCALE(Y(y2)));
	Erase_segment(3, WINSCALE(X(x1)), WINSCALE(Y(y1)), WINSCALE(X(x2)), WINSCALE(Y(y2)));
	...
	if (!blockBitmaps) {
		...
		if (count <= 0 || loops % 10 >= 5) {
			...
			rd.drawRectangle(dpy, p_draw, gc,
					 ...
					 WINSCALE(2 * (half_pause_size + 1)),
					 WINSCALE(2 * (half_pause_size + 1)));
			rd.drawString(dpy, p_draw, gc, WINSCALE(X(x)) - pauseCharWidth / 2,
...
#endif
		rd.drawLine(dpy, p_draw, gc, WINSCALE(X(x0)), WINSCALE(Y(y0)), WINSCALE(X(x1)),
			    WINSCALE(Y(y1)));
		Erase_segment(1, WINSCALE(X(x0)), WINSCALE(Y(y0)), WINSCALE(X(x1)),
			      WINSCALE(Y(y1)));
	}
	else {
		int size = WINSCALE(8);
...
void Gui_paint_connector(int x0, int y0, int x1, int y1, int tractor)
{
	...
	rd.drawLine(dpy, p_draw, gc, WINSCALE(X(x0)), WINSCALE(Y(y0)), WINSCALE(X(x1)),
		    WINSCALE(Y(y1)));
	Erase_segment(1, WINSCALE(X(x0)), WINSCALE(Y(y0)), WINSCALE(X(x1)), WINSCALE(Y(y1)));
	if (tractor) {
		rd.setDashes(dpy, gc, 0, dashes, NUM_DASHES);
	}
}
...
static void Gui_paint_rounddelay(int x, int y)
{
	...
	rd.drawString(dpy, p_draw, gc, WINSCALE(X(x)) - text_width / 2,
		      WINSCALE(Y(y)) + gameFont->ascent / 2, s, t);
}
...
static void Gui_paint_ship_name(int x, int y, other_t * other)
{
	...
	rd.drawString(dpy, p_draw, gc,
		      ...
		      WINSCALE(Y(y) + 16) + gameFont->ascent, other->name, other->name_len);
...
static int Gui_calculate_ship_color(int id, other_t * other)
{
	...
	if (BIT(Setup->mode, TEAM_PLAY)
	    && self != NULL && self->id != id && other != NULL && self->team == other->team) {
		ship_color = BLUE;
	}

	if (self != NULL
	    && self->id != id && other != NULL && self->alliance != ' '
	...
	if (((loops + id) & 0xF) == 0) {
		for (lcnt = 0; lcnt < ship->num_l_light; lcnt++) {
			Rectangle_add(RED,
				      ...
				      Y(y + ship->l_light[lcnt][dir].y) - 2, 6, 6);
			Segment_add(RED,
				    X(x + ship->l_light[lcnt][dir].x) - 8,
				    Y(y + ship->l_light[lcnt][dir].y),
				    X(x + ship->l_light[lcnt][dir].x) + 8,
				    Y(y + ship->l_light[lcnt][dir].y));
			Segment_add(RED, X(x + ship->l_light[lcnt][dir].x),
		...
		for (lcnt = 0; lcnt < ship->num_r_light; lcnt++) {
			...
			Segment_add(rightLightColor,
				    ...
				    X(x + ship->r_light[lcnt][dir].x) + 8,
				    Y(y + ship->r_light[lcnt][dir].y));
			Segment_add(rightLightColor, X(x + ship->r_light[lcnt][dir].x),
...
*/
	...
	if (ecolor != -1) {	/* outer shield */
		...
		rd.drawArc(dpy, p_draw, gc,
			   ...
			   WINSCALE(Y(y + half_e_radius)), WINSCALE(e_radius), WINSCALE(e_radius),
			   0, 64 * 360);
		Erase_arc(WINSCALE(X(x - half_e_radius)), WINSCALE(Y(y + half_e_radius)),
	...
	if (scolor != -1) {
		...
		rd.drawArc(dpy, p_draw, gc,
			   ...
			   WINSCALE(Y(y + half_radius)), WINSCALE(radius), WINSCALE(radius), 0,
			   64 * 360);
		Erase_arc(WINSCALE(X(x - half_radius)), WINSCALE(Y(y + half_radius)),
...
{
	PaintBitmap(p_draw, ship, WINSCALE(X(x) - 16), WINSCALE(Y(y) - 16), WINSCALE(32),
		    WINSCALE(32), ang);
}
...
static void Gui_paint_ship_uncloaked(int id, XPoint * points, int ship_color, int point_count)
{
	...
	if (!useErase) {
		if (lock_id == id && id != -1 && lock_dist != 0) {
			...
			rd.fillPolygon(dpy, p_draw, gc, points, point_count, Complex,
				       CoordModeOrigin);
		}
	}
		    ...
		    int eshield)
{
	...
	if (BIT(instruments, SHOW_SHIP_NAME)
	    && self != NULL && self->id != id && other != NULL) {
