/* $Id: default.c,v 1.7 2007/12/08 16:02:47 kps Exp $
 *
	...
	for (i = 0; i < NELEM(options); i++) {
		...
		if (options[i].fallback && options[i].fallback[0]) {
			printf("        The default %s: %s.\n", (options[i].key == KEY_DUMMY)
			       ? "value is" : (strchr(options[i].fallback, ' ') == NULL)
			       ? "key is" : "keys are", options[i].fallback);
		}
		printf("\n");
	}
...
static int Find_resource(XrmDatabase db, const char *resource, char *result, unsigned size,
			 int *index)
{
#ifndef _WINDOWS
	int i;
	...
	sprintf(str_class, "%s.%c%s", myClass, islower(*resource) ? toupper(*resource) : *resource,
		resource + 1);
...
static int Get_resource(XrmDatabase db, const char *resource, char *result, unsigned size)
{
	int index;
...
static int Get_string_resource(XrmDatabase db, const char *resource, char *result, unsigned size)
{
	char *src, *dst;
	int index, val;
...
static void Get_int_resource(XrmDatabase db, const char *resource, int *result)
{
	int index;
	...
	if (sscanf(resValue, "%d", result) <= 0) {
		errno = 0;
		error("Bad value \"%s\" for option \"%s\", using default...", resValue, resource);
		sscanf(options[index].fallback, "%d", result);
	}
}


static void Get_float_resource(XrmDatabase db, const char *resource, DFLOAT * result)
{
	int index;
	double temp_result;
	char resValue[MAX_CHARS];
...
static void Get_bit_resource(XrmDatabase db, const char *resource, long *mask, int bit)
{
	int index;
	char resValue[MAX_CHARS];
...
{
	int len;
	char *ptr, *lang = getenv("LANG"), *home = getenv("HOME"), path[PATH_MAX + 1];
	XrmDatabase tmpDB;
...
void Parse_options(int *argcp, char **argvp, char *realName, int *port,
		   ...
		   int *join, int *noLocalMotd, char *nickName, char *dispName, char *hostName,
		   char *shut_msg)
{
	char *ptr;
	char *str;
	...
	if (nickName[0] < 'A' || nickName[0] > 'Z') {
		errno = 0;
		error("Your player name \"%s\" should start with an uppercase letter", nickName);
		exit(1);
	}
	...
	Get_bit_resource(rDB, "showRadar", &instruments, SHOW_RADAR);
