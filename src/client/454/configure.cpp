/* $Id: configure.c,v 1.8 2007/12/08 16:02:47 kps Exp $
 *
...
static int Config_create_showRadar(int widget_desc, int *height);
...
static bool config_created = false, config_mapped = false;
static int config_page,
	...
	config_entry_height,
	config_bool_width,
	config_bool_height, config_int_width, config_float_width, config_arrow_width,
	config_arrow_height;
static int *config_widget_desc, config_save_confirm_desc = NO_WIDGET;
static int (*config_creator[])(int widget_desc, int *height) = {
	...
	Config_create_filledDecor, Config_create_texturedDecor, Config_create_texturedBalls,
	...
	Config_create_packetDropMeter, Config_create_packetLagMeter, Config_create_clock,
	Config_create_clockAMPM,
#ifdef _WINDOWS
	Config_create_threadedDraw,
#endif
	Config_create_scaleFactor, Config_create_altScaleFactor,
...
{
	int i, num, height, offset, width, widget_desc;
	bool full;
	...
	for (i = 0; i < NELEM(config_creator); i++)
	{
		if (full == true)
		{
			full = false;
			num++;
			config_widget_desc[num] = Widget_create_form(NO_WIDGET, top, config_x, config_y, config_width,
														 config_height, 0);
			...
			width = 2 * config_button_space + XTextWidth(buttonFont, "PREV", 4);
			offset = config_width - width - config_space;
			widget_desc =
				Widget_create_activate(config_widget_desc[num],
									   offset, height,
									   width, config_button_height,
									   0, "PREV", Config_prev, (void *)(long)num);
			if (widget_desc == 0)
			{
				break;
			}
			width = 2 * config_button_space + XTextWidth(buttonFont, "NEXT", 4);
			offset = (config_width - width) / 2;
			widget_desc =
				Widget_create_activate(config_widget_desc[num],
									   offset, height,
									   width, config_button_height,
									   0, "NEXT", Config_next, (void *)(long)num);
			if (widget_desc == 0)
			{
				break;
			}
			width = 2 * config_button_space + XTextWidth(buttonFont, "CLOSE", 5);
			offset = config_space;
			widget_desc =
				Widget_create_activate(config_widget_desc[num],
									   offset, height,
									   width, config_button_height,
									   0, "CLOSE", Config_close, (void *)(long)num);
			if (widget_desc == 0)
			{
				break;
			}
	...
	{
		config_max = num + 1;
		config_widget_desc = (int *)realloc(config_widget_desc, config_max * sizeof(int));
		config_page = 0;
...
static int Config_create_bool(int widget_desc, int *height,
							  const char *str, bool val, int (*callback)(int, void *, bool *),
							  void *data)
{
	int offset, label_width, boolw;
	...
	Widget_create_label(widget_desc, config_space, *height + (config_entry_height - config_text_height) / 2, label_width,
						config_text_height, 0, str);
	if (config_space + label_width > offset)
	{
	...
	boolw = Widget_create_bool(widget_desc,
							   ...
							   config_bool_width, config_bool_height, 0, val, callback, data);
...
static int Config_create_float(int widget_desc, int *height,
							   ...
							   int (*callback)(int, void *, DFLOAT *), void *data)
{
	int offset, label_width, floatw;
	...
	floatw = Widget_create_float(widget_desc, offset, *height + (config_entry_height - config_text_height) / 2,
								 config_float_width, config_text_height, 0, val, min, max,
								 callback, data);
	offset += config_float_width;
	Widget_create_arrow_left(widget_desc, offset, *height + (config_entry_height - config_arrow_height) / 2,
							 config_arrow_width, config_arrow_height, 0, floatw);
	offset += config_arrow_width;
	Widget_create_arrow_right(widget_desc, offset, *height + (config_entry_height - config_arrow_height) / 2,
							  config_arrow_width, config_arrow_height, 0, floatw);
	*height += config_entry_height + config_space;
...
static int Config_create_power(int widget_desc, int *height)
{
	return Config_create_float(widget_desc, height,
							   "power", &power, MIN_PLAYER_POWER, MAX_PLAYER_POWER,
...
static int Config_create_turnResistance(int widget_desc, int *height)
{
	return Config_create_float(widget_desc, height,
							   ...
							   MAX_PLAYER_TURNRESISTANCE, Config_update_turnResistance, NULL);
}
...
static int Config_create_altPower(int widget_desc, int *height)
{
	return Config_create_float(widget_desc, height,
							   "altPower", &power_s,
							   MIN_PLAYER_POWER, MAX_PLAYER_POWER, Config_update_altPower,
							   NULL);
}
...
static int Config_create_altTurnResistance(int widget_desc, int *height)
{
	return Config_create_float(widget_desc, height,
							   ...
							   MIN_PLAYER_TURNRESISTANCE,
							   MAX_PLAYER_TURNRESISTANCE, Config_update_altTurnResistance,
							   NULL);
}
...
static int Config_create_maxMessages(int widget_desc, int *height)
{
	return Config_create_int(widget_desc, height, "maxMessages", &maxMessages, 1, MAX_MSGS,
							 NULL, NULL);
}

static int Config_create_messagesToStdout(int widget_desc, int *height)
{
	return Config_create_int(widget_desc, height, "messagesToStdout", &messagesToStdout, 0, 2,
							 NULL, NULL);
}
...
static int Config_create_oldMessagesColor(int widget_desc, int *height)
{
	return Config_create_int(widget_desc, height,
							 "oldMessagesColor", &oldMessagesColor, 0, maxColors - 1, NULL,
							 NULL);
}
...
static int Config_create_speedFactHUD(int widget_desc, int *height)
{
	return Config_create_float(widget_desc, height, "speedFactHUD", &hud_move_fact, -10.0, 10.0,
							   NULL, NULL);
}

static int Config_create_speedFactPTR(int widget_desc, int *height)
{
	return Config_create_float(widget_desc, height, "speedFactPTR", &ptr_move_fact, -10.0, 10.0,
							   NULL, NULL);
}

static int Config_create_fuelNotify(int widget_desc, int *height)
{
	return Config_create_int(widget_desc, height, "fuelNotify", &fuelLevel3, 0, 1000, NULL,
							 NULL);
}

static int Config_create_fuelWarning(int widget_desc, int *height)
{
	return Config_create_int(widget_desc, height, "fuelWarning", &fuelLevel2, 0, 1000, NULL,
							 NULL);
}

static int Config_create_fuelCritical(int widget_desc, int *height)
{
	return Config_create_int(widget_desc, height, "fuelCritical", &fuelLevel1, 0, 1000, NULL,
							 NULL);
}
...
static int Config_create_texturedObjects(int widget_desc, int *height)
{
	return Config_create_bool(widget_desc, height, "texturedObjects",
							  (blockBitmaps) ? true : false, Config_update_texturedObjects,
							  NULL);
}

static int Config_create_showRadar(int widget_desc, int *height)
{
	return Config_create_bool(widget_desc, height, "showRadar", BIT(instruments, SHOW_RADAR) ? true : false, Config_update_instruments, (void *)SHOW_RADAR);
}
...
static int Config_create_showItems(int widget_desc, int *height)
{
	return Config_create_bool(widget_desc, height, "showItems", BIT(instruments, SHOW_ITEMS) ? true : false, Config_update_instruments, (void *)SHOW_ITEMS);
}
...
static int Config_create_showItemsTime(int widget_desc, int *height)
{
	return Config_create_float(widget_desc, height,
							   ...
							   MIN_SHOW_ITEMS_TIME, MAX_SHOW_ITEMS_TIME, NULL, NULL);
}

static int Config_create_showScoreDecimals(int widget_desc, int *height)
{
	return Config_create_int(widget_desc, height, "showScoreDecimals", &showScoreDecimals, 0, 2,
							 NULL, NULL);
}
...
static int Config_create_backgroundPointSize(int widget_desc, int *height)
{
	return Config_create_int(widget_desc, height,
							 ...
							 MIN_MAP_POINT_SIZE, MAX_MAP_POINT_SIZE, Config_update_dots, NULL);
}

static int Config_create_sparkSize(int widget_desc, int *height)
{
	return Config_create_int(widget_desc, height,
							 "sparkSize", &spark_size, MIN_SPARK_SIZE, MAX_SPARK_SIZE, NULL,
							 NULL);
}
...
{
	return Config_create_float(widget_desc, height,
							   "sparkProb", &spark_prob, 0.0, 1.0, Config_update_sparkProb,
							   NULL);
}
...
{
	return Config_create_int(widget_desc, height,
							 "charsPerSecond", &charsPerSecond, 10, 255,
...
static int Config_create_toggleShield(int widget_desc, int *height)
{
	return Config_create_bool(widget_desc, height, "toggleShield",
							  (toggle_shield) ? true : false, Config_update_toggleShield, NULL);
}

static int Config_create_autoShield(int widget_desc, int *height)
{
	return Config_create_bool(widget_desc, height, "autoShield",
							  (auto_shield) ? true : false, Config_update_autoShield, NULL);
}

static int Config_create_shotSize(int widget_desc, int *height)
{
	return Config_create_int(widget_desc, height, "shotSize", &shot_size, MIN_SHOT_SIZE,
							 MAX_SHOT_SIZE, NULL, NULL);
}

static int Config_create_teamShotSize(int widget_desc, int *height)
{
	return Config_create_int(widget_desc, height,
							 "teamShotSize", &teamshot_size, MIN_TEAMSHOT_SIZE,
							 MAX_TEAMSHOT_SIZE, NULL, NULL);
}

static int Config_create_showNastyShots(int widget_desc, int *height)
{
	return Config_create_bool(widget_desc, height, "nastyShots",
							  (showNastyShots) ? true : false, Config_update_bool,
							  &showNastyShots);
}

static int Config_create_hudColor(int widget_desc, int *height)
{
	return Config_create_int(widget_desc, height, "hudColor", &hudColor, 1, maxColors - 1, NULL,
							 NULL);
}

static int Config_create_hudLockColor(int widget_desc, int *height)
{
	return Config_create_int(widget_desc, height, "hudLockColor", &hudLockColor, 1,
							 maxColors - 1, NULL, NULL);
}

static int Config_create_wallColor(int widget_desc, int *height)
{
	return Config_create_int(widget_desc, height, "wallColor", &wallColor, 1, maxColors - 1,
							 NULL, NULL);
}

static int Config_create_decorColor(int widget_desc, int *height)
{
	return Config_create_int(widget_desc, height, "decorColor", &decorColor, 1, maxColors - 1,
							 NULL, NULL);
}

static int Config_create_showDecor(int widget_desc, int *height)
{
	return Config_create_bool(widget_desc, height, "showDecor", BIT(instruments, SHOW_DECOR) ? true : false, Config_update_instruments, (void *)SHOW_DECOR);
}
...
static int Config_create_maxVolume(int widget_desc, int *height)
{
	return Config_create_int(widget_desc, height, "maxVolume", &maxVolume, 0, 255, NULL, NULL);
}
#endif

static int Config_create_maxFPS(int widget_desc, int *height)
{
	return Config_create_int(widget_desc, height, "maxFPS", &maxFPS, 1, 255,
...
static int Config_create_clock(int widget_desc, int *height)
{
	return Config_create_bool(widget_desc, height, "clock", BIT(instruments, SHOW_CLOCK) ? true : false, Config_update_instruments, (void *)SHOW_CLOCK);
}
...
static int Config_create_threadedDraw(int widget_desc, int *height)
{
	return Config_create_bool(widget_desc, height, "threadedDraw", ThreadedDraw,
							  Config_update_bool, &ThreadedDraw);
}
#endif
...
static int Config_create_scaleFactor(int widget_desc, int *height)
{
	return Config_create_float(widget_desc, height,
							   ...
							   MIN_SCALEFACTOR, MAX_SCALEFACTOR, Config_update_scaleFactor,
							   NULL);
}
...
{
	return Config_create_float(widget_desc, height,
							   "altScaleFactor", &scaleFactor_s, MIN_SCALEFACTOR,
							   MAX_SCALEFACTOR, NULL, NULL);
}

static int Config_create_markingLights(int widget_desc, int *height)
{
	return Config_create_bool(widget_desc, height, "markingLights",
							  markingLights, Config_update_bool, &markingLights);
}

static int Config_create_save(int widget_desc, int *height)
{
	static char save_str[] = "Save Configuration";
	int space, button_desc, width = 2 * config_button_space + XTextWidth(buttonFont, save_str, strlen(save_str));
	...
	button_desc =
		Widget_create_activate(widget_desc,
							   ...
							   *height + space / 2,
							   width, config_button_height, 0, save_str, Config_save,
							   (void *)save_str);
...
static int Xpilotrc_add(char *line)
{
	...
	if (num_xpilotrc >= max_xpilotrc)
	{
		max_xpilotrc *= 2;
		if ((xpilotrc_ptr =
				 (xpilotrc_t *)realloc(xpilotrc_ptr,
...
static void Xpilotrc_use(char *line)
{
	...
	for (i = 0; i < num_xpilotrc; i++)
	{
		if (strncmp(xpilotrc_ptr[i].line + 7, line + 7, xpilotrc_ptr[i].size + 1) == 0)
		{
			free(xpilotrc_ptr[i].line);
...
static int Config_save(int widget_desc, void *button_str, const char **strptr)
{
	...
	char oldfile[PATH_MAX + 1], newfile[PATH_MAX + 1];
	...
	*strptr = "Saving...";
	...
	Config_save_bool(fp, "showRadar", BIT(instruments, SHOW_RADAR));
