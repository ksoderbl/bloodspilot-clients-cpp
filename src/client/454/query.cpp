/* $Id: query.c,v 1.5 2007/09/27 23:30:59 kps Exp $
 *
...
static int Query_subnet(sock_t *sock,
						struct sockaddr_in *host_addr, struct sockaddr_in *mask_addr, char *msg,
						int msglen)
{
	int i, nbits, max;
	...
	for (i = 1; i <= max; i++)
	{
		...
		sock_get_error(sock);
		sendto(sock->fd, msg, msglen, 0, (struct sockaddr *)&addr, sizeof(addr));
...
int Query_all(sock_t *sock, int port, char *msg, int msglen)
{
	...
	for (len = 0; len + sizeof(struct ifreq) <= ifconf.ifc_len;)
	{
		...
		if ((ifflags & (IFF_LOOPBACK | IFF_POINTOPOINT | IFF_BROADCAST)) != 0)
		{
			...
			if (sendto(sock->fd, msg, msglen, 0, (struct sockaddr *)&addr, sizeof addr) == msglen)
			{
	...
	if (haslb /* && broadcasts == 0 */)
	{
		...
		addr.sin_port = htons(port);
		if (sendto(sock->fd, msg, msglen, 0, (struct sockaddr *)&addr, sizeof addr) ==
			msglen)
		{
			D(printf("\tsendto %s/%d\n", inet_ntoa(addr.sin_addr), port););
			count++;
