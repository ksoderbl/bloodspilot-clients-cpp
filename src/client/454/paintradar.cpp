/* $Id: paintradar.c,v 1.6 2007/09/27 23:30:59 kps Exp $
 *
...
static void Copy_static_radar(void)
{
	...
	if (s_radar != p_radar)
	{
		...
		XCopyArea(dpy, s_radar, p_radar, gc, 0, 0, 256, RadarHeight, 0, 0);
	}
	else
	{
		/* Clear radar */
		XSetForeground(dpy, radarGC, colors[BLACK].pixel);
		XFillRectangle(dpy, p_radar, radarGC, 0, 0, 256, RadarHeight);
	}
#else
...
static void Windows_copy_sliding_radar(float xf, float yf)
{
	...
	slidingradar_y = (RadarHeight - (int)(pos.y * yf + 0.5) - 1 + RadarHeight / 2) % RadarHeight;

	/*
	 * Draw slidingradar in four chunks onto the screen.
	 */
	WinXBltPixToWin(s_radar, radar,
					slidingradar_x, slidingradar_y, 256 - slidingradar_x,
					RadarHeight - slidingradar_y, 0, 0);
	WinXBltPixToWin(s_radar, radar, 0, slidingradar_y, slidingradar_x,
					RadarHeight - slidingradar_y, 256 - slidingradar_x, 0);
	WinXBltPixToWin(s_radar, radar, slidingradar_x, 1, 256 - slidingradar_x, slidingradar_y, 0,
					RadarHeight - slidingradar_y);
	WinXBltPixToWin(s_radar, radar, 0, 1, slidingradar_x, slidingradar_y, 256 - slidingradar_x,
					RadarHeight - slidingradar_y);
	p_radar = radar;
...
static void Paint_checkpoint_radar(float xf, float yf)
{
	...
	if (BIT(Setup->mode, TIMING))
	{
		...
		points[4].x = x;
		points[4].y = y;
		XDrawLines(dpy, p_radar, radarGC, points, 5, 0);
	}
}
...
static void Paint_self_radar(float xf, float yf)
{
	...
	if (selfVisible != 0 && loops % 16 < 13)
	{
		...
		XDrawLine(dpy, p_radar, radarGC, x, y, x1, y1);
		if (BIT(Setup->mode, WRAP_PLAY))
		{
			...
			if (xw != 0)
			{
				XDrawLine(dpy, p_radar, radarGC, x - xw, y, x1 - xw, y1);
			}
			if (yw != 0)
			{
				XDrawLine(dpy, p_radar, radarGC, x, y - yw, x1, y1 - yw);
				if (xw != 0)
				{
					XDrawLine(dpy, p_radar, radarGC, x - xw, y - yw, x1 - xw,
							  y1 - yw);
				}
			}
		}
...
static void Paint_objects_radar(void)
{
	...
	for (i = 0; i < num_radar; i++)
	{
		...
		if (BIT(Setup->mode, WRAP_PLAY))
		{
			...
			if (xw != 0)
			{
				(*radarDrawRectanglePtr)(dpy, p_radar, radarGC, x - xw, y, s, s);
			}
			if (yw != 0)
			{
				(*radarDrawRectanglePtr)(dpy, p_radar, radarGC, x, y - yw, s, s);

				if (xw != 0)
				{
					(*radarDrawRectanglePtr)(dpy, p_radar, radarGC, x - xw,
											 y - yw, s, s);
				}
			}
		}
...
{
	const float xf = 256.0f / (float)Setup->width, yf =
													   (float)RadarHeight / (float)Setup->height;

	if (BIT(instruments, SHOW_RADAR) == 0)
	{
		return;
	}
...
void Paint_world_radar(void)
{
		...
		visibleColor[SETUP_FILLED_NO_DRAW] =
			visibleColor[SETUP_REC_LU] =
				visibleColor[SETUP_REC_RU] =
					visibleColor[SETUP_REC_LD] = visibleColor[SETUP_REC_RD] = visibleColor[SETUP_FUEL] =
						wallRadarColor;
	for (i = 0; i < 10; i++)
	{
		...
		visibleColor[SETUP_DECOR_FILLED] =
			visibleColor[SETUP_DECOR_LU] =
				visibleColor[SETUP_DECOR_RU] =
					visibleColor[SETUP_DECOR_LD] = visibleColor[SETUP_DECOR_RD] = decorRadarColor;
	}
		...
		for (xi = 0; xi < Setup->x; xi++)
		{
			...
			for (yi = 0; yi < Setup->y; yi++)
			{
...
#else
						XSetForeground(dpy, radarGC,
									   colors[currColor].pixel);
#endif
					}
					else
					{
				...
				if (visibleColorChange)
				{
					if (nsegment > 0)
					{
						XDrawSegments(dpy, s_radar, radarGC, segments,
									  nsegment);
						nsegment = 0;
					}
					if (npoint > 0)
					{
						XDrawPoints(dpy, s_radar, radarGC, points, npoint,
									CoordModeOrigin);
						npoint = 0;
					}
					start = end = yp;
	...
	if (npoint > 0)
	{
		XDrawPoints(dpy, s_radar, radarGC, points, npoint, CoordModeOrigin);
	}
