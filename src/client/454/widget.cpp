/* $Id: widget.c,v 1.5 2007/09/27 23:30:59 kps Exp $
 *
...
static widget_t *Widget_new(int *descp)
{
	...
	else
	{
		...
		widgets = (widget_t *)realloc(widgets, max_widgets * sizeof(widget_t));
	}
	if (widgets == NULL)
	{
...
static int Widget_add_child(int parent_desc, int child_desc)
{
	...
	{
		incr = 4;
		form->children = (int *)malloc((form->num_children + incr) * sizeof(*form->children));
	}
	else
	{
		incr = 4 + form->num_children / 2;
		form->children = (int *)realloc(form->children, (form->num_children + incr) * sizeof(*form->children));
	}
	if (form->children == NULL)
	{
		form->num_children = 0;
...
static void Widget_draw_button(widget_t *widget, bool inverse, const char *label)
{
	...
	ShadowDrawString(dpy, widget->window, buttonGC, x, buttonFont->ascent + y, label, fg, bg);
}
...
static void Widget_draw_input(widget_t *widget, const char *str)
{
	...
	XDrawString(dpy, widget->window, textGC,
				(widget->width - XTextWidth(textFont, str, strlen(str))) / 2,
				textFont->ascent + (widget->height - (textFont->ascent + textFont->descent)) / 2, str,
				strlen(str));
}
	...
	for (i = start; i < end; i++)
	{
		if (!mono)
		{
			...
			XDrawString(dpy, widget->window, motdGC,
						x + 1, y + 1, (char *)viewerw->line[i].txt,
						viewerw->line[i].len);
		...
		{
			XDrawString(dpy, widget->window, motdGC,
						x, y, (char *)viewerw->line[i].txt, viewerw->line[i].len);
		}
		y += text_height;
	}
	XSetForeground(dpy, motdGC, colors[WHITE].pixel);
...
{
	widget_viewer_t *viewerw = (widget_viewer_t *)widget->sub;
	int start, end, y0, y1, text_height = viewerw->font->ascent + viewerw->font->descent;
...
static void Widget_button(XEvent *event, int widget_desc, bool pressed)
{
	...
	widget_arrow_t *arroww;
	widget_entry_t *entryw;
	int i, ival, sub_widget_desc;
	DFLOAT fval, delta, fmin, offset, newoffset;
					   ...
					   int border)
{
	...
	window =
		XCreateSimpleWindow(dpy, parent_window,
							x, y, width, height, border, colors[borderColor].pixel,
							  ...
							  int (*callback)(int, void *, const char **), void *user_data)
{
	...
	window =
		XCreateSimpleWindow(dpy, pulldown_widget->window,
							...
							width, height, 1, colors[borderColor].pixel,
							colors[buttonColor].pixel);
...
int Widget_create_float(int parent_desc,
						...
						int (*callback)(int, void *, DFLOAT *), void *user_data)
{
	int widget_desc;
	Window window;
	widget_t *parent_widget;
	...
	XSelectInput(dpy, window, ExposureMask);
	widget_desc =
		Widget_create(WIDGET_INPUT_FLOAT, "input_float", window, width, height, floatw);
	if (widget_desc == NO_WIDGET)
	{
		return NO_WIDGET;
	}
...
int Widget_create_label(int parent_desc, int x, int y, int width, int height, int border,
						const char *str)
{
	int widget_desc;
	Window window;
...
int Widget_create_confirm(const char *confirm_str, int (*callback)(int, void *, const char **))
{
	...
	label_desc = Widget_create_label(popup_desc,
									 ...
									 label_space, label_width, label_height, 0, confirm_str);
...
static int Widget_viewer_calculate_text(int viewer_desc)
{
	...
	for (count = i = 0; count < viewerw->num_lines; count++, i++)
	{
		...
		line[count].txt_width =
			XTextWidth(viewerw->font, (char *)line[count].txt, line[count].len);
						 ...
						 const char *window_name, const char *icon_name, XFontStruct *font)
{
	...
	viewerw->overlay =
		XCreateSimpleWindow(dpy, popup_widget->window,
							...
							0, colors[borderColor].pixel, colors[windowColor].pixel);
