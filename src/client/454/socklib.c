/* $Id: socklib.c,v 1.4 2007/09/27 23:30:59 kps Exp $
 *
...
void sock_get_local_hostname(char *name, unsigned size, int search_domain_for_xpilot)
{
...
{
	if (setsockopt(sock->fd, SOL_SOCKET, SO_BROADCAST, (void *) &flag, sizeof(flag)) < 0) {
...
{
	if (setsockopt(sock->fd, SOL_SOCKET, SO_RCVBUF, (void *) &size, sizeof(size)) < 0) {
		sock_set_error(sock, errno, SOCK_CALL_SETSOCKOPT, __LINE__);
		return SOCK_IS_ERROR;
	}
	return SOCK_IS_OK;
...
{
	if (setsockopt(sock->fd, SOL_SOCKET, SO_SNDBUF, (void *) &size, sizeof(size)) < 0) {
		sock_set_error(sock, errno, SOCK_CALL_SETSOCKOPT, __LINE__);
		return SOCK_IS_ERROR;
	}
	return SOCK_IS_OK;
...
static struct hostent *sock_get_host_by_name(const char *name)
{
	...
	h = WSAAsyncGetHostByName(notifyWnd, WM_GETHOSTNAME, name, chp, MAXGETHOSTSTRUCT);
