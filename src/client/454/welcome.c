/* $Id: welcome.c,v 1.6 2007/09/27 23:30:59 kps Exp $
 *
...
static void Ping_servers(void)
{
	...
	do
	{
		while (outstanding < (ms / interval + 1))
		{
			...
			it_sip = SI_DATA(it);
			sock_send_dest(&sock, it_sip->ip_str, it_sip->port, sbuf.buf, sbuf.len);
...
static int Internet_server_show_cb(int widget, void *user_data, const char **text)
{
	...
	*global_conpar = *conpar;
	...
	Widget_create_activate(subform_widget,
			       ...
			       border ? border : 1, sip->hostname, Internet_server_join_cb,
			       (void *) sip);
	...
	for (i = 0; i < NELEM(labels); i++) {
		label_width = XTextWidth(textFont, labels[i].label, strlen(labels[i].label));
	...
	for (i = 0; i < NELEM(labels); i++) {
		Widget_create_label(subform_widget,
				    ...
				    label_width, labels[i].height, label_border, labels[i].label);
	}
	...
	for (s = sip->playlist; (s = strchr(s, ',')) != NULL; s++) {
		playlist_lines++;
		Widget_create_label(subform_widget, label_x, label_y, label_width, label_height,
				    label_border, "");
		label_y += label_height + label_space;
	}
#endif
