/* $Id: colors.c,v 1.5 2007/09/27 23:30:58 kps Exp $
 *
...
static struct Visual_class_name
{
	...
	{StaticGray, "StaticGray"}, {GrayScale, "GrayScale"}, {StaticColor, "StaticColor"}, {PseudoColor, "PseudoColor"}, {TrueColor, "TrueColor"}, {DirectColor, "DirectColor"}};
...
static void Fill_colormap(void)
{
	...
	if (XAllocColorCells(dpy, colormap, False, NULL, 0, pixels, max_fill) == False)
	{
		errno = 0;
...
#ifdef DEVELOPMENT
			errno = 0;
			error("Can't pre-fill color map, got %d'th pixel %lu", i, pixels[i]);
#endif
...
int Colors_init(void)
{
	 ...
	 */
	if (visual->class == StaticGray || visual->class == StaticColor || visual->class == TrueColor)
	{
		colorSwitch = false;
	}
	...
	num_planes = (mono == true) ? 1 : (maxColors == 16) ? 4
								  : (maxColors == 8)	? 3
														: 2;
 ...
 * less color distance.
 */
static void Fill_color_cube(int reds, int greens, int blues, XColor colors[256])
{
	int i, r, g, b;
...
static int Colors_init_color_cube(void)
{
	...
	for (i = 0; i < NELEM(rgb_cube_sizes); i++)
	{
		...
		if (XAllocColorCells(dpy, (colormap != 0) ? colormap : DefaultColormap(dpy, DefaultScreen(dpy)),
							 False, NULL, 0, &color_cube->pixels[0], n) == False)
		{
		...
		XStoreColors(dpy, (colormap != 0) ? colormap : DefaultColormap(dpy, DefaultScreen(dpy)), colors, n);
