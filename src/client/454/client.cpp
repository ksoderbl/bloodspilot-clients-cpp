/* $Id: client.c,v 1.8 2007/12/08 16:02:47 kps Exp $
 *
...
static int num_others = 0, max_others = 0;
...
int Handle_player(int id, int player_team, int mychar, char *player_name, char *real_name,
				  char *host_name, char *shape)
{
	other_t *other;
	...
	if ((other = Other_by_id(id)) == NULL)
	{
		if (num_others >= max_others)
		{
			...
			{
				Others = (other_t *)realloc(Others, max_others * sizeof(other_t));
			}
			if (Others == NULL)
			{
...
int Handle_war(int robot_id, int killer_id)
{
	...
	{
		errno = 0;
		IFNWINDOWS(error("Can't update war for non-existing player (%d,%d)", robot_id,
						 killer_id);)
		return 0;
	}
	if (killer_id == -1)
	...
	{
		errno = 0;
		IFNWINDOWS(error("Can't update war against non-existing player (%d,%d)", robot_id,
						 killer_id);)
		return 0;
	}
...
int Handle_seek(int programmer_id, int robot_id, int sought_id)
{
	...
	{
		errno = 0;
		error("Bad player seek (%d,%d,%d)", programmer_id, robot_id, sought_id);
		return 0;
	}
	robot->war_id = sought_id;
	sprintf(msg, "%s has programmed %s to seek %s.", programmer->name, robot->name,
			sought->name);
	Add_message(msg);
	scoresChanged = 1;
...
#ifndef _WINDOWS
		errno = 0;
		error("Can't update score for non-existing player %d,%.2f,%d", id, score, life);
#endif
		return 0;
	}
...
int Handle_score_object(DFLOAT score, int x, int y, char *msg)
{
	...
	if (msg[0] != '\0')
	{
		...
		sobj->hud_msg_width = XTextWidth(gameFont, sobj->hud_msg, sobj->hud_msg_len);
	}
	else
		sobj->hud_msg_len = 0;
...
int Client_power(void)
{
	...
	if (Send_power(power) == -1 || Send_power_s(power_s) == -1 || Send_turnspeed(turnspeed) == -1 || Send_turnspeed_s(turnspeed_s) == -1 || Send_turnresistance(turnresistance) == -1 || Send_turnresistance_s(turnresistance_s) == -1 || Send_display() == -1 || Startup_server_motd() == -1)
	{
		return -1;
	}
