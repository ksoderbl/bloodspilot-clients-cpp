/* $Id: paint.c,v 1.5 2007/09/27 23:30:58 kps Exp $
 *
...
void Paint_frame(void)
{
	...
	if (p_radar != radar && radar_exposures > 0)
	{
		...
		{
#ifndef _WINDOWS
			XCopyArea(dpy, p_radar, radar, gc, 0, 0, 256, RadarHeight, 0, 0);
#else
			WinXBltPixToWin(p_radar, radar, 0, 0, 256, RadarHeight, 0, 0);
#endif
		}
		else
		{
...
#ifndef _WINDOWS
			XCopyArea(dpy, p_radar, radar, gc, 0, 0, x, y, w, h);
			XCopyArea(dpy, p_radar, radar, gc, x, 0, w, y, 0, h);
			XCopyArea(dpy, p_radar, radar, gc, 0, y, x, h, w, 0);
			XCopyArea(dpy, p_radar, radar, gc, x, y, w, h, 0, 0);
#else
			Paint_world_radar();
#endif
		}
	...
	{
		XCopyArea(dpy, p_draw, draw, gc, 0, 0, ext_view_width, ext_view_height, 0, 0);
	}
...
static void Paint_score_background(int thisLine)
{
	...
	else
	{
		...
		IFWINDOWS(XFillRectangle(dpy, players, scoreListGC, 0, 0, players_width, BG_IMAGE_HEIGHT);)

		PaintBitmap(players, BM_SCORE_BG, 0, 0, players_width, BG_IMAGE_HEIGHT, 0);
		...
		PaintBitmap(players, BM_LOGO, 0, players_height - LOGO_HEIGHT, players_width,
					LOGO_HEIGHT, 0);
...
void Paint_score_start(void)
{
	...
	ShadowDrawString(dpy, players, scoreListGC,
					 SCORE_BORDER, thisLine, headingStr, colors[WHITE].pixel,
					 colors[BLACK].pixel);

	gcv.line_style = LineSolid;
	XChangeGC(dpy, scoreListGC, GCLineStyle, &gcv);
	XDrawLine(dpy, players, scoreListGC, SCORE_BORDER, thisLine, players_width - SCORE_BORDER,
			  thisLine);
...
void Paint_score_entry(int entry_num, other_t *other, bool best)
{
	...
	else
	{
		...
		{
			sprintf(scoreStr, "%*.*f", 8 - showScoreDecimals, showScoreDecimals,
					other->score);
		}
		else
		{
	...
	if ((other->mychar == 'D' || other->mychar == 'P' || other->mychar == 'W') && !mono)
	{
		...
		XDrawString(dpy, players, scoreListGC, SCORE_BORDER, thisLine, label,
					strlen(label));
	}
	else
	{
		ShadowDrawString(dpy, players, scoreListGC,
						 SCORE_BORDER, thisLine, label, colors[WHITE].pixel,
						 colors[BLACK].pixel);
	}
	...
	if (best)
	{
		XDrawLine(dpy, players, scoreListGC, SCORE_BORDER, thisLine,
...
static void Paint_clock(int redraw)
{
	int minute, hour, height = scoreListFont->ascent + scoreListFont->descent + 3, border = 3;
	time_t t;
	struct tm *m;
	char buf[16];
	...
	if (BIT(instruments, SHOW_CLOCK) == 0)
	{
		if (width != 0)
		{
			...
			XFillRectangle(dpy, players, scoreListGC,
						   256 - (width + 2 * border), 0, width + 2 * border, height);
			width = 0;
		}
		return;
	}
	...
	XSetForeground(dpy, scoreListGC, colors[windowColor].pixel);
	XFillRectangle(dpy, players, scoreListGC, 256 - (width + 2 * border), 0, width + 2 * border,
				   height);
	ShadowDrawString(dpy, players, scoreListGC, 256 - (width + border),
					 scoreListFont->ascent + 4, buf, colors[WHITE].pixel, colors[BLACK].pixel);
}

void ShadowDrawString(Display *dpy, Window w, GC gc, int x, int y, const char *str,
