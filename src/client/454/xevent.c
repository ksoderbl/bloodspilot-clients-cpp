/* $Id: xevent.c,v 1.7 2007/12/08 16:02:48 kps Exp $
 *
...
void Pointer_control_set_state(int onoff)
{
	if (onoff) {
		pointerControl = true;
		XGrabPointer(dpy, draw, true, 0, GrabModeAsync, GrabModeAsync, draw,
			     pointerControlCursor, CurrentTime);
		XWarpPointer(dpy, None, draw, 0, 0, 0, 0, draw_width / 2, draw_height / 2);
...
int Key_init(void)
{
	if (sizeof(keyv) != KEYBOARD_SIZE) {
		error("%s, %d: keyv size %d, KEYBOARD_SIZE is %d", __FILE__, __LINE__, sizeof(keyv),
		      KEYBOARD_SIZE);
		exit(1);
	}
...
void Key_event(XEvent * event)
{
	...
	for (key = Lookup_key(event, ks, true); key != KEY_DUMMY;
	...
	if (pointerControl) {
		if (!talk_mapped) {
			...
			/* This is a HACK to fix mouse control under windows. */
			{
				...
				XWarpPointer(dpy, None, draw, 0, 0, 0, 0, draw_width / 2,
					     draw_height / 2);
			}
			/* fix end */
#endif
