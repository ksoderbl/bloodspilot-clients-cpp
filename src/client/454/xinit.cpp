...
static void Init_spark_colors(void)
{
	...
	for (src = sparkColors; *src && (num_spark_colors < 9); src++)
	{
		if (isascii(*src) && isdigit(*src))
		{
			...
			{
				*dst++ = *src++;
			} while (*src && isascii(*src) && isdigit(*src) && ((dst - buf) < (sizeof(buf) - 1)));
			*dst = '\0';
			src--;
...
int Init_top(void)
{
	...
	{
		itemBitmaps[i] = XCreateBitmapFromData(dpy, top, (char *)itemBitmapData[i].data, ITEM_SIZE,
											   ITEM_SIZE);
	}
...
void WinXCreateItemBitmaps()
{
	...
	for (i = 0; i < NUM_ITEMS; i++)
	{
		itemBitmaps[i][ITEM_HUD] = WinXCreateBitmapFromData(dpy, draw,
															(char *)itemBitmapData[i].data,
															ITEM_SIZE, ITEM_SIZE, colors[hudColor].pixel);
		itemBitmaps[i][ITEM_PLAYFIELD] = WinXCreateBitmapFromData(dpy, draw,
																  (char *)itemBitmapData[i].data,
...
#ifndef _WINDOWS
	if (selectionAndHistory && ((x2 = (message_t *)malloc(2 * MAX_MSGS * sizeof(message_t))) == NULL))
	{
