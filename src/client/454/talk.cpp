...
static void Talk_refresh(void)
{
	...
	XDrawString(dpy, talk_w, talkGC, selection.talk.x1 * XTextWidth(talkFont, talk_str, 1) + TALK_INSIDE_BORDER,
				talkFont->ascent + TALK_INSIDE_BORDER,
...
int Talk_place_cursor(XButtonEvent *xbutton, bool pending)
{
	 ...
	 */
	if (cursor_pos < 0 || cursor_pos > TALK_WINDOW_WIDTH / onewidth || y < 0 || y >= TALK_WINDOW_HEIGHT)
	{
		if (Button == 1 && pending)
		{
