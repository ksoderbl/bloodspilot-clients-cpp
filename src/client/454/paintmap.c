/* $Id: paintmap.c,v 1.5 2007/09/27 23:30:59 kps Exp $
 *
...
void Paint_vbase(void)
{
	...
	if (num_vbase > 0) {
		for (i = 0; i < num_vbase; i++) {
			Gui_paint_base(vbase_ptr[i].x, vbase_ptr[i].y, vbase_ptr[i].xi,
				       vbase_ptr[i].yi, vbase_ptr[i].type);
		}
		...
		for (rxb = xb; rxb <= xe; rxb++, xi++, x += BLOCK_SZ, mapptr += Setup->y) {
			...
			if (!(type & BLUE_BIT)) {
				...
				case SETUP_FILLED_NO_DRAW:
					if (BIT
					    (instruments, SHOW_FILLED_WORLD | SHOW_TEXTURED_WALLS)
					    && fill_top_left == -1) {
				...
				if (!BIT(instruments, SHOW_FILLED_WORLD | SHOW_TEXTURED_WALLS)) {
					...
					if (fill_top_right != -1) {
						...
						rd.fillPolygon(dpy, p_draw, gc, points, 5, Convex,
							       CoordModeOrigin);
						if (wallTileDoit) {
						...
						if (useErase) {
							int left_x =
							    MIN(fill_bottom_left, fill_top_left);
							int right_x =
							    MAX(fill_bottom_right, fill_top_right);
							Erase_rectangle(WINSCALE(X(left_x)) - 1,
									WINSCALE(Y(y + BLOCK_SZ)) -
									1,
									WINSCALE(right_x - left_x) +
									4, WINSCALE(BLOCK_SZ) + 3);
						}
						fill_top_left =
						    fill_top_right = fill_bottom_left =
						    fill_bottom_right = -1;
					}
				}
			}
		...
		if (fill_top_left != -1) {
			...
			if (useErase) {
				...
				Erase_rectangle(WINSCALE(X(left_x)) - 1,
						...
						WINSCALE(x - left_x) + 4, WINSCALE(BLOCK_SZ) + 3);
			}
