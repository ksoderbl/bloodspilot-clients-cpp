/* $Id: blockbitmaps.c,v 1.4 2007/09/27 23:30:58 kps Exp $
 *
...
void Block_bitmap_create(Display * dpy, Drawable d, xp_pixmap_t * xp_pixmap, int number, int width,
			 int height);
	...
	for (i = 0; i < NUM_BITMAPS; i++) {
		IFWINDOWS(Progress("Loading image: %s", xp_pixmaps[i].filename);
		    ...
		    (xp_pixmaps[i].rotations >
		     0) ? xp_pixmaps[i].rotations : -xp_pixmaps[i].rotations;
		...
		if (Picture_init
		    (&xp_pixmaps[i].picture, xp_pixmaps[i].height, xp_pixmaps[i].width,
		     images) == -1) {
			break;
		}
		if (Picture_load(&xp_pixmaps[i].picture, xp_pixmaps[i].filename) == -1) {
			break;
		}
...
void PaintBitmap(Drawable d, int type, int x, int y, int width, int height, int number)
{
...
*/
void PaintFuelSlice(Drawable d, int type, int x, int y, int width, int height, int image, int size)
{
	...
	if (scaled_bitmaps < 3) {
		for (i = 0; i < 3 - scaled_bitmaps; i++) {
			...
			if (xp_pixmaps[rotation_types[type]].bitmaps[number].scale_height !=
			    WINSCALE(32)) {
				Block_bitmap_create(dpy, d, &xp_pixmaps[rotation_types[type]],
...
void Block_bitmap_create(Display * dpy, Drawable d, xp_pixmap_t * xp_pixmap, int image, int width,
			 int height)
{
...
void Block_bitmap_create_begin(Drawable d, xp_pixmap_t * xp_pixmap, int image, int width,
			       int height)
{
	Drawable pixmap;
	...
	pixmap = XCreatePixmap(dpy, d, width, height, dispDepth);
	if (!pixmap) {
...
void Block_bitmap_set_pixel(xp_pixmap_t * xp_pixmap, int image, int x, int y, RGB_COLOR color)
{
	...
	pixel = (RGB) (r, g, b);
	SET_FG(pixel);
	XDrawPoint(dpy, xp_pixmap->bitmaps[image].bitmap, gc, x, y);

	pixel = (color) ? 1 : 0;
	XSetForeground(dpy, maskGC, pixel);
	XDrawPoint(dpy, xp_pixmap->bitmaps[image].mask, maskGC, x, y);
}
...
*/
void Block_bitmap_paint_fuel_slice(Drawable d, int type, int x, int y, int width, int height,
				   int image, int size)
{
	...
	XCopyArea(dpy, xp_pixmaps[type].bitmaps[image].bitmap,
		  ...
		  box->xmax + 1 - box->xmin,
		  size * (box->ymax + 1 - box->ymin) / bit->scale_width, x + box->xmin,
		  y + box->ymin);
}
...
void Block_bitmap_paint(Drawable d, int type, int x, int y, int width, int height, int number)
{
	xp_pixmap_t *pix = &xp_pixmaps[type];
	...
	XCopyArea(dpy, bit->bitmap,
		  d, gc,
		  0 + box->xmin, 0 + box->ymin,
		  box->xmax + 1 - box->xmin, box->ymax + 1 - box->ymin, x + box->xmin,
		  y + box->ymin);
