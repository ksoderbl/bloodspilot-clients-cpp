/* $Id: xinit.c,v 1.5 2007/09/27 23:30:59 kps Exp $
 *
 ...
 */
static XFontStruct *Set_font(Display * dpy, GC gc, const char *fontName, const char *resName)
{
	XFontStruct *font;
...
static void Init_disp_prop(Display * d, Window win, int w, int h, int x, int y, int flags)
{
	XClassHint xclh;
	XWMHints xwmh;
	XSizeHints xsh;
	 ...
	 */
	if (titleFlip)
		sprintf(msg, "Successful connection to server at \"%s\".", servername);
	else
...
int Init_top(void)
{
	...
	if (shieldDrawMode == -1) {
		...
		if (strcmp(ServerVendor(dpy),
			   ...
			   "Network Computing Devices Inc.") == 0 && ProtocolVersion(dpy) == 11)
			shieldDrawMode = 1;
	...
	top = XCreateWindow(dpy,
			    ...
			    top_x, top_y, top_width, top_height, 0, dispDepth, InputOutput, visual,
			    mask, &sattr);
	...
	if (kdpy) {
		...
		keyboard = XCreateSimpleWindow(kdpy,
					       ...
					       BlackPixel(dpy, scr));
		XSelectInput(kdpy, keyboard, KeyPressMask | KeyReleaseMask | FocusChangeMask);
		Init_disp_prop(kdpy, keyboard, top_width, top_height, top_x, top_y, top_flags);
	}
#else				/* _WINDOWS */
	...
	for (i = 0; i < NUM_ITEMS; i++) {
		itemBitmaps[i]
		    = XCreateBitmapFromData(dpy, top, (char *) itemBitmapData[i].data, ITEM_SIZE,
					    ITEM_SIZE);
	}
	...
	XSetState(dpy, gc, WhitePixel(dpy, DefaultScreen(dpy)), BlackPixel(dpy, DefaultScreen(dpy)),
		  GXcopy, AllPlanes);
	XSetState(dpy, radarGC, WhitePixel(dpy, DefaultScreen(dpy)),
...
int Init_playing_windows(void)
{
...
#endif
	draw =
	    XCreateSimpleWindow(dpy, top, 258, 0, draw_width, draw_height, 0, 0,
				colors[BLACK].pixel);
	IFWINDOWS(if (draw != 1) error("draw != 1");)
		radar =
		    XCreateSimpleWindow(dpy, top, 0, 0, 256, RadarHeight, 0, 0,
					colors[BLACK].pixel);
	...
	textWindow = XCreateSimpleWindow(dpy, top, 0, 0, 0, 0, 0, 0, colors[BLACK].pixel);
	textGC = WinXCreateWinDC(textWindow);

	msgWindow = XCreateSimpleWindow(dpy, top, 0, 0, 0, 0, 0, 0, colors[BLACK].pixel);
	...
	buttonWindow = XCreateSimpleWindow(dpy, top, 0, 0, 0, 0, 0, 0, colors[BLACK].pixel);
...
#define BUTTON_WIDTH	84
	...
	button_form = Widget_create_form(0, top, 0, RadarHeight, 256, ButtonHeight + 2, 0);
	Widget_create_activate(button_form,
			       0 + 0 * BUTTON_WIDTH, 0, BUTTON_WIDTH, ButtonHeight, 1, "QUIT",
			       Quit_callback, NULL);
	Widget_create_activate(button_form, 1 + 1 * BUTTON_WIDTH, 0, BUTTON_WIDTH, ButtonHeight, 1,
			       "ABOUT", About_callback, NULL);
	menu_button =
	    Widget_create_menu(button_form, 2 + 2 * BUTTON_WIDTH, 0, BUTTON_WIDTH, ButtonHeight, 1,
			       "MENU");
	Widget_add_pulldown_entry(menu_button, "KEYS", Keys_callback, NULL);
	Widget_add_pulldown_entry(menu_button, "CONFIG", Config_callback, NULL);
	Widget_add_pulldown_entry(menu_button, "SCORE", Score_callback, NULL);
	Widget_add_pulldown_entry(menu_button, "PLAYER", Player_callback, NULL);
	Widget_add_pulldown_entry(menu_button, "MOTD", Motd_callback, NULL);
#ifdef _WINDOWS
	Widget_add_pulldown_entry(menu_button, "CREDITS", Credits_callback, NULL);
#endif
	...
	players
	    = XCreateSimpleWindow(dpy, top,
				  ...
				  players_width, players_height, 0, 0, colors[windowColor].pixel);
#ifdef _WINDOWS
	...
	pointerControlCursor =
	    XCreatePixmapCursor(dpy, pix, pix, &colors[BLACK], &colors[BLACK], 0, 0);
	XFreePixmap(dpy, pix);
	...
	for (i = 0; i < 2 * MAX_MSGS; i++) {
		if (i < MAX_MSGS) {
			...
			IFNWINDOWS(if (selectionAndHistory) TalkMsg_pending[i] = x2;)
				...
				IFNWINDOWS(if (selectionAndHistory) GameMsg_pending[i - MAX_MSGS] =
					   x2;)
					}
					x->txt[0] = '\0';
				x->len = 0;
		...
		void Resize(Window w, int width, int height) {
			...
			if (dbuf_state->type == PIXMAP_COPY) {
				...
				p_draw =
				    XCreatePixmap(dpy, draw, draw_width, draw_height, dispDepth);
			}
#endif
			players_height = top_height - (RadarHeight + ButtonHeight + 2);
			XResizeWindow(dpy, players, players_width, players_height);
#ifdef _WINDOWS
			WinXResize();
#endif
			Talk_resize();
