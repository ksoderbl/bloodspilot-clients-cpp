/* $Id: caudio.c,v 1.5 2007/09/27 23:30:58 kps Exp $
 *
...
int Handle_audio(int type, int volume)
{
	...
	if (!table[type].private[pick])
	{
		...
		for (i = 0; i < MAX_SOUNDS; i++)
			if (i != type && table[i].filenames && table[i].private[pick] && strcmp(table[type].filenames[0], table[i].filenames[0]) == 0)
			{
