/* $Id: netclient.c,v 1.7 2007/12/08 16:02:47 kps Exp $
 *
...
static long last_keyboard_change,
    last_keyboard_ack,
    last_keyboard_update, last_send_anything, reliable_offset, talk_pending, talk_sequence_num,
    talk_last_send;
...
static int Uncompress_map(void)
{
	...
	*p;			/* temporary search pointer */
	...
	while (cmp >= Setup->map_data) {
		...
		while (p < cmp) {
			...
			*cmp &= ~SETUP_COMPRESSED;
			...
			if (ump < cmp) {
				errno = 0;
				error("Map uncompression error (%d,%d)", cmp - Setup->map_data,
				      ump - Setup->map_data);
				return -1;
			}
		}
	...
	if (ump != cmp) {
		errno = 0;
		error("map uncompress error (%d,%d)", cmp - Setup->map_data, ump - Setup->map_data);
		return -1;
	}
...
int Net_start(void)
{
	...
	for (retries = 0;;) {
		...
		if (cbuf.ptr[0] != PKT_REPLY) {
			errno = 0;
			error("Not a reply packet after play (%d,%d,%d)", cbuf.ptr[0],
...
static int Net_packet(void)
{
	...
	while (rbuf.buf + rbuf.len > rbuf.ptr) {
		...
		if (receive_tbl[type] == NULL) {
			errno = 0;
			IFNWINDOWS(error
				   ("Received unknown packet type (%d, %d), dropping frame.", type,
				    prev_type);
			    )
			    Sockbuf_clear(&rbuf);
			break;
			...
			if (result == -1) {
				if (type != PKT_QUIT) {
					errno = 0;
					error("Processing packet type (%d, %d) failed", type,
					      prev_type);
				}
				return -1;
			}
...
int Net_input(void)
{
	...
	for (i = 0; i < receive_window_size; i++) {
		...
		if ((i == receive_window_size - 1 && i > 0)
		    ...
		    || drawPending || (ThreadedDraw
				       && !WaitForSingleObject(dinfo.eventNotDrawing,
							       0) == WAIT_OBJECT_0)
#endif
		    ) {
			/*
	...
	for (i = 1; i < receive_window_size; i++, last_frame++) {
		...
		else if (frame->loops < oldest_frame->loops || oldest_frame->loops == 0) {
			oldest_frame = frame;
		}
	}
...
{
	int n;
	short x, y, vx, vy, lockId, lockDist, fuelSum, fuelMax;
	...
	n = Packet_scanf(&rbuf,
			 ...
			 &ch,
			 &x, &y, &vx, &vy, &heading,
			 &power, &turnspeed, &turnresistance, &lockId, &lockDist, &lockDir,
			 &nextCheckPoint);
	if (n <= 0) {
		return n;
	}
	...
	if (version >= 0x4203) {
		...
		n = Packet_scanf(&rbuf,
				 ...
				 &(num_items[ITEM_TRACTOR_BEAM]), &(num_items[ITEM_AUTOPILOT]));
		if (n <= 0) {
			return n;
		}
	}
	n = Packet_scanf(&rbuf,
			 ...
			 &ext_view_width, &ext_view_height, &debris_colors, &stat, &autopilotLight);
	if (n <= 0) {
		return n;
	}
	...
	if (version < 0x4203) {
		if (version >= 0x3720) {
			n = Packet_scanf(&rbuf, "%c%c%c%c",
					 ...
					 &(num_items[ITEM_HYPERJUMP]), &(num_items[ITEM_PHASING])
			    );
			if (n <= 0) {
				return n;
			}
			...
			if (version >= 0x3200) {
				n = Packet_scanf(&rbuf, "%c", &(num_items[ITEM_EMERGENCY_SHIELD]));
				if (n <= 0) {
					return n;
				}
			}
	...
	Handle_self(x, y, vx, vy, heading,
		    ...
		    nextCheckPoint, autopilotLight, num_items, currentTank, fuelSum, fuelMax,
		    rbuf.len);
...
int Receive_refuel(void)
{
	...
	if ((n = Packet_scanf(&rbuf, "%c%hd%hd%hd%hd", &ch, &x0, &y0, &x1, &y1)) <= 0) {
		return n;
	}
...
int Receive_connector(void)
{
	...
	n = Packet_scanf(&rbuf, "%c%hd%hd%hd%hd%c", &ch, &x0, &y0, &x1, &y1, &tractor);
	if (n <= 0)
		return n;
...
int Receive_trans(void)
{
	...
	if ((n = Packet_scanf(&rbuf, "%c%hd%hd%hd%hd", &ch, &x1, &y1, &x2, &y2)) <= 0) {
		return n;
	}
...
int Receive_player(void)
{
	...
	char name[MAX_CHARS], real[MAX_CHARS], host[MAX_CHARS], shape[2 * MSG_LEN], *cbuf_ptr =
	    cbuf.ptr;

	if ((n = Packet_scanf(&cbuf,
			      "%c%hd%c%c" "%s%s%s" "%S", &ch, &id, &myteam, &mychar, name, real,
			      host, shape)) <= 0) {
		return n;
	}
...
int Receive_reliable(void)
{
	...
	if (rbuf.ptr + len > rbuf.buf + rbuf.len) {
		errno = 0;
		error("Not all reliable data in packet (%d,%d,%d)", rbuf.ptr - rbuf.buf, len,
		      rbuf.len);
		rbuf.ptr += len;
		Sockbuf_advance(&rbuf, rbuf.ptr - rbuf.buf);
		return -1;
...
{
	if (Packet_printf(&wbuf, "%c%hd", PKT_POWER, (int) (power * 256.0)) == -1) {
		return -1;
	}
	return 0;
}
...
{
	if (Packet_printf(&wbuf, "%c%hd", PKT_POWER_S, (int) (power_s * 256.0)) == -1) {
		return -1;
	}
	return 0;
}
...
{
	if (Packet_printf(&wbuf, "%c%hd", PKT_TURNSPEED, (int) (turnspeed * 256.0)) == -1) {
		return -1;
	}
	return 0;
}
...
{
	if (Packet_printf(&wbuf, "%c%hd", PKT_TURNSPEED_S, (int) (turnspeed_s * 256.0)) == -1) {
		return -1;
	}
	return 0;
}
...
{
	if (Packet_printf(&wbuf, "%c%hd", PKT_TURNRESISTANCE, (int) (turnresistance * 256.0)) == -1) {
		return -1;
	}
	return 0;
}
...
{
	if (Packet_printf(&wbuf, "%c%hd", PKT_TURNRESISTANCE_S, (int) (turnresistance_s * 256.0)) ==
	    -1) {
		return -1;
	}
	return 0;
...
int Send_display(void)
{
	...
	if (width_wanted == ext_view_width &&
	    ...
	    debris_colors == num_spark_colors && spark_rand == old_spark_rand && last_loops != 0) {
		return 0;
	}
	...
	if (simulating) {
		...
		Check_view_dimensions();
			       ...
			       width_wanted, height_wanted, num_spark_colors, spark_rand) == -1) {
		return -1;
	}
