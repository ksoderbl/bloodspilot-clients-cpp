/* $Id: about.c,v 1.5 2007/09/27 23:30:58 kps Exp $
 *
...
int DrawShadowText(Display *dpy, Window w, GC gc,
				   int x_border, int y_start, const char *str, unsigned long fg, unsigned long bg)
{
	...
	case 1:
		...
		for (i = first; i <= last; i++)
		{
			...
			y = DrawShadowText(dpy, about_w, textGC,
							   ...
							   Item_get_text(i), colors[WHITE].pixel,
							   colors[BLACK].pixel);
			...
			XFillRectangle(dpy, about_w, textGC,
						   BORDER, box_start, 2 * ITEM_SIZE + 2 * BORDER,
						   box_end - box_start);
			...
			if (about_page == 0 && itemsplit == -1 && box_end >= (ABOUT_WINDOW_HEIGHT - BORDER * 2 - 4 - (2 * BTN_BORDER + buttonFont->ascent + buttonFont->descent)))
			{
				...
				XFillRectangle(dpy, about_w, textGC,
							   BORDER, box_start, ABOUT_WINDOW_WIDTH,
							   box_end - box_start);
				XSetForeground(dpy, textGC, colors[WHITE].pixel);
				break;
			}
...
static void About_create_window(void)
{
	const int windowWidth = ABOUT_WINDOW_WIDTH,
			  buttonWindowHeight = 2 * BTN_BORDER + buttonFont->ascent + buttonFont->descent, windowHeight = ABOUT_WINDOW_HEIGHT;
	int textWidth;
	...
	about_w = XCreateWindow(dpy,
							...
							0, 0, windowWidth, windowHeight, 2, dispDepth, InputOutput, visual,
							mask, &sattr);
	...
	about_close_b = XCreateSimpleWindow(dpy, about_w,
										...
										2 * BTN_BORDER + textWidth, buttonWindowHeight, 0, 0,
	...
	XSelectInput(dpy, about_close_b, ExposureMask | ButtonPressMask | ButtonReleaseMask);
	XSelectInput(dpy, about_next_b, ExposureMask | ButtonPressMask | ButtonReleaseMask);
	XSelectInput(dpy, about_prev_b, ExposureMask | ButtonPressMask | ButtonReleaseMask);
...
void Expose_button_window(int color, Window w)
{
	...
	if (w == about_close_b)
		ShadowDrawString(dpy, w, buttonGC,
						 ...
						 "CLOSE", colors[WHITE].pixel, colors[BLACK].pixel);
	if (w == about_next_b)
		ShadowDrawString(dpy, w, buttonGC,
						 BTN_BORDER, buttonFont->ascent + BTN_BORDER,
						 "NEXT", colors[WHITE].pixel, colors[BLACK].pixel);
	if (w == about_prev_b)
		ShadowDrawString(dpy, w, buttonGC,
						 BTN_BORDER, buttonFont->ascent + BTN_BORDER,
						 "PREV", colors[WHITE].pixel, colors[BLACK].pixel);
}
...
int Keys_callback(int widget_desc, void *data, const char **unused)
{
	...
	if (keys_created == false)
	{
		...
		char *buf = calloc(bufsize, 1), *end = buf, *help, *str;
