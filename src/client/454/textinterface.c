/* $Id: textinterface.c,v 1.5 2007/09/27 23:30:59 kps Exp $
 *
...
static bool Get_contact_message(sockbuf_t *sbuf, const char *contact_server,
							 ...
							 int auto_shutdown, char *shutdown_reason, Connect_param_t *conpar)
{
	...
	for (;;)
	{
		...
		else
		{
			...
			if (sock_connect(&ibuf->sock, conpar->server_addr, conpar->server_port) ==
					SOCK_IS_ERROR &&
				!dgram_one_socket)
			{
		...
		for (i = 0; i < count && !connected; i++)
		{
			...
			do
			{
				...
				if (Get_contact_message(&sbuf, servers[i], conpar))
				{
					...
					connected =
						Connect_to_server(auto_connect, list_servers,
