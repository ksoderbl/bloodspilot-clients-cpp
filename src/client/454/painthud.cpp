/* $Id: painthud.c,v 1.9 2007/12/08 16:02:48 kps Exp $
 *
...
#define METER_HEIGHT 8
	...
	const int mw1_4 = METER_WIDTH / 4,
			  mw2_4 = METER_WIDTH / 2, mw3_4 = 3 * METER_WIDTH / 4, mw4_4 = METER_WIDTH, BORDER = 5;
	int x, xstr;
	...
	if (!blockBitmaps)
	{
		Rectangle_add(RED, x + 2, y + 2,
					  (int)(((METER_WIDTH - 3) * val) / (max ? max : 1)),
					  METER_HEIGHT - 3);
		SET_FG(colors[WHITE].pixel);
		rd.drawRectangle(dpy, p_draw, gc,
						 WINSCALE(x), WINSCALE(y), WINSCALE(METER_WIDTH),
						 WINSCALE(METER_HEIGHT));
		Erase_4point(WINSCALE(x), WINSCALE(y), WINSCALE(METER_WIDTH),
					 WINSCALE(METER_HEIGHT));
	...
	else
	{
		...
		PaintMeter(p_draw, BM_METER, WINSCALE(x), WINSCALE(y), WINSCALE(METER_WIDTH),
				   WINSCALE(11), width);
		SET_FG(colors[WHITE].pixel);
	}

	rd.drawString(dpy, p_draw, gc,
				  (xstr), WINSCALE(y) + (gameFont->ascent + METER_HEIGHT) / 2, title,
				  strlen(title));
		...
		if (sobj->count > 0)
		{
			if (sobj->count % 3)
			{
				...
				if (wrap(&x, &y))
				{
					...
					rd.drawString(dpy, p_draw, gc, x, y, sobj->msg,
								  sobj->msg_len);
...
void Paint_meters(void)
{
	...
	if (BIT(instruments, SHOW_TURNSPEED_METER) || control_count)
		Paint_meter(-10, 60, "Turnspeed", (int)displayedTurnspeed,
					(int)MAX_PLAYER_TURNSPEED);
	if (control_count > 0)
		control_count--;
	if (BIT(instruments, SHOW_PACKET_SIZE_METER))
		Paint_meter(-10, 80, "Packet", (packet_size >= 4096) ? 4096 : packet_size, 4096);
...
static void Paint_lock(int hud_pos_x, int hud_pos_y)
{
	...
	rd.drawString(dpy, p_draw, gc,
				  ...
				  WINSCALE(hud_pos_y - HUD_SIZE + HUD_OFFSET - BORDER) - gameFont->descent, target->name, target->name_len);
	Erase_rectangle(WINSCALE(hud_pos_x) - target->name_width / 2 - 1,
					...
					target->name_width + 2, gameFont->ascent + gameFont->descent);
	...
	if (BIT(Setup->mode, LIMITED_LIVES) || lock_dist != 0)
	{
		...
		rd.drawString(dpy, p_draw, gc,
					  ...
					  WINSCALE(hud_pos_y - HUD_SIZE + HUD_OFFSET - BORDER) - gameFont->descent, str, 3);
		Erase_rectangle(WINSCALE(hud_pos_x + HUD_SIZE - HUD_OFFSET + BORDER) - 1,
						WINSCALE(hud_pos_y - HUD_SIZE + HUD_OFFSET - BORDER) - gameFont->descent - gameFont->ascent, XTextWidth(gameFont, str, 3) + 2,
						gameFont->ascent + gameFont->descent);
	}
	SET_FG(colors[hudColor].pixel);
	...
	if (lock_dist != 0)
	{
		...
		if (lock_dist > WARNING_DISTANCE || warningCount++ % 2 == 0)
		{
			...
			{
				Arc_add(hudColor, (int)(hud_pos_x + HUD_SIZE * 0.6 * tcos(lock_dir) - size * 0.5),
						(int)(hud_pos_y - HUD_SIZE * 0.6 * tsin(lock_dir) - size * 0.5), size, size, 0, 64 * 360);
			}
			else
			{
				SET_FG(colors[hudLockColor].pixel);
				x = (int)(hud_pos_x + HUD_SIZE * 0.6 * tcos(lock_dir) - size * 0.5), y = (int)(hud_pos_y - HUD_SIZE * 0.6 * tsin(lock_dir) - size * 0.5), rd.fillArc(dpy, p_draw, gc, WINSCALE(x), WINSCALE(y), WINSCALE(size), WINSCALE(size), 0, 64 * 360);
				Erase_rectangle(WINSCALE(x), WINSCALE(y), WINSCALE(size),
								WINSCALE(size));
				SET_FG(colors[hudColor].pixel);
			}
		}
...
void Paint_hudradar(void)
{
	...
	float xf = (float)hrw / (float)Setup->width, yf = (float)hrh / (float)Setup->height;
	...
	for (i = 0; i < num_radar; i++)
	{
		...
		/* paint non-enemy objects in green, if available */
		if ((sz & 0x80) != 0)
		{
			sz &= ~0x80;
			color = maxColors > 4 ? 4 : BLUE;
		}

		{
			int x = radar_ptr[i].x * hrscale - (world.x + ext_view_width / 2) * xf;

			int y = radar_ptr[i].y * hrscale - (world.y + ext_view_height / 2) * yf;
			...
			Arc_add(color,
					...
					-y + ext_view_height / 2 - sz / 2, sz, sz, 0, 64 * 360);
		}
	}
}
...
void Paint_HUD(void)
{
	...
	int hud_pos_y;
	int did_fuel = 0;
	int i, j, maxWidth = -1, rect_x, rect_y, rect_width, rect_height;
	...
	if (BIT(instruments, SHOW_HUD_HORIZONTAL))
	{
		rd.drawLine(dpy, p_draw, gc,
					...
					WINSCALE(hud_pos_x + HUD_SIZE),
					WINSCALE(hud_pos_y - HUD_SIZE + HUD_OFFSET));
		Erase_segment(0, WINSCALE(hud_pos_x - HUD_SIZE),
					  WINSCALE(hud_pos_y - HUD_SIZE + HUD_OFFSET),
					  WINSCALE(hud_pos_x + HUD_SIZE),
					  WINSCALE(hud_pos_y - HUD_SIZE + HUD_OFFSET));
		rd.drawLine(dpy, p_draw, gc, WINSCALE(hud_pos_x - HUD_SIZE),
					WINSCALE(hud_pos_y + HUD_SIZE - HUD_OFFSET),
					WINSCALE(hud_pos_x + HUD_SIZE),
					WINSCALE(hud_pos_y + HUD_SIZE - HUD_OFFSET));
		Erase_segment(0, WINSCALE(hud_pos_x - HUD_SIZE),
	...
	if (BIT(instruments, SHOW_HUD_VERTICAL))
	{
		rd.drawLine(dpy, p_draw, gc,
					...
					WINSCALE(hud_pos_x - HUD_SIZE + HUD_OFFSET),
					WINSCALE(hud_pos_y + HUD_SIZE));
		Erase_segment(0, WINSCALE(hud_pos_x - HUD_SIZE + HUD_OFFSET),
					  WINSCALE(hud_pos_y - HUD_SIZE),
					  WINSCALE(hud_pos_x - HUD_SIZE + HUD_OFFSET),
					  WINSCALE(hud_pos_y + HUD_SIZE));
		rd.drawLine(dpy, p_draw, gc, WINSCALE(hud_pos_x + HUD_SIZE - HUD_OFFSET),
					WINSCALE(hud_pos_y - HUD_SIZE),
					WINSCALE(hud_pos_x + HUD_SIZE - HUD_OFFSET),
					WINSCALE(hud_pos_y + HUD_SIZE));
		Erase_segment(0, WINSCALE(hud_pos_x + HUD_SIZE - HUD_OFFSET),
	...
	for (i = 0; i < NUM_ITEMS; i++)
	{
		...
		if (num >= 0)
		{
			...
			/* Paint item symbol */
			Paint_item_symbol((u_byte)i, p_draw, gc, horiz_pos - ITEM_SIZE, vert_pos,
							  ITEM_HUD);
			...
			rd.drawString(dpy, p_draw, gc,
						  ...
						  vert_pos + ITEM_SIZE / 2 + gameFont->ascent / 2, str, len);
	...
	if (rect_width > 0)
	{
		...
		rect_x -= rect_width;
		Erase_rectangle(rect_x - 1, rect_y - 4, rect_width + 2, rect_height + 8);
	}
	...
	if (fuelCount || fuelSum < fuelLevel3)
	{
		...
		rd.drawString(dpy, p_draw, gc,
					  WINSCALE(hud_pos_x + HUD_SIZE - HUD_OFFSET + BORDER),
					  WINSCALE(hud_pos_y + HUD_SIZE - HUD_OFFSET + BORDER) + gameFont->ascent, str, strlen(str));
		Erase_rectangle(WINSCALE(hud_pos_x + HUD_SIZE - HUD_OFFSET + BORDER) - 1,
						WINSCALE(hud_pos_y + HUD_SIZE - HUD_OFFSET + BORDER),
		...
		if (numItems[ITEM_TANK])
		{
			...
			rd.drawString(dpy, p_draw, gc,
						  WINSCALE(hud_pos_x + HUD_SIZE - HUD_OFFSET + BORDER),
						  WINSCALE(hud_pos_y + HUD_SIZE - HUD_OFFSET + BORDER) + gameFont->descent + 2 * gameFont->ascent, str, strlen(str));
			Erase_rectangle(WINSCALE(hud_pos_x + HUD_SIZE - HUD_OFFSET + BORDER) - 1, WINSCALE(hud_pos_y + HUD_SIZE - HUD_OFFSET + BORDER) + gameFont->descent + gameFont->ascent,
							XTextWidth(gameFont, str, strlen(str)) + 2,
							gameFont->ascent + gameFont->descent);
		}
	}
	...
	for (i = 0, j = 0; i < MAX_SCORE_OBJECTS; i++)
	{
		...
		if (sobj->hud_msg_len > 0)
		{
			...
			rd.drawString(dpy, p_draw, gc,
						  ...
						  WINSCALE(hud_pos_y + HUD_SIZE - HUD_OFFSET + BORDER) + gameFont->ascent + j * (gameFont->ascent + gameFont->descent), sobj->hud_msg,
						  sobj->hud_msg_len);
	...
	if (time_left > 0)
	{
		...
		rd.drawString(dpy, p_draw, gc,
					  ...
					  WINSCALE(hud_pos_y - HUD_SIZE + HUD_OFFSET - BORDER) - gameFont->descent, str, strlen(str));
		Erase_rectangle(WINSCALE(hud_pos_x - HUD_SIZE + HUD_OFFSET - BORDER) - size - 1, WINSCALE(hud_pos_y - HUD_SIZE + HUD_OFFSET - BORDER) - gameFont->ascent - gameFont->descent, size + 2,
						gameFont->ascent + gameFont->descent);
	}

	/* Update the modifiers */
	modlen = strlen(mods);
	rd.drawString(dpy, p_draw, gc, WINSCALE(hud_pos_x - HUD_SIZE + HUD_OFFSET - BORDER) - XTextWidth(gameFont, mods, modlen),
				  WINSCALE(hud_pos_y + HUD_SIZE - HUD_OFFSET + BORDER) + gameFont->ascent, mods, strlen(mods));
	...
	if (autopilotLight)
	{
		...
		rd.drawString(dpy, p_draw, gc,
					  ...
					  WINSCALE(hud_pos_y - HUD_SIZE + HUD_OFFSET - BORDER) - gameFont->descent * 2 - gameFont->ascent, autopilot,
					  sizeof(autopilot) - 1);
		...
		Erase_rectangle(WINSCALE(hud_pos_x) - text_width / 2,
						...
						text_width + 2, gameFont->ascent + gameFont->descent);
	}
	...
	if (BIT(instruments, SHOW_FUEL_GAUGE) == 0 || !((fuelCount) || (fuelSum < fuelLevel3 &&
																	((fuelSum < fuelLevel1 && (loops % 4) < 2) || (fuelSum < fuelLevel2 && fuelSum > fuelLevel1 && (loops % 8) < 4) || (fuelSum > fuelLevel2)))))
		return;
	...
	rd.fillRectangle(dpy, p_draw, gc,
					 ...
					 WINSCALE(HUD_OFFSET - (2 * FUEL_GAUGE_OFFSET)), WINSCALE(size));
...
void Paint_messages(void)
{
	...
	const int BORDER = 10, SPACING = messageFont->ascent + messageFont->descent + 1;
	message_t *msg;
	int msg_color;
...
#ifndef _WINDOWS
			|| !selectionAndHistory || (selection.draw.state != SEL_PENDING && selection.draw.state != SEL_EMPHASIZED)
#endif
		)
		{
		...
		if (selectionAndHistory && selection.draw.state == SEL_EMPHASIZED && i < maxMessages && TALK_MSG_SCREENPOS(last_msg_index, i) >= selection.draw.y1 && TALK_MSG_SCREENPOS(last_msg_index, i) <= selection.draw.y2)
		{
			...
			else if (TALK_MSG_SCREENPOS(last_msg_index, i) == selection.draw.y1)
			{
				...
				else
				{
					...
					xoff2 =
						XTextWidth(messageFont, msg->txt, selection.draw.x1);
					...
					else
					{
						...
						else
						{
							/*___xxx___*/
							/*       ^ */
							l2 = selection.draw.x2 - selection.draw.x1 +
								 1;
							ptr3 = &(msg->txt[selection.draw.x2 + 1]);
							xoff3 =
								XTextWidth(messageFont, msg->txt,
										   selection.draw.x2 + 1);
...
void Add_message(char *message)
{
	 ...
	 */
	if (messagesToStdout == 2 || (messagesToStdout == 1 && message[0] && message[strlen(message) - 1] == ']'))
	{
...
void Paint_recording(void)
{
	...
	Erase_rectangle(x - 1, WINSCALE(10), w + 2, gameFont->ascent + gameFont->descent);
}
