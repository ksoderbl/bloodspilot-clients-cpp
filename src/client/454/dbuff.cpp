/* $Id: dbuff.c,v 1.5 2007/09/27 23:30:58 kps Exp $
 *
...
dbuff_state_t *start_dbuff(Display *display, Colormap xcolormap, dbuff_t type, int num_planes,
						   XColor *colors)
{
	dbuff_state_t *state;
...
void dbuff_switch(dbuff_state_t *state)
{
	...
	if (state->type == COLOR_SWITCH)
	{
		...
		if (state->cmap_hack.fbfd != -1)
		{
			...
			if (ioctl(state->cmap_hack.fbfd, FBIOPUTCMAP, &state->cmap_hack.hardcmap) ==
				-1)
			{
