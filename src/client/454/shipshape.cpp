/* $Id: shipshape.c,v 1.6 2007/09/27 23:30:59 kps Exp $
 *
...
#define GRID_RESET() (memset(grid.pt, 0, sizeof grid.pt), \
	...
	int i, j, x, y, dx, dy, inx, iny, max, ofNum, ofLeft, ofRight, /* old format */
		shape_version = 0;
	...
	if (shapeLimits)
	{
		const int isLow = -8, isHi = 8, isLeft = 8, isRight = -8,
				  ...
				  horMax = 15, verMax = 15, horMin = -15, verMin = -15, minCount = 3, minSize = 22 + 16;
		int low = 0, hi = 0, left = 0, right = 0, count = 0, change, max = 0, lowest = 0, highest = 0, leftmost = 0, rightmost = 0;
		int invalid = 0;
	...
	if (!(w->pts[0] = (position_t *)malloc(w->num_points * i)) || (w->num_l_gun && !(w->l_gun[0] = (position_t *)malloc(w->num_l_gun * i))) || (w->num_r_gun && !(w->r_gun[0] = (position_t *)malloc(w->num_r_gun * i))) || (w->num_l_rgun && !(w->l_rgun[0] = (position_t *)malloc(w->num_l_rgun * i))) || (w->num_r_rgun && !(w->r_rgun[0] = (position_t *)malloc(w->num_r_rgun * i))) || (w->num_l_light && !(w->l_light[0] = (position_t *)malloc(w->num_l_light * i))) || (w->num_r_light && !(w->r_light[0] = (position_t *)malloc(w->num_r_light * i))) || (w->num_m_rack && !(w->m_rack[0] = (position_t *)malloc(w->num_m_rack * i))))
	{
...
void Convert_ship_2_string(shipshape_t *w, char *buf, char *ext, unsigned shape_version)
{
	...
	if (shape_version >= 0x3200)
	{
		...
		{
			sprintf(&buf[buflen], " %d,%d", (int)w->pts[i][0].x, (int)w->pts[i][0].y);
			buflen += strlen(&buf[buflen]);
		}
		sprintf(&buf[buflen], ")(EN: %d,%d)(MG: %d,%d)",
				(int)w->engine[0].x, (int)w->engine[0].y, (int)w->m_gun[0].x,
				(int)w->m_gun[0].y);
		buflen += strlen(&buf[buflen]);
		...
		if (w->num_l_gun > 0)
		{
			...
			{
				sprintf(&tmp[tmplen], " %d,%d", (int)w->l_gun[i][0].x,
						(int)w->l_gun[i][0].y);
		...
		if (w->num_r_gun > 0)
		{
			...
			{
				sprintf(&tmp[tmplen], " %d,%d", (int)w->r_gun[i][0].x,
						(int)w->r_gun[i][0].y);
				tmplen += strlen(&tmp[tmplen]);
			}
			strcpy(&tmp[tmplen], ")");
		...
		if (w->num_l_rgun > 0)
		{
			...
			{
				sprintf(&tmp[tmplen], " %d,%d", (int)w->l_rgun[i][0].x,
						(int)w->l_rgun[i][0].y);
				tmplen += strlen(&tmp[tmplen]);
			}
			strcpy(&tmp[tmplen], ")");
		...
		if (w->num_r_rgun > 0)
		{
			...
			{
				sprintf(&tmp[tmplen], " %d,%d", (int)w->r_rgun[i][0].x,
						(int)w->r_rgun[i][0].y);
				tmplen += strlen(&tmp[tmplen]);
			}
			strcpy(&tmp[tmplen], ")");
		...
		if (w->num_l_light > 0)
		{
			...
			{
				sprintf(&tmp[tmplen], " %d,%d", (int)w->l_light[i][0].x,
						(int)w->l_light[i][0].y);
				tmplen += strlen(&tmp[tmplen]);
			}
			strcpy(&tmp[tmplen], ")");
		...
		if (w->num_r_light > 0)
		{
			...
			{
				sprintf(&tmp[tmplen], " %d,%d", (int)w->r_light[i][0].x,
						(int)w->r_light[i][0].y);
				tmplen += strlen(&tmp[tmplen]);
			}
			strcpy(&tmp[tmplen], ")");
		...
		if (w->num_m_rack > 0)
		{
			...
			{
				sprintf(&tmp[tmplen], " %d,%d", (int)w->m_rack[i][0].x,
						(int)w->m_rack[i][0].y);
				tmplen += strlen(&tmp[tmplen]);
			}
			strcpy(&tmp[tmplen], ")");
	...
	else
	{
		...
		{
			sprintf(&buf[buflen], "(%d,%d)", (int)w->pts[i][0].x,
					(int)w->pts[i][0].y);
			buflen += strlen(&buf[buflen]);
		}
	}
