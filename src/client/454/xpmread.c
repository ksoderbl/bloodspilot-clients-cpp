/* $Id: xpmread.c,v 1.5 2007/09/27 23:30:59 kps Exp $
 *
...
static void xpm_print_error(XPM_read * xpmr)
{
	...
	sprintf(buf, "XPM read %s%s%s",
		...
		xpmr->error_str ? ": " : "", xpmr->error_str ? xpmr->error_str : "");
	error(buf);
}
...
static int xpm_parse_data(XPM_read * xpmr)
{
	...
	if (sscanf(xpmr->token, "%u %u %u %u",
		   &xpmr->xpm->width, &xpmr->xpm->height, &xpmr->xpm->ncolors,
		   &xpmr->xpm->cpp) != 4) {
...
static int xpm_colors_to_pixels(XPM * xpm, enum XPM_key key, unsigned long *pixels)
{
	int i, j, k;
	XColor xcolor;
	char *color_name;
	...
	for (i = 0; i < xpm->ncolors; i++) {
		...
		for (k = key; k >= 0; k--) {
			if (xpm->colors[i].keys[k]) {
				...
				if (XParseColor
				    (dpy, DefaultColormap(dpy, DefaultScreen(dpy)), color_name,
				     &xcolor)) {
					break;
				}
		...
		if (!color_name) {
			for (k = key + 1; k <= XPM_c; k++) {
				if (xpm->colors[i].keys[k]) {
					color_name = xpm->colors[i].keys[k];
					if (XParseColor
					    (dpy, DefaultColormap(dpy, DefaultScreen(dpy)),
					     color_name, &xcolor)) {
						break;
					}
					printf("Can't parse color \"%s\"\n", color_name);
...
static XImage *xpm_convert_to_image(XPM * xpm)
{
	...
	img = XCreateImage(dpy, visual, dispDepth, ZPixmap,
			   ...
			   (dispDepth <= 8) ? 8 : (dispDepth <= 16) ? 16 : 32, 0);
	if (!img) {
...
char **xpm_data_from_image(XImage * img)
{
	...
	size = (6 + 6 + 4 + 4)
	    + ncolors * (2 + 2 + 2 + 10)
	    + img->height * img->width * cpp + num_ptrs * sizeof(char *);
