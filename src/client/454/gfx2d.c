/* $Id: gfx2d.c,v 1.5 2007/09/27 23:30:58 kps Exp $
 *
...
int Picture_init(xp_picture_t * picture, int height, int width, int images)
{
	...
	for (i = 0; i < images; i++) {
		picture->data[i] =
		    (RGB_COLOR *) malloc(picture->width * picture->height * sizeof(RGB_COLOR));
		if (!picture->data[i]) {
			error("Not enough memory.");
			return -1;
		}
...
static int Picture_find_path(const char *filename, char *path, size_t path_size)
{
	...
	if (!strchr(filename, PATHNAME_SEP) && texturePath != NULL) {
		for (dir = texturePath; *dir; dir = colon) {
			if (is_this_windows() || !(colon = strchr(dir, ':'))) {
				len = strlen(dir);
				colon = &dir[len];
			}
			else {
...
int Picture_load(xp_picture_t * picture, const char *filename)
{
	...
	errno = 0;
	if ((c1 = Picture_getc(f)) != 'P' || (c2 = Picture_getc(f)) != '6') {
	...
	if (picture->width * picture->images == width || picture->width == width) {
		...
		error("image size is wrong (%d %d) (%d %d)", picture->width, picture->height, width,
		      height);
		fclose(f);
		return -1;
	}
...
*/
void Picture_set_pixel(xp_picture_t * picture, int image, int x, int y, RGB_COLOR color)
{
...
*/
RGB_COLOR Picture_get_pixel(const xp_picture_t * picture, int image, int x, int y)
{
	if (x < 0 || y < 0 || x >= picture->width || y >= picture->height) {
		return RGB24(0, 0, 0);
		/* this might be an error, but it can be a convenience to allow the
...
*/
static RGB_COLOR Picture_get_pixel_avg(const xp_picture_t * picture, int image, double x, double y)
{
	int r_x, r_y;
...
*/
RGB_COLOR Picture_get_rotated_pixel(const xp_picture_t * picture, int x, int y, int image)
{
...
static void Picture_scale_x_slice(const xp_picture_t * picture, int image, int *r, int *g, int *b,
				  int x, int y, double xscale, double xfrac, double yfrac)
{
	double weight;
	RGB_COLOR col;
...
RGB_COLOR Picture_get_pixel_area(const xp_picture_t * picture, int image, double x1, double y1,
				 double dx, double dy)
{
	int r, g, b;
	double area;
