/* $Id: paintdata.c,v 1.6 2007/12/08 16:02:47 kps Exp $
 *
...
void Erase_do_arc(int x, int y, int width, int height, int angle1, int angle2)
{
	XArc *p;
...
void Erase_do_segment(int width, int x1, int y1, int x2, int y2)
{
	...
	EXPAND(erp->seg_ptr[width], erp->num_seg[width], erp->max_seg[width], XSegment, 1);
...
{
	EXPAND(erp->seg_ptr[0], erp->num_seg[0], erp->max_seg[0], XSegment, n);
...
void Erase_do_points(int width, XPoint * pointp, int n)
{
	...
	EXPAND(erp->seg_ptr[width], erp->num_seg[width], erp->max_seg[width], XSegment, n - 1);
	p = &erp->seg_ptr[width][erp->num_seg[width]];
	for (i = 1; i < n; i++) {
		p->x1 = pointp->x;
		p->y1 = pointp->y;
...
void Erase_do_4point(int x, int y, int width, int height)
{
	...
	EXPAND(erp->seg_ptr[0], erp->num_seg[0], erp->max_seg[0], XSegment, 4);
...
void Segment_end(void)
{
	...
	for (i = 0; i < maxColors; i++) {
		if (num_seg[i] > 0) {
			SET_FG(colors[i].pixel);
			rd.drawSegments(dpy, p_draw, gc, seg_ptr[i], num_seg[i]);
...
int Handle_self(int x, int y, int vx, int vy, int newHeading,
		...
		u_byte * newNumItems, int newCurrentTank, int newFuelSum, int newFuelMax,
		int newPacketSize)
{
	pos.x = x;
	pos.y = y;
...
int Handle_ship(int x, int y, int id, int dir, int shield, int cloak, int eshield, int phased,
		int deflector)
{
	ship_t t;
