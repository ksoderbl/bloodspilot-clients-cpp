/* $Id: talkmacros.c,v 1.5 2007/09/27 23:30:59 kps Exp $
 *
			...
			case TALK_FAST_SPECIAL_TALK_CHAR:
				...
				case '=': /* String comparison */
					...
					if (!strcmp(tmpptr1, tmpptr2))
					{
						/* True */
						if ((tmpptr3 =
								 Talk_macro_get_field(inbuf, 3)) == NULL)
						{
					...
					else if (n_fields == 4)
					{
						/* False */
						if ((tmpptr3 =
								 Talk_macro_get_field(inbuf, 4)) == NULL)
						{
				...
				case 'r':
					...
					if (n_fields <= 0 || nextpos == NULL)
						break;
					if ((tmpptr =
							 Talk_macro_get_field(inbuf, randomMT() % n_fields + 1)) == NULL)
					{
