/* $Id: talk.c,v 1.5 2007/09/27 23:30:59 kps Exp $
 *
...
static void Talk_create_window(void)
{
	...
	talk_w
	    = XCreateSimpleWindow(dpy, draw,
				  ...
				  TALK_OUTSIDE_BORDER, colors[WHITE].pixel, colors[BLACK].pixel);
...
void Talk_cursor(bool visible)
{
	...
	if (visible == false) {
		 ...
		 */
		if (selectionAndHistory && talk_cursor.point < strlen(talk_str)
...
void Talk_map_window(bool map)
{
	...
	if (map == true) {
		...
		XWarpPointer(dpy, None, talk_w,
			     0, 0, 0, 0, TALK_WINDOW_WIDTH - (TALK_WINDOW_HEIGHT / 2),
...
static void Talk_refresh(void)
{
	...
	XDrawString(dpy, talk_w, talkGC, selection.talk.x1 * XTextWidth(talkFont, talk_str, 1)
		    + TALK_INSIDE_BORDER,
		    talkFont->ascent + TALK_INSIDE_BORDER,
...
static void Talk_delete_emphasized_text(void)
{
	...
	if (newlen < oldlen) {
		...
		XDrawString(dpy, talk_w, talkGC,
			    ...
			    talkFont->ascent + TALK_INSIDE_BORDER,
			    &talk_str[talk_cursor.point], oldlen - talk_cursor.point);
		XSetForeground(dpy, talkGC, colors[WHITE].pixel);
	}
	if (talk_cursor.point < newlen) {
		XDrawString(dpy, talk_w, talkGC,
			    talk_cursor.point * onewidth + TALK_INSIDE_BORDER,
			    talkFont->ascent + TALK_INSIDE_BORDER,
			    &new_str[talk_cursor.point], newlen - talk_cursor.point);
	}
	Talk_cursor(true);
...
int Talk_do_event(XEvent * event)
{
	...
	case Expose:
		XClearWindow(dpy, talk_w);
		XDrawString(dpy, talk_w, talkGC,
			    TALK_INSIDE_BORDER, talkFont->ascent + TALK_INSIDE_BORDER, talk_str,
			    strlen(talk_str));
		if (selectionAndHistory && selection.talk.state == SEL_EMPHASIZED) {
			Talk_refresh();
		}
	...
	if (overwrite) {
		...
		if (talk_cursor.point < str_len) {
			...
			XDrawString(dpy, talk_w, talkGC,
				    ...
				    talkFont->ascent + TALK_INSIDE_BORDER,
				    &talk_backup[talk_cursor.point], str_len - talk_cursor.point);
			XSetForeground(dpy, talkGC, colors[WHITE].pixel);
		}
		...
		XDrawString(dpy, talk_w, talkGC,
			    talk_cursor.point * char_width + TALK_INSIDE_BORDER,
			    talkFont->ascent + TALK_INSIDE_BORDER,
			    &tmp_str[talk_cursor.point], new_len - talk_cursor.point);
	}
...
{
	if (talk_created) {
		XMoveResizeWindow(dpy, talk_w, TALK_WINDOW_X, TALK_WINDOW_Y, TALK_WINDOW_WIDTH,
				  TALK_WINDOW_HEIGHT);
	}
}
...
int Talk_place_cursor(XButtonEvent * xbutton, bool pending)
{
	 ...
	 */
	if (cursor_pos < 0 || cursor_pos > TALK_WINDOW_WIDTH / onewidth || y < 0
	    || y >= TALK_WINDOW_HEIGHT) {
		if (Button == 1 && pending) {
...
void Talk_window_cut(XButtonEvent * xbutton)
{
	...
	if (ButtonState == ButtonPress) {
		...
		strncpy(tmp, &talk_str[selection.talk.x1], selection.talk.x2 - selection.talk.x1);
