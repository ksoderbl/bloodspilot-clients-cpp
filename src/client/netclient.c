 ...
 * Local variables.
 */
static sockbuf_t rbuf, cbuf, wbuf;
static frame_buf_t *Frames;
static int (*receive_tbl[256]) (void), (*reliable_tbl[256]) (void);
static int keyboard_delta;
static unsigned magic;
static time_t last_send_anything;
static long last_keyboard_change,
    last_keyboard_update, reliable_offset, talk_pending, talk_sequence_num, talk_last_send;
static long keyboard_change[KEYBOARD_STORE], keyboard_update[KEYBOARD_STORE],
...
static int Uncompress_map(void)
{
	...
	*p;			/* temporary search pointer */
	...
	while (cmp >= Setup->map_data) {
		...
		while (p < cmp) {
			...
			*cmp &= ~SETUP_COMPRESSED;
			...
			cmp--;
			if (ump < cmp) {
				warn("Map uncompression error (%d,%d)", cmp - Setup->map_data,
				     ump - Setup->map_data);
				return -1;
			}
		}
	...
	if (ump != cmp) {
		warn("map uncompress error (%d,%d)", cmp - Setup->map_data, ump - Setup->map_data);
		return -1;
	}
			...
			for (retries = 0;; retries++) {
				if (retries >= 10) {
					warn("Can't read setup after %d retries "
					     "(todo=%d, left=%d)", retries, todo,
					     cbuf.len - (cbuf.ptr - cbuf.buf));
					return -1;
				}
...
int Net_verify(Connect_param_t *conpar)
{
	...
	time_t last;
	char *user_name = conpar->user_name;
	char *nick_name = conpar->nick_name;
	char *disp = conpar->disp_name;
...
int Net_init(char *server, int port)
{
	...
	for (i = 0; i < receive_window_size; i++) {
		Frames[i].loops = 0;
		if (Sockbuf_init
		    (&Frames[i].sbuf, &sock, CLIENT_RECV_SIZE,
	...
	if (Sockbuf_init(&cbuf, NULL, CLIENT_RECV_SIZE, SOCKBUF_WRITE | SOCKBUF_READ | SOCKBUF_LOCK)
	    == -1) {
	...
	/* write buffer */
	if (Sockbuf_init(&wbuf, &sock, CLIENT_SEND_SIZE, SOCKBUF_WRITE | SOCKBUF_DGRAM) == -1) {
...
int Net_flush(void)
{
	...
	if (last_keyboard_ack != last_keyboard_change && last_keyboard_update != last_loops)
		/*
	...
	for (retries = 0;;) {
		if (retries == 0 || (time(NULL) - last) > 1) {
			...
			if ((retries < 5 && Send_shape(shipShape) == -1)
			    || Packet_printf(&wbuf, "%c", PKT_PLAY) <= 0 || Client_power() == -1
#ifdef SOUND
			...
			if (rbuf.ptr[0] != PKT_RELIABLE) {
				if (rbuf.ptr[0] == PKT_QUIT) {
					...
					if (rbuf.ptr + 11 >= rbuf.buf + rbuf.len) {
						printf
						    ("skipping unexpected frame while starting\n");
						continue;
					}
		...
		if (cbuf.ptr[0] != PKT_REPLY) {
			warn("Not a reply packet after play (%d,%d,%d)", cbuf.ptr[0],
			     cbuf.ptr - cbuf.buf, cbuf.len);
			return -1;
		}
		if (Receive_reply(&type, &result) <= 0) {
...
static int Net_packet(void)
{
	...
	while (rbuf.buf + rbuf.len > rbuf.ptr) {
		...
		if (receive_tbl[type] == NULL) {
			warn("Received unknown packet type (%d, %d), " "dropping frame.", type,
			     prev_type);
			Sockbuf_clear(&rbuf);
			break;
		}
			...
			if (result == -1) {
				if (type != PKT_QUIT)
					warn("Processing packet type (%d, %d) failed", type,
					     prev_type);
				return -1;
			}
...
int Net_input(void)
{
	...
	frame_buf_t *frame, *last_frame, *oldest_frame = &Frames[0], tmpframe;
	time_t time_now;
...
int Receive_start(void)
{
	...
	if ((n = Packet_scanf(&rbuf, "%c%ld%ld", &ch, &loops_num, &key_ack)) <= 0)
		return n;
...
int Receive_motd(void)
{
	...
	if ((n = Packet_scanf(&cbuf, "%c%ld%hd%ld", &ch, &off, &len, &size)) <= 0)
		return n;
...
{
	int n;
	short x, y, vx, vy, lockId, lockDist, sFuelSum, sFuelMax, sViewWidth, sViewHeight;
	uint8_t ch, sNumSparkColors, sHeading, sPower, sTurnSpeed,
	    sTurnResistance, sNextCheckPoint, lockDir, sAutopilotLight, currentTank, sStat;
	uint8_t num_items[NUM_ITEMS];
	...
	n = Packet_scanf(&rbuf,
			 ...
			 &ch,
			 &x, &y, &vx, &vy, &sHeading,
			 &sPower, &sTurnSpeed, &sTurnResistance, &lockId, &lockDist, &lockDir,
			 &sNextCheckPoint);
	if (n <= 0)
		return n;
	...
	n = Packet_scanf(&rbuf,
			 ...
			 &sViewWidth, &sViewHeight, &sNumSparkColors, &sStat, &sAutopilotLight);
	if (n <= 0)
		return n;
...
int Receive_refuel(void)
{
	...
	if ((n = Packet_scanf(&rbuf, "%c%hd%hd%hd%hd", &ch, &x_0, &y_0, &x_1, &y_1)) <= 0)
		return n;
...
int Receive_connector(void)
{
	...
	n = Packet_scanf(&rbuf, "%c%hd%hd%hd%hd%c", &ch, &x_0, &y_0, &x_1, &y_1, &tractor);
	if (n <= 0)
		return n;
...
int Receive_laser(void)
{
	...
	if ((n = Packet_scanf(&rbuf, "%c%c%hd%hd%hd%c", &ch, &color, &x, &y, &len, &dir)) <= 0)
		return n;
...
int Receive_ball(void)
{
	...
	if (version < 0x4F14) {
		...
		if ((n = Packet_scanf(&rbuf, "%c%hd%hd%hd%c", &ch, &x, &y, &id, &style)) <= 0)
			return n;
	}
...
int Receive_ship(void)
{
	...
	if ((n = Packet_scanf(&rbuf, "%c%hd%hd%hd" "%c%c", &ch, &x, &y, &id, &dir, &flags)) <= 0)
		return n;
	shield = ((flags & 1) != 0);
	cloak = ((flags & 2) != 0);
	eshield = ((flags & 4) != 0);
	...
	if ((n = Handle_ship(x, y, id, angle, shield, cloak, eshield, phased, deflector)) == -1)
		return -1;
	return 1;
}
...
{				/* since 3.8.0 */
	...
	if ((n = Packet_scanf(&rbuf, "%c%hd%hd%c%c%c", &ch, &x, &y, &wrecktype, &size, &rot)) <= 0)
		return n;
...
{				/* since 4.4.0 */
	...
	if ((n = Packet_scanf(&rbuf, "%c%hd%hd%c%c", &ch, &x, &y, &type_size, &rot)) <= 0)
		return n;
...
int Receive_trans(void)
{
	...
	if ((n = Packet_scanf(&rbuf, "%c%hd%hd%hd%hd", &ch, &x_1, &y_1, &x_2, &y_2)) <= 0)
		return n;
...
int Receive_appearing(void)
{
	...
	short x, y, id, count;
	uint8_t ch;
	if ((n = Packet_scanf(&rbuf, "%c%hd%hd%hd%hd", &ch, &x, &y, &id, &count)) <= 0)
		return n;
...
int Receive_seek(void)
{
	...
	if ((n =
	     Packet_scanf(&cbuf, "%c%hd%hd%hd", &ch, &programmer_id, &robot_id, &sought_id)) <= 0)
		return n;
...
int Receive_player(void)
{
	...
	char nick_name[MAX_CHARS], user_name[MAX_CHARS], host_name[MAX_CHARS], shape[2 * MSG_LEN],
	    *cbuf_ptr = cbuf.ptr;

	if ((n = Packet_scanf(&cbuf,
			      "%c%hd%c%c" "%s%s%s" "%S",
			      &ch, &id, &myteam, &mychar, nick_name, user_name, host_name,
			      shape)) <= 0)
		return n;
	...
	if ((n =
	     Handle_player(id, myteam, mychar, nick_name, user_name, host_name, shape,
			   myself)) == -1)
		return -1;
	return 1;
}
...
int Receive_score_object(void)
{
	...
	if (version < 0x4500 || (version >= 0x4F09 && version < 0x4F11)) {
		short rcv_score;
		n = Packet_scanf(&cbuf, "%c%hd%hu%hu%s", &ch, &rcv_score, &x, &y, msg);
		score = rcv_score;
	}
	else {
		/* newer servers send scores with two decimals */
		int rcv_score;
		n = Packet_scanf(&cbuf, "%c%d%hu%hu%s", &ch, &rcv_score, &x, &y, msg);
...
int Receive_score(void)
{
	...
	if (version < 0x4500 || (version >= 0x4F09 && version < 0x4F11)) {
		short rcv_score;
		n = Packet_scanf(&cbuf, "%c%hd%hd%hd%c", &ch, &id, &rcv_score, &life, &mychar);
		score = rcv_score;
		alliance = ' ';
	}
	else {
		/* newer servers send scores with two decimals */
		int rcv_score;
		n = Packet_scanf(&cbuf, "%c%hd%d%hd%c%c", &ch, &id, &rcv_score, &life, &mychar,
				 &alliance);
		score = (double) rcv_score / 100;
	}
	if (n <= 0)
...
int Receive_target(void)
{
	...
	if ((n = Packet_scanf(&rbuf, "%c%hu%hu%hu", &ch, &num, &dead_time, &damage)) <= 0)
		return n;
...
int Send_ack(long rel_loops)
{
	...
	if ((n = Packet_printf(&wbuf, "%c%ld%ld", PKT_ACK, reliable_offset, rel_loops)) <= 0) {
		if (n == 0)
			return 0;
...
int Receive_reliable(void)
{
	...
	if ((n = Packet_scanf(&rbuf, "%c%hd%ld%ld", &ch, &len, &rel, &rel_loops)) == -1)
		return -1;
	if (n == 0) {
	...
	if (rbuf.ptr + len > rbuf.buf + rbuf.len) {
		warn("Not all reliable data in packet (%d,%d,%d)", rbuf.ptr - rbuf.buf, len,
		     rbuf.len);
		rbuf.ptr += len;
		Sockbuf_advance(&rbuf, rbuf.ptr - rbuf.buf);
		return -1;
...
{
	if (Packet_printf(&wbuf, "%c%hd", PKT_POWER, (int) (pwr * 256.0)) == -1)
		return -1;
	return 0;
}
...
{
	if (Packet_printf(&wbuf, "%c%hd", PKT_POWER_S, (int) (pwr_s * 256.0)) == -1)
		return -1;
	return 0;
}
...
{
	if (Packet_printf(&wbuf, "%c%hd", PKT_TURNSPEED, (int) (turnspd * 256.0)) == -1)
		return -1;
	return 0;
}
...
{
	if (Packet_printf(&wbuf, "%c%hd", PKT_TURNSPEED_S, (int) (turnspd_s * 256.0)) == -1)
		return -1;
	return 0;
}
...
{
	if (Packet_printf(&wbuf, "%c%hd", PKT_TURNRESISTANCE, (int) (turnres * 256.0)) == -1)
		return -1;
	return 0;
}
...
{
	if (Packet_printf(&wbuf, "%c%hd", PKT_TURNRESISTANCE_S, (int) (turnres_s * 256.0)) == -1)
		return -1;
	return 0;
}
...
/*
 * 'team_talk' should be set to true if Net_talk() should
 * make sure that the 'str' string is sent only to
 * friendly (team) players.
 */
int Net_talk(const char *str, bool team_talk)
{
	if (str[0] == '\\') {
		/* it's a clientcommand! */
	...
	/*
	 * Server commands begin with '/'.
	 * Let's not send those to team.
	 */
	if (str[0] != '/' && team_talk) {
		if (BIT(Setup->mode, TEAM_PLAY)) {
			if (self)
				sprintf(talk_str, "%d:", self->team);
		}
		else {
			; /* handle alliances somehow here */
		}
	}
	strlcat(talk_str, str, sizeof talk_str);
...
int Send_display_ng(int width, int height, int sparks, int spark_colors)
{
	...
	if (width_wanted == server_display.view_width &&
	    ...
	    spark_colors == server_display.num_spark_colors && sparks == server_display.spark_rand
	    && last_loops != 0) {
		return 0;
	}

	if (Packet_printf
	    (&wbuf, "%c%hd%hd%c%c", PKT_DISPLAY, width_wanted, height_wanted, spark_colors,
	     sparks) == -1)
		return -1;
...
int Send_modifier_bank(int bank)
{
	if (bank < 0 || bank >= NUM_MODBANKS)
		return -1;
	if (Packet_printf(&wbuf, "%c%c%s", PKT_MODIFIERBANK, bank, modBankStr[bank]) == -1)
		return -1;
	return 0;
}
