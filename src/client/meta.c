...
/* $Id: meta.c,v 1.7 2007/11/28 18:27:51 kps Exp $ */
...
int Welcome_sort_server_list(void)
{
	...
	while ((vp = List_pop_front(old_list)) != NULL) {
		...
		for (it = List_begin(new_list); it != List_end(new_list); LI_FORWARD(it)) {
			...
			if (delta < 0)
				...
				continue;
			delta = strcmp(sip_old->version, sip_new->version);
			if (delta < 0)
				break;
			else if (delta > 0)
				continue;
	...
	for (it = List_begin(new_list); it != List_end(new_list); LI_FORWARD(it)) {
		sip_new = SI_DATA(it);
		printf("%2d %5s %-31s %u", sip_new->users, sip_new->domain, sip_new->hostname,
		       sip_new->port);
...
int Get_meta_data(char *errorstr)
{
	...
	if (!connections) {
		sprintf(errorstr, "Could not establish connections with any metaserver"
			", either the meta is not responding or you have" " network problems");
		return -1;
	}
	...
	if (server_count > 0) {
		sprintf(errorstr, "Received information about %d Internet servers", server_count);
