	...
	if (num_vdecor > 0)
	{
		for (i = 0; i < num_vdecor; i++)
		{
			...
			Gui_paint_decor(vdecor_ptr[i].x, vdecor_ptr[i].y,
							vdecor_ptr[i].xi, vdecor_ptr[i].yi, vdecor_ptr[i].type,
							last, more_y);
		}
	...
	for (yi = min.y; yi <= max.y; yi++)
	{
		for (xi = min.x; xi <= max.x; xi++)
		{
			Gui_paint_decor_dot((int)(xi * dx - BLOCK_SZ / 2),
								(int)(yi * dy - BLOCK_SZ / 2), backgroundPointSize);
		}
	}
}
...
void Paint_objects(void)
{
		...
		for (xoff = min.x; xoff <= max.x; xoff++)
		{
			for (yoff = min.y; yoff <= max.y; yoff++)
			{
				Gui_paint_fuel(fuels[i].bounds.x + xoff * Setup->width,
							   fuels[i].bounds.y + yoff * Setup->height, fuels[i].fuel);
			}
		}
	}
		...
		for (xoff = min.x; xoff <= max.x; xoff++)
		{
			for (yoff = min.y; yoff <= max.y; yoff++)
			{
				Gui_paint_base(bases[i].bounds.x + xoff * Setup->width,
							   ...
							   bases[i].id, bases[i].team, bases[i].type);
			}
		}
	}
	...
	for (i = 0; i < num_checks; i++)
	{
		...
		for (xoff = min.x; xoff <= max.x; xoff++)
		{
			for (yoff = min.y; yoff <= max.y; yoff++)
			{
				Gui_paint_setup_check_ng(checks[i].bounds.x + xoff * Setup->width,
...
void Paint_world(void)
{
	...
	if (hrLimitTime > 0.0)
	{
		Gui_paint_hudradar_limit((int)(world.x + ext_view_width / 2 - hudRadarLimit * MAX_VIEW_SIZE / 2),
	...
	for (ryb = yb; ryb <= ye; ryb++, yi++, y += BLOCK_SZ, mapbase++)
	{
		...
		for (rxb = xb; rxb <= xe; rxb++, xi++, x += BLOCK_SZ, mapptr += Setup->y)
		{
			...
			if (!(type & BLUE_BIT))
			{
				...
				case SETUP_FILLED_NO_DRAW:
					if ((wallDrawMode == DRAW_FILLED || wallDrawMode == DRAW_TEXTURED) && fill_top_left == -1 && oldServer)
						fill_top_left = fill_bottom_left = x;
					break;
				case SETUP_CHECK:
					Gui_paint_setup_check_ng(x, y,
											 (Check_index_by_pos(xi, yi) ==
											  nextCheckPoint));
					break;
				...
				/*FALLTHROUGH*/ case SETUP_SPACE_DOT:
			...
			{
				if (!(wallDrawMode == DRAW_FILLED || wallDrawMode == DRAW_TEXTURED))
				{
					Gui_paint_walls(x, y, type, 0, 0);
				...
				else
				{
					...
					if (fill_top_right != -1)
					{
						Gui_paint_filled_slice(fill_bottom_left,
											   ...
											   fill_top_right,
											   fill_bottom_right, y);
						fill_top_left = fill_top_right = fill_bottom_left =
							fill_bottom_right = -1;
					}
				}
			}
		...
		if (fill_top_left != -1)
		{
			Gui_paint_filled_slice(fill_bottom_left, fill_top_left, x, x, y);
