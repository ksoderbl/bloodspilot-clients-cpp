...
int Check_view_dimensions(void)
{
	...
	if (server_display.view_width != srv_width ||
		...
		server_display.num_spark_colors != num_spark_colors || server_display.spark_rand != spark_rand)
	{
		if (Send_display_ng(srv_width, srv_height, spark_rand, num_spark_colors))
			return -1;
	}
...
static void Determine_team_order(struct team_score *team_order[], struct team_score team[])
{
	int i, j, k;
...
static int Team_heading(int entrynum, int teamnum, int teamlives, double teamscore)
{
	other_t tmp;
	tmp.id = -1;
	tmp.team = teamnum;
...
static int Team_score_table(int entrynum, int teamnum, struct team_score team, other_t **order)
{
	other_t *other;
	int i, j;
	bool drawn = false;
...
void Paint_score_table(void)
{
	...
	struct team_score team[MAX_TEAMS], pausers, *team_order[MAX_TEAMS];
	other_t *other, **order;
	int i, j, entrynum = 0;
	...
	else
	{
		...
		for (i = 0; i < num_playing_teams; i++) {
			entrynum = Team_heading(entrynum,
						team_order[i] - &team[0], team_order[i]->life,
						team_order[i]->score);
		}
#endif
	}
