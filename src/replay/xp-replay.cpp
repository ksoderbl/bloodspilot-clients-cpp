...
static struct button_init
{
	...
	{record_bits, 1, record_width, record_height, recordCallback, BUTTON_RELEASE, 0},
	{rewind_bits, 0, rewind_width, rewind_height, rewindCallback, 0, 1},
	{revplay_bits, 0, revplay_width, revplay_height, revplayCallback, 0, 1},
	{play_bits, 0, play_width, play_height, playCallback, 0, 1},
	{fastf_bits, 0, fastf_width, fastf_height, fastfCallback, 0, 1},
	{pause_bits, 0, pause_width, pause_height, pauseCallback, 0, 1},
	{stop_bits, 0, stop_width, stop_height, stopCallback, BUTTON_PRESSED, 1},
	{eject_bits, 0, eject_width, eject_height, quitCallback, BUTTON_RELEASE, 0},
};
...
static char small_bold_font[] = "-*-helvetica-bold-r-*--14-*-*-*-*-*-iso8859-1";
...
static Pixmap RReadTile(struct xprc *rc)
{
	...
	tile = XCreatePixmap(dpy, RootWindow(dpy, screen_num), width, height, depth);
	if (!tile)
	{
...
static struct rGC *RReadGCValues(struct xprc *rc)
{
	...
	{
		openErrorWindow(rc->ewin, "GC expected on position %ld, not %d", ftell(rc->fp), c);
		return NULL;
	}
	else
	{
...
static void FreeShapes(struct shape *shp)
{
	...
	while (shp)
	{
		...
		case RC_DRAWLINES:
			MyFree(shp->shape.lines.points, shp->shape.lines.npoints * sizeof(XPoint),
				   MEM_POINT);
			break;

		case RC_DRAWSTRING:
			MyFree(shp->shape.string.string, shp->shape.string.length, MEM_STRING);
			break;

		case RC_FILLPOLYGON:
			MyFree(shp->shape.polygon.points,
				   shp->shape.polygon.npoints * sizeof(XPoint), MEM_POINT);
			break;

		case RC_FILLRECTANGLES:
			MyFree(shp->shape.rectangles.rectangles,
				   shp->shape.rectangles.nrectangles * sizeof(XRectangle), MEM_POINT);
			break;

		case RC_DRAWARCS:
			MyFree(shp->shape.arcs.arcs, shp->shape.arcs.narcs * sizeof(XArc),
				   MEM_POINT);
			break;

		case RC_DRAWSEGMENTS:
			MyFree(shp->shape.segments.segments,
				   shp->shape.segments.nsegments * sizeof(XSegment), MEM_POINT);
			break;
...
{
	int c = 0, prev_c;
	struct shape *shp = NULL, *shphead = NULL, *newshp;
	XPoint *xpp;
	XRectangle *xrp;
	XArc *xap;
	XSegment *xsp;
	...
	while (!done)
	{
		...
		case EOF:
			openErrorWindow(rc->ewin, "Premature End-Of-File encountered. Truncating.");
			done = True;
			rc->eof = True;
			continue;
		...
		case RC_DAMAGED:
			...
			case RC_DRAWSTRING:
				...
				shp->shape.string.string = cp =
					(char *)MyMalloc((size_t)c, MEM_STRING);
				while (c--)
					*cp++ = getc(rc->fp);
				break;
...
static int readNewFrame(struct xprc *rc)
{
	...
	{
		openErrorWindow(rc->ewin,
						"Corrupt record file, next frame expected, "
						"not %d.  Truncating.",
						c);
		rc->eof = True;
		return -1;
...
static void allocViewColors(struct xprc *rc)
{
	...
	for (i = 0; i < rc->maxColors; i++)
	{
		...
		for (j = 0; j < i; j++) {
			cp2 = &rc->colors[j];
			if (cp->red == cp2->red && cp->green == cp2->green && cp->blue == cp2->blue) {
				break;
			}
		}
		if (j < i) {
...
#endif
		if (cp->red < 0x0100 &&
			cp->green < 0x0100 && cp->blue < 0x0100 && colormap == DefaultColormap(dpy, screen_num))
		{
			rc->pixels[i] = BlackPixel(dpy, screen_num);
			continue;
		}
		if (cp->red >= 0xFF00 &&
			cp->green >= 0xFF00 && cp->blue >= 0xFF00 && colormap == DefaultColormap(dpy, screen_num))
		{
...
static void drawShapes(struct frame *f, XID drawable, struct xprc *rc)
{
	...
	for (sp = f->shapes; sp != NULL; sp = sp->next)
	{
		if (sp->gc != NULL)
		{
			...
			{
				XSetDashes(dpy, rc->gc, sp->gc->dash_offset, sp->gc->dash_list,
						   sp->gc->num_dashes);
			}
		}
		...
		case RC_DRAWARC:
			XDrawArc(dpy, drawable, rc->gc,
					 ...
					 sp->shape.arc.width, sp->shape.arc.height, sp->shape.arc.angle1,
					 sp->shape.arc.angle2);
			break;
		...
		case RC_DRAWLINE:
			XDrawLine(dpy, drawable, rc->gc,
					  sp->shape.line.x1, sp->shape.line.y1, sp->shape.line.x2,
					  sp->shape.line.y2);
			break;
		...
		case RC_DRAWRECTANGLE:
			XDrawRectangle(dpy, drawable, rc->gc,
						   ...
						   sp->shape.rectangle.width, sp->shape.rectangle.height);
			break;
		...
		case RC_DRAWSTRING:
			...
			XDrawString(dpy, drawable, rc->gc,
						...
						sp->shape.string.string, (int)sp->shape.string.length);
			break;
		...
		case RC_FILLPOLYGON:
			XFillPolygon(dpy, drawable, rc->gc,
						 ...
						 sp->shape.polygon.shape, sp->shape.polygon.mode);
			break;
		...
		case RC_PAINTITEMSYMBOL:
			...
			XFillRectangle(dpy, drawable, rc->gc,
						   sp->shape.symbol.x, sp->shape.symbol.y, ITEM_SIZE,
						   ITEM_SIZE);
		...
		case RC_DRAWARCS:
			XDrawArcs(dpy, drawable, rc->gc, sp->shape.arcs.arcs, sp->shape.arcs.narcs);
			break;

		case RC_DRAWSEGMENTS:
			XDrawSegments(dpy, drawable, rc->gc, sp->shape.segments.segments,
						  sp->shape.segments.nsegments);
			break;

		case RC_DAMAGED:
			if (sp->shape.damage.damaged)
				XFillRectangle(dpy, drawable, rc->gc, 0, 0, f->width, f->height);
			break;
...
static void OverWriteMsg(struct xprc *rc, const char *msg)
{
	...
	XFillRectangle(dpy, rc->topview, rc->gc, area_x, area_y, area_w, area_h);
	XSetForeground(dpy, rc->gc, rc->pixels[WHITE]);
	XSetLineAttributes(dpy, rc->gc, 0, LineSolid, CapButt, JoinMiter);
	XDrawRectangle(dpy, rc->topview, rc->gc, area_x, area_y, area_w, area_h);
	XSetFont(dpy, rc->gc, rc->gameFont->fid);
	XDrawString(dpy, rc->topview, rc->gc, text_x, text_y + font->ascent, msg, len);
	XFlush(dpy);
}
...
static void redrawWindow(struct xprc *rc)
{
	...
	if (attrib.width != (int)rc->cur->width || attrib.height != (int)rc->cur->height)
	{
		...
		XReconfigureWMWindow(dpy, rc->topview, screen_num, CWWidth | CWHeight, &values);
	}
...
static void Init_wm_prop(Window win,
						 ...
						 unsigned min_w, unsigned min_h, unsigned max_w, unsigned max_h, int flags)
{
	XClassHint xclh;
	XWMHints xwmh;
	XSizeHints xsh;
	...
	xsh.x = x;
	xsh.width = w;
	xsh.base_width = xsh.min_width = min_w;
	xsh.max_width = max_w;
	xsh.width_inc = 1;
	xsh.height = h;
	xsh.base_height = xsh.min_height = min_h;
	 ...
	 */
	XSetWMProperties(dpy, win, NULL, NULL, Argv, Argc, &xsh, &xwmh, &xclh);
...
static struct recordwin *Init_recordwindow(unsigned long bg, void *data)
{
	...
	rwin->win = XCreateSimpleWindow(dpy, RootWindow(dpy, screen_num),
									x, y, w, h, 1, BlackPixel(dpy, screen_num), bg);
	rwin->gc = XCreateGC(dpy, rwin->win, 0, NULL);
	XSetForeground(dpy, rwin->gc, BlackPixel(dpy, screen_num));
	XSelectInput(dpy, rwin->win, KeyPressMask | KeyReleaseMask | ButtonPressMask);
	...
	rwin->mark_start_but = CreateButton(dpy, rwin->win,
										...
										image, 0, 0,
										BlackPixel(dpy, screen_num),
										markSaveStart, data, BUTTON_TEXT | BUTTON_RELEASE, 0);
	...
	rwin->mark_end_but = CreateButton(dpy, rwin->win,
									  ...
									  image, 0, 0,
									  BlackPixel(dpy, screen_num),
									  markSaveEnd, data, BUTTON_TEXT | BUTTON_RELEASE, 0);
	...
	rwin->recbut = CreateButton(dpy, rwin->win,
								...
								image, 0, 0,
								BlackPixel(dpy, screen_num),
								saveStartToEndPPM, data, BUTTON_TEXT | BUTTON_RELEASE, 0);
	...
	rwin->recbut = CreateButton(dpy, rwin->win,
								...
								image, 0, 0,
								BlackPixel(dpy, screen_num),
								saveStartToEndXPR, data, BUTTON_TEXT | BUTTON_RELEASE, 0);
	GetButtonSize(rwin->recbut, &w, &h);
	if (x < (int)w)
		x = w;
	y += h + 5;
	...
	XReconfigureWMWindow(dpy, rwin->win, screen_num, CWX | CWY | CWWidth | CWHeight, &values);

	Init_wm_prop(rwin->win, x, y, w, h, 0, 0, 0, 0, PSize | PPosition);
	return (rwin);
}
...
static void openErrorWindow(struct errorwin *ewin, const char *fmt, ...)
{
	...
	XReconfigureWMWindow(dpy, ewin->win, screen_num, CWX | CWY | CWWidth | CWHeight, &values);
...
static struct errorwin *Init_errorwindow(unsigned long bg)
{
	...
	ewin->font = loadQueryFont("-*-helvetica-medium-r-*-*-18-*-*-*-*-*-*-*", ewin->gc);
	...
	ewin->but = CreateButton(dpy, ewin->win, 5, 15,
							 0, 0, image, 0, 0,
							 BlackPixel(dpy, screen_num),
							 closeErrorWindow, (void *)ewin, BUTTON_TEXT | BUTTON_RELEASE, 0);
	XSelectInput(dpy, ewin->win, ExposureMask);
	*ewin->message = '\0';
	ewin->len = 0;
	ewin->grabbed = True;
	Init_wm_prop(ewin->win, x, y, w, h, 0, 0, 0, 0, PSize | PPosition);
	return (ewin);
}
...
static void Init_topview(struct xprc *rc)
{
	...
	rc->topview = XCreateSimpleWindow(dpy, RootWindow(dpy, screen_num),
									  x, y, w, h, 1, BlackPixel(dpy, screen_num),
									  BlackPixel(dpy, screen_num));

	Init_wm_prop(rc->topview, x, y, w, h, 0, 0, 0, 0, PSize | PPosition);
	...
	for (i = 0; i < NUM_ITEMS; i++)
		itemBitmaps[i] =
			XCreateBitmapFromData(dpy, rc->topview, (char *)itemData[i], ITEM_SIZE,
								  ITEM_SIZE);
...
static void Init_topmain(struct xui *ui, struct xprc *rc)
{
	...
	ui->topmain = XCreateSimpleWindow(dpy, RootWindow(dpy, screen_num),
									  0, 0, topw, toph + 100, 1, BlackPixel(dpy, screen_num),
									  ui->mainbg);
	...
	for (i = 0; i < (int)NUM_BUTTONS; i++)
	{
		...
		p.icon = XCreateBitmapFromData(dpy, ui->topmain,
									   (char *)buttonInit[i].data, buttonInit[i].width,
									   buttonInit[i].height);
		...
		ui->buttons[i] =
			CreateButton(dpy, ui->topmain,
						 ...
						 buttonInit[i].callback, (void *)ui, buttonInit[i].flags,
						 buttonInit[i].group);
	...
	for (i = 1; i < 5; i++)
	{
		...
		ui->labels[i].y =
			ui->labels[i - 1].y + ui->boldFont->ascent + ui->boldFont->descent + 2;
	}
	toph = ui->labels[4].y + ui->boldFont->ascent + ui->boldFont->descent + 5;
	for (i = 0; i < 5; i++)
	{
		w = XTextWidth(ui->boldFont, ui->labels[i].name, (int)strlen(ui->labels[i].name));
		if (w > mw)
			mw = w;
	}
	for (i = 0; i < 5; i++)
	...
	XReconfigureWMWindow(dpy, ui->topmain, screen_num, CWX | CWY | CWWidth | CWHeight, &values);

	Init_wm_prop(ui->topmain, topx, topy, topw, toph, 0, 0, 0, 0, PSize | PPosition);
...
static void redrawLabel(struct xui *ui, struct xprc *rc, struct label *lb)
{
...
#if 0
	XClearArea(dpy, ui->topmain, lb->x, lb->y, lw + 2,
		   ui->boldFont->ascent + ui->boldFont->descent, False);
#endif
	XClearArea(dpy, ui->topmain,
			   (int)(lb->x + lw + cw + 1), lb->y,
			   vw + 2, (unsigned)(ui->smallFont->ascent + ui->smallFont->descent), False);
	if (lb->j)
	{
		lw = -lw - cw - vw;
		cw = -cw - vw;
	...
	XDrawString(dpy, ui->topmain, ui->gc,
				(int)(lb->x + 1 + lw), lb->y + ui->boldFont->ascent, lb->name,
				(int)strlen(lb->name));
	XDrawString(dpy, ui->topmain, ui->gc, (int)(lb->x + 1 + cw), lb->y + ui->boldFont->ascent,
				": ", 2);
	XSetFont(dpy, ui->gc, ui->smallFont->fid);
	XDrawString(dpy, ui->topmain, ui->gc,
				(int)(lb->x + 1 + vw), lb->y + ui->smallFont->ascent, value_str,
...
static void redrawError(struct errorwin *ewin)
{
	...
	if (!ewin->grabbed)
		XGrabPointer(dpy, ewin->win, True, 0, GrabModeAsync, GrabModeSync, ewin->win, None,
					 CurrentTime);
...
static void SaveFramesPPM(struct xprc *rc)
{
	...
	{
		openErrorWindow(rc->ewin,
						"First save frame exceeds last save frame, "
						"not saving");
		return;
	}
	if (!rc->seekable)
	...
	pixmap = XCreatePixmap(dpy, RootWindow(dpy, screen_num),
						   rc->view_width, rc->view_height, (unsigned)DefaultDepth(dpy, screen_num));
	...
	for (save = begin; !done; save = save->next)
	{
		sprintf(buf, "Saving frame %d (of %d) ...\n",
				save->number - begin->number + 1, end->number - begin->number + 1);
		OverWriteMsg(rc, buf);
		XSetForeground(dpy, rc->gc, rc->pixels[BLACK]);
		XFillRectangle(dpy, pixmap, rc->gc, 0, 0, rc->view_width, rc->view_height);
		XFlush(dpy);
		if (!save->shapes)
		...
		img =
			XGetImage(dpy, pixmap, 0, 0, rc->view_width, rc->view_height, AllPlanes,
					  ZPixmap);
		...
		if (rc->scale > 0)
		{
			sprintf(buf, "Scaling frame %d (of %d) ...\n",
					save->number - begin->number + 1, end->number - begin->number + 1);
			OverWriteMsg(rc, buf);
			ScalePPM(rgbdata, rc->view_width, rc->view_height, rc->scale, rc->gamma,
					 fp);
		}
	...
	if (rc->scale > 0)
		MyFree(rgbdata, (size_t)(3 * rc->view_width * rc->view_height), MEM_MISC);
	else
...
static XImage *pixmap2image(Pixmap pixmap)
{
	...
	if (!XGetGeometry(dpy, pixmap, &rootw, &x, &y, &width, &height, &border_width, &depth))
		return NULL;

	img = XGetImage(dpy, pixmap, 0, 0, width, height, AllPlanes, ZPixmap);
	if (!img)
		return NULL;
...
static void dox(struct xui *ui, struct xprc *rc)
{
	...
	for (;;)
	{
		...
		while (XEventsQueued(dpy, QueuedAfterFlush) > 0 || (!currentSpeed && !forceRedraw && !frameStep))
		{
			...
			case KeyPress:
				count = XLookupString(&(event.xkey), &c, 1, &keysym, &compose);
				if (count == NoSymbol)
					break;
...
static void usage(void)
{
	...
	printf("    If filename is a dash - then standard input is used.\n"
		   "    Valid options are:\n"
		   "        -scale \"factor\"\n"
...
int main(int argc, char **argv)
{
	...
	for (argi = 1; argi < argc; argi++)
	{
		...
		else if (!strcmp(argv[argi], "-gamma"))
		{
			...
			loopAtEnd = 1;
		else if (!strcmp(argv[argi], "-version") || !strcmp(argv[argi], "--version"))
			version();
		else
		{
			if (!strncmp(argv[argi], "-h", 2) ||
				!strncmp(argv[argi], "--h", 3) || !strcmp(argv[argi], "-?"))
				usage();
			else if (argi < argc - 1)
			{
