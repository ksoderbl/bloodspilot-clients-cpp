	...
	while (!done) {
		...
		default:
			openErrorWindow(rc->ewin,
					...
					"core. Truncating...", c, prev_c, f->number, frame_count,
					frames_in_core);
			done = True;
			continue;
		}
...
static int readNewFrame(struct xprc *rc)
{
	...
	if (c != RC_NEWFRAME) {
		openErrorWindow(rc->ewin,
				"Corrupt record file, next frame expected, " "not %d.  Truncating.",
				c);
		rc->eof = True;
		return -1;
	}
...
static void SaveFramesPPM(struct xprc *rc)
{
	...
	if (begin->number > end->number) {
		openErrorWindow(rc->ewin,
				"First save frame exceeds last save frame, " "not saving");
		return;
	}
	if (!rc->seekable) {
		if (!begin->shapes) {
	...
	pixmap = XCreatePixmap(dpy, RootWindow(dpy, screen_num),
			       rc->view_width, rc->view_height, (unsigned) DefaultDepth(dpy,
											screen_num));
